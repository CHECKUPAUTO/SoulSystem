...
//!
//! Stores full conversation history with metadata, search, and export.
...
impl ConversationStore {
    ...
    /// Remove all sessions (and their messages) where `created_at < older_than`.
    ...
    /// Keep only the `max_sessions` most recently updated sessions; remove the rest.
    /// Returns the number of sessions removed.
...
mod tests {
    ...
    #[test]
    fn test_add_message_with_tool_calls() {
        let dir = tempdir().unwrap();
        let store = ConversationStore::open(&dir.path().join("test.db")).unwrap();
        let session_id = store.create_session("Tool Test", None).unwrap();
    ...
    #[test]
    fn test_search_messages_empty_results() {
        let dir = tempdir().unwrap();
        let store = ConversationStore::open(&dir.path().join("test.db")).unwrap();
    ...
    fn test_export_session_format() {
        ...
        assert!(export.contains("assistant: Hi!"));
    }
    ...
    fn test_get_messages_empty_session() {
        let dir = tempdir().unwrap();
        let store = ConversationStore::open(&dir.path().join("test.db")).unwrap();
        let session_id = store.create_session("Empty", None).unwrap();
