...
fn detect_intent(text: &str) -> String {
    ...
    {
        "question"
    } else if lower.starts_with("fais")
    ...
    {
        "command"
    } else if lower.contains("merci") || lower.contains("bravo") || lower.contains("super") {
        "social"
    } else {
        "work"
    }
...
async fn api_model(
    ...
    *s.spikes.entry("model".to_string()).or_insert(0) += 1;
    ...
    Json(
        serde_json::json!({"speaker_id": req.speaker_id, "formality": profile.formality, "intent": profile.intent, "emotional_state": profile.emotional_state, "interaction_count": profile.interaction_count}),
    )
}
...
async fn api_adapt(
    ...
    *s.spikes.entry("adapt".to_string()).or_insert(0) += 1;
    ...
    Json(
        serde_json::json!({"speaker_id": req.speaker_id, "recommended_tone": tone, "recommended_style": style, "formality_match": true}),
    )
}
...
async fn api_social_context(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    *s.tick_count.entry("context".to_string()).or_insert(0) += 1;
    ...
    if let Some(ctx) = ctx {
        Json(
            serde_json::json!({"group_size": ctx.group_size, "dominant_formality": ctx.dominant_formality, "active_speakers": ctx.active_speakers, "mood": ctx.mood, "hierarchy": ctx.hierarchy}),
        )
    } else {
        Json(
            serde_json::json!({"group_size": 1, "dominant_formality": "neutral", "active_speakers": [], "mood": "neutral", "hierarchy": "flat"}),
        )
    }
}
...
async fn api_profile(
    ...
    *s.spikes.entry("profile".to_string()).or_insert(0) += 1;
    ...
    if let Some(f) = &req.formality {
        profile.formality = f.clone();
    }
    if let Some(p) = &req.preferences {
        profile.preferences = p.clone();
    }
    profile.last_seen = chrono::Utc::now().timestamp_millis();
    Json(
        serde_json::json!({"status": "updated", "speaker_id": req.speaker_id, "formality": profile.formality, "preferences": profile.preferences}),
    )
}
...
async fn main() {
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:{}", PORT))
        ...
        .unwrap();
    tracing::info!(
        "soullink-social v{} listening on port {} (N={})",
        VERSION,
        PORT,
        N
    );
