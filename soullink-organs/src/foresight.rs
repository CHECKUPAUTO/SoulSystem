...
#[derive(Deserialize)]
struct PredictRequest { context: String, horizon_hours: Option<f64> }

async fn api_predict(State(s): State<Arc<AppState>>, Json(req): Json<PredictRequest>) -> Json<serde_json::Value> {
    *s.tick_count.entry("predict".to_string()).or_insert(0) += 1;
    *s.spikes.entry("predict".to_string()).or_insert(0) += 1;
    s.last_stimulus.insert("predict".to_string(), Instant::now());
    ...
    let (predicted, confidence, trend) = if let Some(prev) = existing {
        ...
        let t = if prev.predicted_value > smoothed { "up" } else if prev.predicted_value < smoothed { "down" } else { "stable" };
        (smoothed * (1.0 + 0.01 * horizon), 0.7, t.to_string())
    } else {
        (0.5, 0.3, "unknown".to_string())
    };

    let pred = Prediction { context: req.context.clone(), predicted_value: predicted, confidence, trend: trend.clone(), timestamp: chrono::Utc::now().timestamp_millis() };
    s.predictions.insert(req.context.clone(), pred.clone());

    Json(serde_json::json!({"predicted_value": pred.predicted_value, "confidence": pred.confidence, "trend": pred.trend, "horizon_hours": horizon}))
}
...
#[derive(Deserialize)]
struct PrefetchRequest { target: String, priority: Option<f64> }

async fn api_prefetch(State(s): State<Arc<AppState>>, Json(req): Json<PrefetchRequest>) -> Json<serde_json::Value> {
    ...
    *s.spikes.entry("prefetch".to_string()).or_insert(0) += 1;
    ...
    Json(serde_json::json!({"status": "prefetch_triggered", "target": req.target, "priority": priority, "estimated_latency_ms": 50}))
}

async fn api_foresight_health(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    Json(serde_json::json!({"status": "healthy", "predictions_count": s.predictions.len(), "patterns_count": s.patterns.len(), "uptime_secs": s.start_time.elapsed().as_secs()}))
}
...
async fn api_learn(State(s): State<Arc<AppState>>, Json(req): Json<LearnRequest>) -> Json<serde_json::Value> {
    *s.spikes.entry("learn".to_string()).or_insert(0) += 1;
    if let Some(name) = &req.concept {
        let p = Pattern { name: name.clone(), period_hours: 24.0, strength: 0.5, last_seen: chrono::Utc::now().timestamp_millis() };
...
#[derive(Deserialize)]
struct QueryRequest { query: String, top_k: Option<usize> }

async fn api_query(State(s): State<Arc<AppState>>, Json(req): Json<QueryRequest>) -> Json<serde_json::Value> {
...
async fn main() {
    ...
    tracing::info!("soullink-foresight v{} listening on port {} (N={})", VERSION, PORT, N);
