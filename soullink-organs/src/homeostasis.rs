...
fn default_state() -> Arc<AppState> {
    ...
    // Default thresholds
    s.thresholds.insert("cpu".to_string(), Threshold { metric: "cpu".into(), min: 0.0, max: 80.0, current: 0.0 });
    s.thresholds.insert("memory".to_string(), Threshold { metric: "memory".into(), min: 0.0, max: 90.0, current: 0.0 });
    s.thresholds.insert("latency".to_string(), Threshold { metric: "latency".into(), min: 0.0, max: 500.0, current: 0.0 });
    s.thresholds.insert("queue_depth".to_string(), Threshold { metric: "queue_depth".into(), min: 0.0, max: 100.0, current: 0.0 });
    s
}
...
async fn api_vitals(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    *s.spikes.entry("vitals".to_string()).or_insert(0) += 1;
    ...
    let v = Vitals { cpu_usage: cpu, memory_usage: mem, active_organs: 0, total_organs: 13, avg_latency_ms: 10.0, queue_depth: 0, timestamp: chrono::Utc::now().timestamp_millis() };
    // Update threshold current values
    if let Some(mut t) = s.thresholds.get_mut("cpu") { t.current = cpu; }
    if let Some(mut t) = s.thresholds.get_mut("memory") { t.current = mem; }
    s.vitals.insert("current".to_string(), v.clone());
    Json(serde_json::json!({"cpu_usage": v.cpu_usage, "memory_usage": v.memory_usage, "active_organs": v.active_organs, "total_organs": v.total_organs, "avg_latency_ms": v.avg_latency_ms, "queue_depth": v.queue_depth}))
}

fn read_cpu_usage() -> f64 {
    let Ok(content) = std::fs::read_to_string("/proc/loadavg") else { return 0.0 };
    let parts: Vec<&str> = content.split_whitespace().collect();
    parts.first().and_then(|v| v.parse::<f64>().ok()).unwrap_or(0.0) * 100.0 / 8.0 // 8 cores
}
...
fn read_mem_usage() -> f64 {
    ...
    for line in content.lines() {
        if line.starts_with("MemTotal:") { total = line.split_whitespace().nth(1).and_then(|v| v.parse().ok()).unwrap_or(0.0); }
        if line.starts_with("MemAvailable:") { available = line.split_whitespace().nth(1).and_then(|v| v.parse().ok()).unwrap_or(0.0); }
    }
...
async fn api_regulate(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    *s.spikes.entry("regulate".to_string()).or_insert(0) += 1;
    ...
    if cpu > 80.0 { actions.push("throttle_cpu"); }
    if mem > 90.0 { actions.push("throttle_memory"); }
...
async fn api_recover(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    *s.spikes.entry("recover".to_string()).or_insert(0) += 1;
    ...
    let organs = [
        ("science", 9010), ("mind", 9011), ("engineer", 9012), ("crypto", 9013), ("creative", 9014), ("meta", 9015),
        ("memory", 9030), ("reflex", 9035), ("integration", 9032), ("perception", 9033), ("affect", 9034), ("language", 9036), ("reasoning", 9031),
    ];
    let mut offline = Vec::new();
    for (name, port) in &organs {
        let url = format!("http://127.0.0.1:{}/api/stats", port);
        if minreq::get(&url).send().is_err() { offline.push(*name); }
    }
    Json(serde_json::json!({"status": "recovery_check", "offline_organs": offline, "total_checked": organs.len()}))
}
...
async fn main() {
    ...
    tracing::info!("soullink-homeostasis v{} listening on port {} (N={})", VERSION, PORT, N);
