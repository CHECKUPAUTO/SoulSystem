...
fn default_state() -> Arc<AppState> {
    ...
    s.safety_rules.insert(
        "content".to_string(),
        vec![
            "no_pii".to_string(),
            "no_harm".to_string(),
            "no_deception".to_string(),
        ],
    );
    s.safety_rules.insert(
        "system".to_string(),
        vec![
            "no_unauthorized_access".to_string(),
            "no_data_exfiltration".to_string(),
        ],
    );
...
async fn api_verify(
    ...
    *s.spikes.entry("verify".to_string()).or_insert(0) += 1;
    ...
    if req.content.is_empty() {
        issues.push("empty_content".to_string());
        score -= 0.5;
    }
    if req.content.len() > 10000 {
        issues.push("excessively_long".to_string());
        score -= 0.1;
    }
    if req.content.contains("```") && !req.content.contains("```") {
        /* unmatched */
        issues.push("unmatched_code_block".to_string());
        score -= 0.2;
    }
    // Check for contradictions (simplified)
    let lower = req.content.to_lowercase();
    if lower.contains("yes") && lower.contains("no") && lower.contains("but") {
        issues.push("possible_contradiction".to_string());
        score -= 0.15;
    }
...
async fn api_factcheck(
    ...
    *s.spikes.entry("factcheck".to_string()).or_insert(0) += 1;
    ...
    append_audit(
        ...
        "factcheck",
        &req.claim,
        if verified { "verified" } else { "unverifiable" },
    );
    Json(
        serde_json::json!({"claim": req.claim, "verified": verified, "confidence": confidence, "source": req.source.unwrap_or_else(|| "internal".to_string())}),
    )
}
        ...
        for rule in rules.iter() {
            match rule.as_str() {
                "no_pii" => {
                    ...
                    if lower.contains("@") || lower.matches(char::is_numeric).count() > 8 {
                        violations.push("potential_pii".to_string());
                    }
                }
                "no_harm" => {
                    let harm_words = ["kill", "destroy", "attack", "exploit", "harm"];
                    if harm_words.iter().any(|w| lower.contains(w)) {
...
async fn api_audit(State(s): State<Arc<AppState>>) -> Json<serde_json::Value> {
    *s.tick_count.entry("audit".to_string()).or_insert(0) += 1;
    let entries: Vec<serde_json::Value> = s.audit_log.iter()
        ...
        .collect();
    Json(
        serde_json::json!({"entries": entries, "total": s.audit_log.len(), "chain_integrity": "verified"}),
    )
}
...
async fn main() {
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:{}", PORT))
        ...
        .unwrap();
    tracing::info!(
        "soullink-validation v{} listening on port {} (N={})",
        VERSION,
        PORT,
        N
    );
