...
pub struct HuskStore {
    ...
    /// re-read from disk. A synchronized cache keeps `get` on `&self` and makes
    /// the store safe to share across asynchronous runtime tasks; invalidated on the only
    /// thing that changes a value — `put`/`delete` of that key.
    cache: Arc<RwLock<Lru>>,
}
...
impl HuskStore {
    ...
    pub fn open(
        ...
        Ok(Self {
            ...
            segments,
            next_seq,
            cache: Arc::new(RwLock::new(Lru::new(cache_cap))),
        })
    }
    ...
    pub fn put(&mut self, key: &str, value: Vec<u8>) -> io::Result<()> {
        ...
        self.cache
            .write()
            .expect("cold-index cache lock poisoned")
            .remove(key); // the only thing that can stale the cache
        self.maybe_flush()
    }
