...
//! SoulKernel v0.5.0 — Autonomous Agent Kernel (Niveau 10.0)
//!
...
use soul_kernel::action::Action;
use soul_kernel::autocode;
use soul_kernel::bi_bridge::http_server::{start_bridge_server, BridgeState};
use soul_kernel::bi_bridge::{BiBridge, DownlinkMessage, UplinkMessage};
use soul_kernel::goal_store::GoalFile;
use soul_kernel::hnn_bridge::HnnState;
use soul_kernel::learning::QTable;
use soul_kernel::llm::{LlmEngine, LlmResponse};
use soul_kernel::memory::WeaviateMemory;
use soul_kernel::onaeu_bridge::OnaeuBridge;
use soul_kernel::perception::SystemSnapshot;
use soul_kernel::selfmod::SelfModEngine;
use soul_kernel::{CoreState, HistoryEntry};

mod goal_store;

const _EVOLUTION_LOG: &str = "/tmp/soul_kernel_evolution.log";
            ...
            _ = ticker.tick() => {
                ...
                if should_evolve {
                    ...
                    tokio::spawn(async move {
                        ...
                        let selfmod = SelfModEngine::new("/app/soul-kernel");
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter("info,soul_kernel=debug")
        .with_target(true)
        .with_thread_ids(true)
        .init();

    info!("╔══════════════════════════════════════════════════════════════╗");
    info!("║  SoulKernel v0.5.0 — Autonomous Agent Kernel (Niveau 10.0)   ║");
...
mod tests {
    ...
    fn core_state_persistence() {
        ...
        let _ = fs::remove_file("/tmp/soul_kernel_state.json");
    }
