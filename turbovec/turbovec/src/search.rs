...
/// Inline GEMM: C[nq x dim] = A[nq x dim] @ B[dim x dim]^T
fn matmul_transpose(nq: usize, dim: usize, a: &[f32], b: &[f32]) -> Vec<f32> {
    let mut c = vec![0.0_f32; nq * dim];
    for j in 0..nq {
        for i in 0..dim {
            let mut sum = 0.0_f32;
            for k in 0..dim {
                sum += a[j * dim + k] * b[i * dim + k];
            }
            c[j * dim + i] = sum;
        }
    }
...
pub fn search(
    ...
    // Batched rotation: q_rot = queries @ rotation^T via inline GEMM.
    let mut q_rot = matmul_transpose(nq, dim, queries, rotation);
