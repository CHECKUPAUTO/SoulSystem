    ...
    if path.exists() {
        match tokio::fs::read_to_string(&path).await {
            Ok(content) => match serde_json::from_str::<serde_json::Value>(&content) {
                Ok(data) => {
                    let neurons: Vec<neuron::Neuron> = data
                        .get("neurons")
                        .and_then(|v| serde_json::from_value(v.clone()).ok())
                        .unwrap_or_default();
                    let synapses: Vec<neuron::Synapse> = data
                        .get("synapses")
                        .and_then(|v| serde_json::from_value(v.clone()).ok())
                        .unwrap_or_default();
                    let total_neurons = data
                        ...
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0);
                    let growth_events = data
                        .get("growth_events")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0);
                    let total_spikes = data
                        .get("total_spikes")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0);
                    tracing::info!(
                        "📂 State loaded: {} neurons, {} synapses",
                        neurons.len(),
                        synapses.len()
                    );
                    return BrainState {
