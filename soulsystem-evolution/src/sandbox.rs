...
fn main() {{
    ...
    match agent_code() {{
        Ok(results) => {{
            ...
            println!("SOULSYSTEM_EVO_TESTS:{{}}/{{}}", passed, total);
            println!("SOULSYSTEM_EVO_TIME_US:{{}}", start.elapsed().as_micros());
            println!("SOULSYSTEM_EVO_STATUS:OK");
        }}
        Err(e) => {{
            println!("SOULSYSTEM_EVO_STATUS:ERROR:{{}}", e);
        }}
    }}
}}"#,
                code
    ...
    fn parse_test_results(stdout: &str) -> (usize, usize) {
        for line in stdout.lines() {
            if let Some(rest) = line.strip_prefix("SOULSYSTEM_EVO_TESTS:") {
        ...
        // Si pas de ligne SOULSYSTEM_EVO_TESTS, considérer 1/1 si le programme a tourné
        if stdout.contains("SOULSYSTEM_EVO_STATUS:OK") {
            (1, 1)
        } else {
            (0, 1)
        }
