...
```rust
use soulsystem::math::hessian::{compute_hessian, classify_critical_point, CriticalPointType};
...
```rust
use soulsystem::math::optimizer::Adam;
...
```rust
use soulsystem::math::compression::{truncated_svd, lora_decompose};
...
```rust
use soulsystem::math::langevin::{LangevinIntegrator, LangevinConfig, initial_state};
...
```rust
use soulsystem::math::attention::{scaled_dot_product_attention, MultiHeadAttention};
...
```rust
use soulsystem::math::rl::{PPOConfig, RolloutBuffer, LinearPolicy, ppo_loss, Transition};
...
```rust
use soulsystem::math::contrastive::{info_nce_loss, info_nce_gradient, Projector};
...
```rust
use soulsystem::math::moe::{MixtureOfExperts, MoEConfig};
...
```rust
use soulsystem::math::quantize::{quantize, dequantize, compression_ratio, QuantConfig};
...
```rust
use soulsystem::math::graph::{ConceptGraph, GNNStack, graph_readout};
...
unzip -o soulsystem-v0.2-math.zip -d /opt/soulsystem-evolution
...
Les modules sont exposés via `crate::math::*` dans le projet SoulSystem, ou via `soulsystem::math::*` si utilisé comme dépendance Cargo.
