...
| ID | Invariant | PR | Status |
|----|-----------|----|--------|
| INV-CI-1 | `cargo deny` is a required CI gate. | P | HELD (`deny.toml` repaired against cargo-deny 0.20.2's current schema — the pre-existing config used an obsolete pre-0.16 schema and failed to parse at all, so the gate was configured but non-functional before this PR; `cargo deny check` now runs clean: `advisories ok, bans ok, licenses ok, sources ok`. `.github/workflows/ci.yml`'s new `deny` job runs `EmbarkStudios/cargo-deny-action@v2` on every push/PR. Two genuinely fixable advisories were resolved by upgrading rather than ignored: `anyhow` 1.0.102→1.0.103 (RUSTSEC-2026-0190, unsound `Error::downcast_mut`) and `crossbeam-epoch` 0.9.18→0.9.20 (RUSTSEC-2026-0204, invalid pointer dereference); both verified compatible via `cargo check --workspace --all-targets`. Seven remaining advisories are `unmaintained`-only findings with no available upstream fix and are tracked as explicit, individually-commented `ignore` entries in `deny.toml` rather than silently passing. `cargo audit` was not added as a separate gate — `cargo deny`'s `[advisories]` section already covers the RustSec advisory database it draws from, so a second tool would duplicate the same check rather than add coverage.) |
