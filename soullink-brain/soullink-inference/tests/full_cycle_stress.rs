...
fn full_cycle_vram_lifecycle() {
    ...
    let snap_busy = HardwareSnapshot {
        numa_nodes: vec![
            NumaNodeStatus { node_id: 0, cpu_cores: vec![], memory_total_gb: 64.0, memory_free_gb: 30.0, l3_cache_kb: 30720 },
            NumaNodeStatus { node_id: 1, cpu_cores: vec![], memory_total_gb: 64.0, memory_free_gb: 28.0, l3_cache_kb: 30720 },
        ],
        gpu: GpuStatus {
            vram_total_gb: 8.0,
    ...
    let search_req = GenerateRequest {
        model: "cutlass".into(), prompt: "search".into(),
        max_tokens: 0, temperature: 0.0,
        priority: Priority::VectorSearch, quantization: None,
    };
    match router.route(&search_req, &snap_busy) {
        ExecutionTarget::VramWait => println!("✅ Phase 1: VectorSearch → VramWait (Ollama loaded, 0.5GB free)"),
        other => panic!("Expected VramWait, got {:?}", other),
    }

    // Preprocess should also be VramWait
    let preprocess_req = GenerateRequest {
        model: "dali".into(), prompt: "preprocess".into(),
        max_tokens: 0, temperature: 0.0,
        priority: Priority::Preprocess, quantization: None,
    };
    match router.route(&preprocess_req, &snap_busy) {
        ExecutionTarget::VramWait => println!("✅ Phase 1b: Preprocess → VramWait (0.5GB < 2GB threshold)"),
        other => panic!("Expected VramWait, got {:?}", other),
    }

    // HNN should still work (tiny)
    let hnn_req = GenerateRequest {
        model: "hnn".into(), prompt: "step".into(),
        max_tokens: 0, temperature: 0.0,
        priority: Priority::HnnStep, quantization: None,
    };
    match router.route(&hnn_req, &snap_busy) {
        ExecutionTarget::CutlassHnn => println!("✅ Phase 1c: HnnStep → CutlassHnn (fits in 500MB)"),
    ...
    match router.route(&think_req, &snap_busy) {
        ExecutionTarget::CpuNuma { node, .. } => println!("✅ Phase 1d: Think → CpuNuma-{} (VRAM too low for LLM)", node),
        ExecutionTarget::FallbackOllama => println!("✅ Phase 1d: Think → FallbackOllama (acceptable)"),
    ...
    match router.route(&search_req, &snap_free) {
        ExecutionTarget::CutlassSearch => println!("✅ Phase 2: VectorSearch → CutlassSearch (7.5GB free)"),
...
fn vram_threshold_boundary() {
    ...
    let snap_below = HardwareSnapshot {
        numa_nodes: vec![NumaNodeStatus { node_id: 0, cpu_cores: vec![], memory_total_gb: 64.0, memory_free_gb: 30.0, l3_cache_kb: 30720 }],
        gpu: GpuStatus { vram_total_gb: 8.0, vram_free_gb: 0.9, gpu_temp_c: 70, sm_clock_mhz: 2100, max_sm_clock_mhz: 2790, throttle_active: false },
        timestamp_ms: 0,
    };
    assert!(matches!(router.route(&search_req, &snap_below), ExecutionTarget::VramWait));

    // Just above threshold (1.1GB > 1.0GB)
    let snap_above = HardwareSnapshot {
        numa_nodes: snap_below.numa_nodes.clone(),
        gpu: GpuStatus { vram_total_gb: 8.0, vram_free_gb: 1.1, gpu_temp_c: 70, sm_clock_mhz: 2100, max_sm_clock_mhz: 2790, throttle_active: false },
        timestamp_ms: 0,
    };
    assert!(matches!(router.route(&search_req, &snap_above), ExecutionTarget::CutlassSearch));
