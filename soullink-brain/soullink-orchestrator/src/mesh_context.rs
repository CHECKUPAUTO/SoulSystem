...
impl MeshContext {
    ...
    pub fn build(
        ...
        let turbulence_max = ok_brains.iter()
            .map(|b| b.turbulence)
            .fold(0.0_f64, f64::max);
        let active_neurons_total: u32 = ok_brains.iter().map(|b| b.active_neurons).sum();

        let top_concepts: Vec<ConceptSummary> = merged.iter().take(5).filter_map(|v| {
            Some(ConceptSummary {
                concept:      v.get("concept")?.as_str()?.to_string(),
                score:        v.get("score").and_then(|x| x.as_f64()).unwrap_or(0.0),
                source_brain: v.get("source_brain").and_then(|x| x.as_str()).unwrap_or("?").to_string(),
            })
        }).collect();
    ...
    pub fn format_for_prompt(&self) -> String {
        let concepts_line = if self.top_concepts.is_empty() {
            ...
            self.top_concepts.iter().take(5)
                .map(|c| format!("  - {} (score {:.2}, from {})", c.concept, c.score, c.source_brain))
                .collect::<Vec<_>>()
                .join("\n")
        };

        let brains_line = self.brains.iter()
            .filter(|b| !b.errored)
            .map(|b| format!("  - {}: attractor={}, turbulence={:.2}, active_neurons={}",
        ...
        let errored_note = if self.brains_errored.is_empty() {
            ...
            format!("\n(brains that errored: {})", self.brains_errored.join(", "))
        };
...
fn summarize_brain(name: &str, v: &Value) -> BrainSummary {
    ...
    BrainSummary {
        brain:          name.to_string(),
        attractor:      v.get("attractor").and_then(|x| x.as_str()).unwrap_or("unknown").to_string(),
        turbulence:     v.get("turbulence").and_then(|x| x.as_f64()).unwrap_or(0.0),
        momentum:       v.get("momentum").and_then(|x| x.as_f64()).unwrap_or(0.0),
        active_neurons: v.get("active_neurons").and_then(|x| x.as_u64()).unwrap_or(0) as u32,
        concept_count:  v.get("top_concepts").and_then(|x| x.as_array()).map(|a| a.len()).unwrap_or(0),
        errored:        false,
    }
}
...
fn dominant_attractor(brains: &[&BrainSummary]) -> String {
    ...
    for b in brains {
        let entry = counts.entry(b.attractor.as_str()).or_insert((0, attractor_score(&b.attractor)));
        entry.0 += 1;
    }
    counts.into_iter()
        .max_by(|a, b| {
...
mod tests {
    ...
    fn build_aggregates_telemetry() {
        ...
        results.insert("broken".into(), json!({
    ...
    fn format_handles_no_concepts() {
        let results = HashMap::from([
            ("meta".to_string(), json!({"attractor": "Transient", "turbulence": 0.5})),
        ]);
    ...
    fn dominant_attractor_ties_broken_by_stability() {
        ...
        let results = HashMap::from([
            ("science".to_string(), json!({"attractor": "StableOrbit",      "turbulence": 0.1})),
            ("mind".to_string(),    json!({"attractor": "StrangeAttractor", "turbulence": 0.9})),
        ]);
