# OpenClaw — Système d'Évolution Autonome

## Vue d'ensemble

OpenClaw Evolution est un écosystème computationnel auto-améliorant déployé en **container Docker** à côté d'une instance Ollama existante. Il compose d'agents évolutifs, d'un cerveau web asynchrone, d'un moteur de mutation LLM local, et de couches d'abstraction conceptuelle.
...
|----------|--------|-------------|
| `OPENCLAW_OLLAMA_URL` | `http://localhost:11434` | URL Ollama |
| `OPENCLAW_MODEL` | `llama3` | Modèle LLM |
| `OPENCLAW_MAX_LLM_CONCURRENT` | `2` | Max appels LLM simultanés |
| `OPENCLAW_CONTEXT_WINDOW` | `2048` | Fenêtre de contexte (tokens) |
| `OPENCLAW_POPULATION_SIZE` | `20` | Taille population d'agents |
| `OPENCLAW_MUTATION_RATE` | `0.3` | Taux de mutation |
| `OPENCLAW_SURVIVAL_RATE` | `0.5` | Taux de survie |
| `OPENCLAW_SNAPSHOT_INTERVAL` | `5` | Cycles entre snapshots |
| `OPENCLAW_CYCLE_DELAY` | `5` | Secondes entre cycles |
...
- `openclaw-data` → snapshots + agents
- `openclaw-logs` → logs d'exécution
