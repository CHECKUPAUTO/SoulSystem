...
use crate::agents::core_design::CoreDesign;
use crate::agents::critic::Critic;
use crate::agents::planner::Planner;
use crate::config::Config;
use crate::errors::CoreError;
use crate::llm::LlmClient;
use crate::memory::MemoryStore;
use crate::models::{ExecutionSummary, TaskResult};
...
impl Orchestrator {
    ...
    pub async fn process_task(&self, msg: &TaskMessage) -> Result<TaskResult, CoreError> {
        let overall_deadline = Duration::from_secs(480);
    ...
    async fn run_pipeline(&self, msg: &TaskMessage) -> Result<TaskResult, CoreError> {
        let now = time::OffsetDateTime::now_utc().unix_timestamp();
        let base = AgentBase::from_llm_client(self.llm.as_ref().clone());

        // Step 3: Planner
        let planner = Planner::new(base, Arc::clone(&self.memory));
        let plan_start = Instant::now();
        let plan = tokio::time::timeout(Duration::from_secs(180), planner.plan(&msg.task_text))
            .await
        ...
        let base2 = AgentBase::from_llm_client(self.llm.as_ref().clone());
        let designer = CoreDesign::new(base2);

        let base3 = AgentBase::from_llm_client(self.llm.as_ref().clone());
        let critic = Critic::new(base3);
        ...
        for retry in 0..3 {
            ...
            let report = tokio::time::timeout(Duration::from_secs(60), critic.review(&plan, &sub))
                .await
                .map_err(|_| CoreError::Timeout)?;
            let report = report?;
            self.metrics
...
// Simple md5 wrapper for task hashing
mod task_hash {
