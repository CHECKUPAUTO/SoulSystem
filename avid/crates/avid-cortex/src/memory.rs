...
pub struct EpisodicMemory { pub session_id: Uuid, pub interactions: Vec<Interaction> }
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Interaction { pub timestamp: DateTime<Utc>, pub role: String, pub content: String }
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SemanticFact { pub key: String, pub value: String, pub confidence: f32 }
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProceduralSkill { pub name: String, pub steps: Vec<String> }
...
impl MemorySystem {
    ...
    pub fn add_interaction(&mut self, session_id: Uuid, role: String, content: String) {
        let entry = self.episodic.entry(session_id).or_insert_with(|| EpisodicMemory { session_id, interactions: Vec::new() });
        entry.interactions.push(Interaction { timestamp: Utc::now(), role, content });
    }
    pub fn learn_fact(&mut self, key: String, value: String, confidence: f32) {
        self.semantic.push(SemanticFact { key, value, confidence });
    }
