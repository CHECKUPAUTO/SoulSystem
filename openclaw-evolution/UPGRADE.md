...
- **Impact** : Le prompt Ollama demande maintenant du code avec `println!("OPENCLAW_TESTS:{passed}/{total}")` pour que le harness puisse parser les résultats.
...
cp -r /opt/openclaw-evolution /opt/openclaw-evolution.v1.backup

# Extraire la v0.2 par-dessus
cd /opt/openclaw-evolution
unzip -o /chemin/vers/openclaw-v0.2.zip
...
```
OPENCLAW_MODEL_FAST=qwen3.5:4b     # ← adapter au modèle installé
OPENCLAW_MODEL_DEEP=llama3:8b      # ← adapter au modèle installé  
OPENCLAW_MODEL_EMBED=nomic-embed-text
...
# Restaurer v0.1
cp -r /opt/openclaw-evolution.v1.backup/* /opt/openclaw-evolution/
...
```bash
OPENCLAW_STAGNATION_THRESHOLD=20
```

### "OOM VRAM avec llama3:8b en mode DEEP"
Réduire le contexte DEEP :
```bash
OPENCLAW_CONTEXT_WINDOW_DEEP=2048
```
