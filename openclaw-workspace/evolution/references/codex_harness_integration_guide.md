...
- **OAuth Scope Fix (#64713, commit 58708e6f)**: Codex OAuth scopes were silently dropped during token refresh. Fixed by fuller-stack-dev.
- This is critical for ACP harness reliability — Codex sessions depend on valid OAuth.
- Cross-reference: `oauth_scope_chain_pattern.md` — generalized pattern from this fix
- Regression test added in `tools-invoke-http.cron-regression.test.ts` (commit 6883273)
...
---
## Upstream Reference (Updated 2026-04-12)

**OAuth Scope Fix** (commit 58708e6f): OAuth scopes were silently dropped during Codex authentication.
- Community fix by @fuller-stack-dev
- Scope preservation across refresh flows
- Relevant to all OAuth provider integrations

Related: See `oauth_scope_chain_pattern.md` for generalized pattern.
