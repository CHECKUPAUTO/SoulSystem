...
fn run_scenario(
    ...
    let mut perceiver = PTNLPerceiver::new(
        D_INPUT, D_LATENT, D_TIME, M, T, T_FUTURE, &device).unwrap();
    ...
    for step in T..total {
        ...
        // V6 — t_axis pour Time2Vec (mode pas-de-futur car bench V5).
        let t_axis_vals: Vec<f64> = (0..T + T_FUTURE)
            .map(|i| (i as f64 - T as f64 + 1.0) / T as f64)
            .collect();
        let t_axis = Tensor::from_slice(&t_axis_vals, T + T_FUTURE, &device).unwrap();
