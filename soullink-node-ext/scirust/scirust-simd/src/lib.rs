...
// =============================================================================
// SIMD JIT cache — detect once, reuse for all calls
// =============================================================================

use std::sync::atomic::{AtomicU8, Ordering};

/// Detected SIMD feature level (cached after first call).
/// 3 = AVX-512, 2 = AVX2, 1 = SSE2/NEON, 0 = scalar
pub fn simd_level() -> u8 {
    ...
    let level: u8 = {
        ...
        {
            if std::arch::is_x86_feature_detected!("avx512f") { 3 }
            else if std::arch::is_x86_feature_detected!("avx2")   { 2 }
            else if std::arch::is_x86_feature_detected!("sse2")   { 1 }
            else { 0 }
        }
        #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
        {
            ...
            {
                if std::arch::is_aarch64_feature_detected!("neon") { 1 } else { 0 }
            }
