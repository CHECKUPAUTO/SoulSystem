...
**Vulnerability:** `Action::execute` in `openclaw-u` was vulnerable to command injection and path traversal because it passed unsanitized strings from potentially untrusted sources (LLM, remote downlink) to system commands (`systemctl`, `iptables`, `sh -c`) and file operations.
