...
use scirust_trading_core::{
    Category, CodifiedEvent, Exchange, EventTiming, MarketReaction, MarketState, Polarity, Reliability,
    Side, Symbol, Target,
};
use chrono::Utc;
...
fn main() {
    ...
    println!("\n[1] MarketState:");
    println!("    {} {} @ {:.2}", state.exchange_str(), state.symbol.pair(), state.mid);
    println!("    spread: {:.1} bps, vol: {:.1}%", state.spread_bps, state.realized_vol_pct);
    println!("    imbalance(5): {:.2}, imbalance(20): {:.2}", state.imbalance_5, state.imbalance_20);
    ...
    println!("    source: {}, reliability: {:.2}", event.source, event.source_reliability.value());
    ...
    println!("\n[3] MarketReaction:");
    println!("    direction: {} ({}), magnitude: {:.0} bps", reaction.direction, if reaction.direction > 0 { "up" } else { "down" }, reaction.magnitude);
    println!("    Δ+5m: {:.1} bps, Δ+15m: {:.1} bps, Δ+60m: {:.1} ± {:.1} bps", reaction.delta_5min_bps, reaction.delta_15min_bps, reaction.delta_60min_bps, reaction.delta_60min_std_bps);
    println!("    volume spike: {:.1}x, significant: {}", reaction.volume_spike_ratio, reaction.is_significant());
    ...
    println!("    confidence: {:.2}", enriched.semantic_confidence.unwrap());
