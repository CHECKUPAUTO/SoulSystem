...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.soulsystem/soulsystem.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.soulsystem/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'soulsystem onboard' or set DATABASE_URL environment variable".to_string(),
            })?;
...
/// Default libSQL database path (~/.soulsystem/soulsystem.db).
pub fn default_libsql_path() -> PathBuf {
    soulsystem_base_dir().join("soulsystem.db")
}
