...
impl SecretsConfig {
    ...
    ///    `~/.soulsystem/.env` as `SECRETS_MASTER_KEY=…`.
    ///
    ...
    /// zeroized, but the key ultimately lives in `~/.soulsystem/.env`
    ...
    pub(crate) async fn resolve() -> Result<Self, ConfigError> {
        Self::resolve_with_env_path(&crate::bootstrap::soulsystem_env_path()).await
    }

    /// Testable variant of [`Self::resolve`] that writes its `.env`
    /// fallback to an explicit path. Production code calls
    /// [`Self::resolve`], which targets `~/.soulsystem/.env`.
...
/// soulsystem `.env` file. Returns `None` if the file is missing, the
