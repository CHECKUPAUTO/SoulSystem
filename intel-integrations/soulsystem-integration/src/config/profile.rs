...
//!
//! ```bash
//! SOULSYSTEM_PROFILE=server soulsystem
//! ```
//!
//! **Lookup order** for `SOULSYSTEM_PROFILE=foo`:
//! 1. `~/.soulsystem/profiles/foo.toml`  (user-defined, highest priority)
...
/// Directory under the SoulSystem base dir where user-defined profiles live.
...
/// Read `SOULSYSTEM_PROFILE` and merge the profile onto `settings`.
///
...
pub fn apply_profile(settings: &mut Settings) -> Result<(), ConfigError> {
    let name = match optional_env("SOULSYSTEM_PROFILE")? {
...
fn load_profile(name: &str) -> Result<Settings, ConfigError> {
    ...
    if name.contains('/') || name.contains('\\') || name.contains("..") || name.starts_with('.') {
        return Err(ConfigError::InvalidValue {
            key: "SOULSYSTEM_PROFILE".to_string(),
            message: format!(
...
/// Path to the user-defined profiles directory: `~/.soulsystem/profiles/`.
fn user_profiles_dir() -> PathBuf {
    soulsystem_base_dir().join(PROFILES_DIR)
}
...
mod tests {
    ...
    fn no_profile_is_noop() {
        let _guard = lock_env();
        // Ensure SOULSYSTEM_PROFILE is not set.
        unsafe { std::env::remove_var("SOULSYSTEM_PROFILE") };
    ...
    fn unknown_profile_errors() {
        let _guard = lock_env();
        unsafe { std::env::set_var("SOULSYSTEM_PROFILE", "nonexistent-xyz-987") };
    ...
    fn local_profile_disables_gateway_and_sandbox() {
        let _guard = lock_env();
        unsafe { std::env::set_var("SOULSYSTEM_PROFILE", "local") };
    ...
    fn server_profile_enables_postgres() {
        let _guard = lock_env();
        unsafe { std::env::set_var("SOULSYSTEM_PROFILE", "server") };
    ...
    fn server_multitenant_sets_higher_parallelism() {
        let _guard = lock_env();
        unsafe { std::env::set_var("SOULSYSTEM_PROFILE", "server-multitenant") };
    ...
    fn local_sandbox_enables_sandbox() {
        let _guard = lock_env();
        unsafe { std::env::set_var("SOULSYSTEM_PROFILE", "local-sandbox") };
    ...
    fn user_profile_overrides_builtin() {
        ...
        // `soulsystem_base_dir()` is cached in a `LazyLock`. Instead we test the
