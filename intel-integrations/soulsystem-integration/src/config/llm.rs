...
impl LlmConfig {
    ...
    pub fn for_testing() -> Self {
        Self {
            ...
            session: SessionConfig {
                ...
                session_path: std::env::temp_dir().join("soulsystem-test-session.json"),
            },
            nearai: NearAiConfig {
    ...
    pub(crate) fn resolve(settings: &Settings) -> Result<Self, ConfigError> {
        ...
        // state after `soulsystem models set-provider`, which intentionally writes
        ...
        let openai_codex = if is_openai_codex {
            ...
            let session_path = optional_env("OPENAI_CODEX_SESSION_PATH")?
                .map(PathBuf::from)
                .unwrap_or_else(|| soulsystem_base_dir().join("openai_codex_session.json"));
...
/// Get the default session file path (~/.soulsystem/session.json).
pub fn default_session_path() -> PathBuf {
    soulsystem_base_dir().join("session.json")
}
