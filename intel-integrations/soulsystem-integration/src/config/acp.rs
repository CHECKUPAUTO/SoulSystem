...
//! `~/.soulsystem/acp-agents.json` (disk fallback) and in the database settings
...
pub fn default_config_path() -> PathBuf {
    soulsystem_base_dir().join("acp-agents.json")
}
...
mod tests {
    ...
    async fn test_load_nonexistent_returns_empty() {
        let path = std::env::temp_dir().join("soulsystem-test-nonexistent-acp.json");
