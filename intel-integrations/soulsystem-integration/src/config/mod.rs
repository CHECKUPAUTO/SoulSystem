...
//!
//! `DATABASE_URL` lives in `~/.soulsystem/.env` (loaded via dotenvy early
//! in startup).
...
impl Config {
    ...
    pub fn for_testing(
        ...
        Self {
            ...
            channels: ChannelsConfig {
                ...
                signal: None,
                tui: None,
                wasm_channels_dir: std::env::temp_dir().join("soulsystem-test-channels"),
    ...
    pub async fn from_db_with_toml(
        ...
        crate::bootstrap::load_soulsystem_env();
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.soulsystem/.env`
    ...
    /// If `None`, tries the default path `~/.soulsystem/config.toml` (missing
...
pub(crate) fn resolve_owner_id(settings: &Settings) -> Result<String, ConfigError> {
    let env_owner_id = self::helpers::optional_env("SOULSYSTEM_OWNER_ID")?;
                ...
                .is_some_and(|value| !value.trim().is_empty()))
    {
        WARNED_EXPLICIT_DEFAULT_OWNER_ID.call_once(|| {
            tracing::warn!(
                "SOULSYSTEM_OWNER_ID resolved to the legacy 'default' scope explicitly; durable state will keep legacy owner behavior"
            );
        });
    }
...
mod tests {
    ...
    fn config_for_owner(owner_id: &str) -> Config {
        let tmp = std::env::temp_dir().join(format!("soulsystem-resolve-test-{owner_id}"));
