...
//!
//! See [issue #1328](https://github.com/nearai/soulsystem/issues/1328).
//!
...
mod tests {
    ...
    fn released_migrations_are_immutable() {
        ...
        let lockfile_contents = std::fs::read_to_string(&lockfile_path).unwrap_or_else(|e| {
            panic!(
                "missing {}: {e}\nRun `cargo test -p soulsystem -- --ignored \
    ...
    ///
    /// ```text
    /// cargo test -p soulsystem -- --ignored regenerate_migration_checksums_lockfile
    /// ```
    ///
    ...
    ///   cargo test -p soulsystem -- --ignored \
    ...
    ///
    /// ```text
    /// DATABASE_URL=postgres://localhost/soulsystem_test \
    ...
    async fn realign_repairs_known_bad_checksum_against_postgres() {
        ...
        let database_url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://localhost/soulsystem_test".to_string());
