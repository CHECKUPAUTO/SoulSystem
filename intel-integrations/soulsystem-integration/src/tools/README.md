...
5. Install with `soulsystem tool install path/to/tool.wasm`
...
When running `soulsystem tool auth <tool>`:
...
Both are first-class in the extension system (`soulsystem tool install` handles both), but they have different strengths.

**WASM Tools (SoulSystem native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, SoulSystem can't prevent leaks
