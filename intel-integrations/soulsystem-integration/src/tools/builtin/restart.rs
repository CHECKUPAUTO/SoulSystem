...
//! ## Architecture
//!
//! SoulSystem runs inside a Docker container with an entrypoint loop that monitors exit codes:
//! - **Exit code 0** (clean): Reset failure counter, wait `SOULSYSTEM_RESTART_DELAY` (default 5s), restart
//! - **Exit code ≠ 0** (failure): Increment failure counter, exit after `SOULSYSTEM_MAX_FAILURES` (default 10)
//!
...
impl Tool for RestartTool {
    ...
    fn description(&self) -> &str {
        "Restart the SoulSystem agent process. The process exits cleanly (code 0) and the \
    ...
    async fn execute(
        ...
        // Check if running inside a Docker container via SOULSYSTEM_IN_DOCKER env var.
        // The Docker entrypoint sets this to "true". For local development, it's unset or "false".
        // The entrypoint restart loop only works inside a Docker container (soulsystem-worker).
        let in_docker = std::env::var("SOULSYSTEM_IN_DOCKER")
            .map(|v| v.to_lowercase() == "true")
            .unwrap_or(false);

        tracing::debug!("[RestartTool::execute] SOULSYSTEM_IN_DOCKER={}", in_docker);
        ...
        if !in_docker {
            ...
            return Err(ToolError::ExecutionFailed(
                "Restart is only available when running inside the Docker container. \
                 For local development, please restart SoulSystem manually."
                    .to_string(),
            ));
        }
        ...
        //
        // - The soulsystem-worker Docker container runs an entrypoint loop that monitors
        //   the exit code of the `soulsystem run` process:
        //   * Exit code 0 = clean restart: reset failure counter, wait SOULSYSTEM_RESTART_DELAY
        //     (default 5s), then restart the process
        //   * Exit code ≠ 0 = failure: increment counter, exit after SOULSYSTEM_MAX_FAILURES
        //     (default 10 failures)
        //
        ...
        let restart_disabled = std::env::var("SOULSYSTEM_DISABLE_RESTART")
            .map(|v| {
        ...
        tokio::spawn(async move {
            ...
            if !restart_disabled {
                ...
                tracing::info!(
                    "[RestartTool] Exit disabled (SOULSYSTEM_DISABLE_RESTART set), skipping std::process::exit(0)"
                );
            }
        });
        ...
        let msg = format!(
            "Restarting in {delay} second(s). The process will exit cleanly and the \
             entrypoint restart loop will bring SoulSystem back online."
        );
...
mod tests {
    ...
    fn enable_docker_env() {
        unsafe {
            std::env::set_var("SOULSYSTEM_IN_DOCKER", "true");
            // Keep restart tests from terminating the shared lib-test process.
            std::env::set_var("SOULSYSTEM_DISABLE_RESTART", "true");
        }
    }
    ...
    fn test_restart_tool_description() {
        ...
        assert!(desc.contains("SoulSystem"));
    ...
    fn test_restart_tool_requires_docker_environment() {
        // Test that restart is rejected when not in Docker (SOULSYSTEM_IN_DOCKER not set or false)
        ...
        if !in_docker {
            // Simulating what the tool would do when SOULSYSTEM_IN_DOCKER is not set
            assert!(
                !in_docker,
                "Test environment should have SOULSYSTEM_IN_DOCKER unset or false"
            );
        }
    }
}
