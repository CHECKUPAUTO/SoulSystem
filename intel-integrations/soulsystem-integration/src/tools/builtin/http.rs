...
const USER_AGENT: &str = concat!(
    ...
    " (https://github.com/nearai/soulsystem)"
);
...
impl Tool for HttpTool {
    ...
    fn requires_approval(&self, params: &serde_json::Value) -> ApprovalRequirement {
        let has_credentials = soulsystem_safety::params_contain_manual_credentials(params)
...
mod tests {
    ...
    fn test_save_to_accepts_simple_tmp_path() {
        let path = validate_save_to_path("/tmp/test_soulsystem_photo.jpg").unwrap();
    ...
    fn test_save_to_accepts_nested_tmp_path() {
        let path = validate_save_to_path("/tmp/soulsystem_test_subdir/nested/file.png").unwrap();
        assert!(path.starts_with("/tmp"));
        let _ = std::fs::remove_dir_all("/tmp/soulsystem_test_subdir");
    }
