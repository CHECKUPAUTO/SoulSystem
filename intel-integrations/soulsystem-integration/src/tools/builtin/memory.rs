...
/// mission. See `soulsystem_engine::runtime::self_modify_enabled` for the
    ...
    async fn execute(
        ...
        if let Some(old_str) = old_string {
            ...
            if needs_py_validation {
                ...
                if let Err(reason) = soulsystem_engine::executor::validate_python_syntax(&preview) {
                    return Err(ToolError::InvalidParameters(format!(
        ...
        if needs_py_validation {
            ...
            if let Err(reason) = soulsystem_engine::executor::validate_python_syntax(&final_content) {
                return Err(ToolError::InvalidParameters(format!(
                    "orchestrator patch has invalid Python syntax: {reason}"
                )));
            }
...
mod tests {
    ...
    // See: https://github.com/nearai/soulsystem/pull/1118
