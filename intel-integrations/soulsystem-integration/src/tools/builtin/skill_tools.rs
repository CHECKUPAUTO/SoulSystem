...
use soulsystem_skills::catalog::{
    SkillCatalog, catalog_entry_is_installed, resolve_catalog_slug_for_name,
};
use soulsystem_skills::registry::SkillRegistry;
    ...
    Fut: Future<Output = Result<String, SkillFetchError>>,
{
    ...
    while let Some(dep_name) = queue.pop_front() {
        if !soulsystem_skills::validate_skill_name(&dep_name) {
            report
                .failed
        ...
        let download_url = soulsystem_skills::catalog::skill_download_url(registry_url, &dep_name);
        match fetcher(download_url).await {
            Ok(dep_content) => {
                let normalized = soulsystem_skills::normalize_line_endings(&dep_content);
                match soulsystem_skills::registry::SkillRegistry::prepare_install_to_disk(
                    &user_dir,
                    &dep_name,
                    &normalized,
                )
...
impl Tool for SkillInstallTool {
    ...
    async fn execute(
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            let download_url = soulsystem_skills::catalog::skill_download_url(
                self.catalog.registry_url(),
                &download_key,
            );
        ...
        let normalized = soulsystem_skills::normalize_line_endings(&content);
        ...
        let (user_dir, skill_name_from_parse, install_content) = {
            ...
            let (skill_name, install_content) =
                soulsystem_skills::registry::SkillRegistry::resolve_install_content(
                    &normalized,
                    requested_identifier.as_deref(),
                )
        ...
        let (skill_name, loaded_skill) =
            soulsystem_skills::registry::SkillRegistry::prepare_install_to_disk(
                &user_dir,
...
pub(crate) async fn fetch_skill_content(url: &str) -> Result<String, SkillFetchError> {
    ...
    // Basic size check
    if content.len() as u64 > soulsystem_skills::MAX_PROMPT_FILE_SIZE {
        return Err(SkillFetchError::from_message(format!(
            "Skill content too large: {} bytes (max {} bytes)",
            content.len(),
            soulsystem_skills::MAX_PROMPT_FILE_SIZE
        )));
    }
...
impl Tool for SkillRemoveTool {
    ...
    async fn execute(
        ...
        soulsystem_skills::registry::SkillRegistry::delete_skill_files(&skill_path)
            .await
            .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
...
mod tests {
    ...
    fn test_find_catalog_slug_for_display_name() {
        let entries = vec![soulsystem_skills::catalog::CatalogEntry {
    ...
    fn test_resolve_catalog_slug_for_display_name_is_ambiguous() {
        let entries = vec![
            soulsystem_skills::catalog::CatalogEntry {
    ...
    async fn test_chain_install_recurses_into_transitive_skill_dependencies() {
        ...
        let dep_a_url = soulsystem_skills::catalog::skill_download_url(registry_url, "dep-a");
        let dep_b_url = soulsystem_skills::catalog::skill_download_url(registry_url, "dep-b");
