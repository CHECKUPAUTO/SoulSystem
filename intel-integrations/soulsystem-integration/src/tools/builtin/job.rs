...
impl CreateJobTool {
    ...
    fn mode_description(&self) -> String {
        let mut desc =
            String::from("Execution mode. 'worker' (default) uses the SoulSystem sub-agent.");
        if self.claude_code_enabled() {
            desc.push_str(
        ...
        for (secret_name, env_var_value) in creds_obj {
            ...
            if !exists {
                return Err(ToolError::ExecutionFailed(format!(
                    "secret '{}' not found. Store it first via 'soulsystem tool auth' or the web UI.",
                    secret_name
                )));
            }
...
fn projects_base() -> PathBuf {
    soulsystem_base_dir().join("projects")
}

/// Resolve the project directory, creating it if it doesn't exist.
///
/// Auto-creates `~/.soulsystem/projects/{project_id}/` so every sandbox job has a
...
/// it is validated to fall within `~/.soulsystem/projects/` after canonicalization.
        ...
        if self.sandbox_enabled() {
            ...
            props.insert("project_dir".into(), serde_json::json!({
                ...
                "description": "Path to an existing project directory to mount into the container. \
                                Must be under ~/.soulsystem/projects/. If omitted, a fresh directory is created."
            }));
            props.insert("credentials".into(), serde_json::json!({
                ...
                "description": "Map of secret names to env var names. Each secret must exist in the \
                                secrets store (via 'soulsystem tool auth' or web UI). Example: \
            ...
            if self.acp_enabled() {
                props.insert(
                    ...
                    serde_json::json!({
                        "type": "string",
                        "description": "Name of the ACP agent to use (from 'soulsystem acp list'). \
