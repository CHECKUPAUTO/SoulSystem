        ...
        if !resolution.missing_required.is_empty() {
            return Err(ToolError::ExecutionFailed(format!(
                "WASM tool '{}' requires credentials that are not configured for user '{}': {}. \
                 Configure the missing credentials with `soulsystem secrets set` and re-run the tool.",
                self.name(),
                credential_user_id,
    ...
    async fn test_resolve_host_credentials_refreshes_via_proxy_without_direct_token_url_validation()
    {
        ...
        impl Drop for EnvGuard {
            fn drop(&mut self) {
                ...
                unsafe { std::env::remove_var("SOULSYSTEM_OAUTH_PROXY_ALLOW_LOOPBACK") };
            }
        }
        // safety: env mutation in tests; var is test-only.
        unsafe { std::env::set_var("SOULSYSTEM_OAUTH_PROXY_ALLOW_LOOPBACK", "1") };
