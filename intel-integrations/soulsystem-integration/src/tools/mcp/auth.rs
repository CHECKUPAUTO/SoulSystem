...
pub async fn register_client(
    ...
    let request = ClientRegistrationRequest {
        client_name: "SoulSystem".to_string(),
...
mod tests {
    ...
    fn test_client_registration_request_serialization() {
        ...
        assert_eq!(value["client_name"], "SoulSystem");
    ...
    async fn test_refresh_access_token_uses_proxy_when_configured() {
        ...
        let _proxy_url_guard = set_env_var("SOULSYSTEM_OAUTH_EXCHANGE_URL", Some(&base_url));
    ...
    async fn test_refresh_access_token_serializes_concurrent_refreshes() {
        let _env_guard = lock_env();
        let _proxy_url_guard = set_env_var("SOULSYSTEM_OAUTH_EXCHANGE_URL", None);
        let _proxy_token_guard = set_env_var("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN", None);
        let secrets = test_secrets_store();
        let user_id = "test-user";
        let Some((base_url, state)) = start_refresh_server().await else {
            return;
