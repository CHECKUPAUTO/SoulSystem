# SoulSystem Network Security Reference

This document catalogs every network-facing surface in SoulSystem, its authentication mechanism, bind address, security controls, and known findings. Use this as the authoritative reference during code reviews that touch network-facing code.
...
SoulSystem operates across four trust boundaries:
...
- Webhook senders must prove knowledge of the shared secret. The secret is never transmitted in the clear by SoulSystem itself.
...
The listener is **ephemeral** — it is started only when an OAuth flow is initiated (e.g., `soulsystem tool auth <name>`) and shut down after the callback is received or the timeout expires.
...
Google OAuth client ID and secret are compiled into the binary (with compile-time override via `SOULSYSTEM_GOOGLE_CLIENT_ID` / `SOULSYSTEM_GOOGLE_CLIENT_SECRET`). As noted in the source, Google Desktop App client secrets are [not actually secret](https://developers.google.com/identity/protocols/oauth2/native-app) per Google's documentation.
...
This is by design: MCP server URLs come from **operator-controlled configuration** (config files, environment variables, or the CLI `tool install` command), not from user input or LLM output. A compromised config file is outside SoulSystem's threat model — it would imply the operator's machine is already compromised.
