...
const TELEGRAM_TEST_API_BASE_ENV: &str = "SOULSYSTEM_TEST_TELEGRAM_API_BASE_URL";
...
pub struct ExtensionManager {
    ...
    /// Resolved once at construction from `SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN`,
...
impl ExtensionManager {
    ...
    /// used to build OAuth redirect URIs when `SOULSYSTEM_OAUTH_CALLBACK_URL` is not set.
    ...
    /// - `SOULSYSTEM_OAUTH_CALLBACK_URL` is set to a non-loopback URL, OR
    ...
    ///
    /// Priority:
    /// 1. `SOULSYSTEM_OAUTH_CALLBACK_URL` env var (via `callback_url()`)
    ...
    /// pairing store cannot have rows. See nearai/soulsystem#1921.
    ...
    async fn broadcast_extension_status(&self, name: &str, status: &str, message: Option<&str>) {
        if let Some(ref sse) = *self.sse_manager.read().await {
            sse.broadcast(soulsystem_common::AppEvent::ExtensionStatus {
                extension_name: soulsystem_common::ExtensionName::from_trusted(name.to_string()),
        ...
        if let Some(url) = url {
            ...
            return match kind {
                ...
                ExtensionKind::AcpAgent => Err(ExtensionError::InstallFailed(
                    "ACP agents are configured via 'soulsystem acp add', not the extension manager"
                        .to_string(),
                )),
            }
            .map_err(|e| {
    ...
    pub async fn activate(
        ...
        match kind {
            ...
            ExtensionKind::AcpAgent => Ok(ActivateResult {
                ...
                message: format!(
                    "ACP agent '{}' is managed via 'soulsystem acp' commands",
                    name
                ),
            }),
        }
    ...
    pub async fn remove(&self, name: &str, user_id: &str) -> Result<String, ExtensionError> {
        ...
        match kind {
            ...
            ExtensionKind::WasmChannel => {
                ...
                Ok(format!(
                    "Removed channel '{}'. Restart SoulSystem for the change to take effect.",
                    name
                ))
            }
            ExtensionKind::ChannelRelay => {
            ...
            ExtensionKind::AcpAgent => {
                // ACP agents are managed via `soulsystem acp remove`
                Ok(format!(
                    "ACP agent '{}' should be removed via 'soulsystem acp remove {}'",
                    name, name
                ))
            }
        }
    ...
    async fn try_install_from_source(
        ...
        match entry.kind {
            ...
            ExtensionKind::AcpAgent => Err(ExtensionError::InstallFailed(
                "ACP agents are configured via 'soulsystem acp add', not the registry".to_string(),
            )),
        }
    }
            ...
            crate::registry::artifacts::find_wasm_artifact(&resolved_dir, binary_name, "release")
                ...
                ExtensionError::InstallFailed(format!(
                    "'{}' requires building from source. Build artifact not found. \
                         ...
                         or use `soulsystem registry install {}`.",
                    name,
        ...
        if self.should_use_gateway_mode() {
            ...
            let task_handle = tokio::spawn(async move {
                ...
                if let Some(ref sse) = sse_manager {
                    sse.broadcast(soulsystem_common::AppEvent::OnboardingState {
                        extension_name: soulsystem_common::ExtensionName::from_trusted(ext_name),
                        state: if success {
                            soulsystem_common::OnboardingStateDto::Ready
                        } else {
                            soulsystem_common::OnboardingStateDto::Failed
                        },
                        request_id: None,
    ...
    async fn auth_channel_relay(
        ...
        // Generate CSRF nonce — SoulSystem validates this on the callback to ensure
                ...
                if needs_pairing && let Some(ref sse) = *self.sse_manager.read().await {
                    ...
                    sse.broadcast_for_user(
                        ...
                        soulsystem_common::OnboardingStateDto::pairing_required(
                            soulsystem_common::ExtensionName::from_trusted(name.clone()),
                            None,
                            None,
                            None,
                            Some(format!(
...
mod tests {
    ...
    async fn ensure_extension_ready_reports_needs_auth_for_wasm_channel() {
        // Serialize against tests that mutate SOULSYSTEM_OAUTH_CALLBACK_URL
    ...
    async fn auth_wasm_channel_status_uses_persisted_secret_oauth_descriptor() {
        ...
        unsafe {
            std::env::remove_var("SOULSYSTEM_OAUTH_CALLBACK_URL");
        }
    }
    ...
    /// Regression for nearai/soulsystem#1921's sibling: registry-backed wasm
    ...
    /// Regression for nearai/soulsystem#1921 — caller-level coverage.
    ///
    ...
    async fn test_remove_wasm_tool_clears_pending_oauth_state_and_activation_error() {
        ...
        mgr.pending_oauth_flows().write().await.insert(
            ...
            crate::auth::oauth::PendingOAuthFlow {
                extension_name: soulsystem_common::ExtensionName::new("gmail").unwrap(),
        ...
        mgr.pending_oauth_flows().write().await.insert(
            ...
            crate::auth::oauth::PendingOAuthFlow {
                extension_name: soulsystem_common::ExtensionName::new("web-search").unwrap(),
                display_name: "Web Search".to_string(),
                token_url: "https://example.com/token".to_string(),
    ...
    // `SOULSYSTEM_OAUTH_CALLBACK_URL` env var, ignoring `self.tunnel_url`.
    ...
    fn make_manager_with_tunnel(tunnel_url: Option<String>) -> ExtensionManager {
        ...
        let tools = Arc::new(crate::tools::ToolRegistry::new());
        let mcp = Arc::new(McpSessionManager::new());
        let dir = std::env::temp_dir().join("soulsystem-test-gateway-mode");
    ...
    fn should_use_gateway_mode_true_for_tunnel_url() {
        ...
        let original = std::env::var("SOULSYSTEM_OAUTH_CALLBACK_URL").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::remove_var("SOULSYSTEM_OAUTH_CALLBACK_URL");
        }
        ...
        unsafe {
            if let Some(val) = original {
                std::env::set_var("SOULSYSTEM_OAUTH_CALLBACK_URL", val);
            }
        }
    }
    ...
    /// Clears `SOULSYSTEM_OAUTH_CALLBACK_URL` for the duration, restoring on drop.
    struct EnvGuard {
    ...
    fn test_telegram_token_colon_preserved_in_validation_url() {
        ...
        // a concurrent test from setting SOULSYSTEM_TEST_TELEGRAM_API_BASE_URL.
