//! Agent configuration to SoulSystem settings mapping.

use secrecy::SecretString;
use std::collections::HashMap;

use super::reader::AgentImportConfig;

/// Map Agent configuration to SoulSystem settings (dotted-key format).
pub fn map_agent_config_to_settings(
...
pub fn extract_credentials(config: &AgentImportConfig) -> Vec<(String, SecretString)> {
...
mod tests {
    use super::*;
    use crate::import::legacy_agent::reader::{AgentImportConfig, AgentImportLlmConfig};

    #[test]
    fn test_map_llm_config() {
        let mut config = AgentImportConfig {
            llm: None,
            embeddings: None,
            other_settings: HashMap::new(),
        };

        config.llm = Some(AgentImportLlmConfig {
        ...
        let settings = map_agent_config_to_settings(&config);
