//! Agent data migration orchestration and detection.
...
pub use reader::AgentReader;

/// agent importer that coordinates migration of all data types.
...
impl AgentImporter {
    ...
    /// Detect if an agent installation exists at the default location (~/.agent).
    pub fn detect() -> Option<PathBuf> {
        if let Ok(home) = std::env::var("HOME") {
            let agent_dir = PathBuf::from(home).join(".soulsystem");
    ...
    pub async fn import(&self) -> Result<ImportStats, ImportError> {
        ...
        // Read Agent data
        let reader = AgentReader::new(&self.opts.agent_path)?;
        ...
        let settings_map = settings::map_agent_config_to_settings(&config);
        ...
        if !self.opts.dry_run {
            ...
            if let Ok(_count) = reader.list_workspace_files() {
                match self
                    .workspace
                    .import_from_directory(&self.opts.agent_path.join("workspace"))
                    .await
                {
