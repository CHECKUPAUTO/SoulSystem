...
/// `SOULSYSTEM_GOOGLE_CLIENT_ID` and `SOULSYSTEM_GOOGLE_CLIENT_SECRET` env
...
const GOOGLE_CLIENT_ID: &str = match option_env!("SOULSYSTEM_GOOGLE_CLIENT_ID") {
    Some(v) => v,
    None => "564604149681-efo25d43rs85v0tibdepsmdv5dsrhhr0.apps.googleusercontent.com",
};
const GOOGLE_CLIENT_SECRET: &str = match option_env!("SOULSYSTEM_GOOGLE_CLIENT_SECRET") {
    Some(v) => v,
...
pub fn builtin_client_id_override_env(secret_name: &str) -> Option<&'static str> {
    match secret_name {
        "google_oauth_token" => Some("SOULSYSTEM_GOOGLE_CLIENT_ID"),
        _ => None,
    }
}
...
///
/// In hosted deployments, SoulSystem may resolve the platform Google client ID from
