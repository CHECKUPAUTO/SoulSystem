...
use soulsystem_common::ExtensionName;
use rand::RngCore;
...
/// author could redirect SoulSystem to send the freshly-minted access token to
...
///
/// This is the case when `SOULSYSTEM_OAUTH_CALLBACK_URL` is set to a non-loopback
/// URL, meaning the user's browser will redirect to a hosted gateway rather than
/// localhost.
pub fn use_gateway_callback() -> bool {
    crate::config::helpers::env_or_override("SOULSYSTEM_OAUTH_CALLBACK_URL")
        .map(|raw| {
            url::Url::parse(&raw)
                .ok()
...
pub fn exchange_proxy_url() -> Option<String> {
    crate::config::helpers::env_or_override("SOULSYSTEM_OAUTH_EXCHANGE_URL")
...
/// `SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN`. Existing hosted instances continue to
...
pub fn oauth_proxy_auth_token() -> Option<String> {
    ...
    normalized_env_value("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN")
...
fn current_instance_name() -> Option<String> {
    crate::config::helpers::env_or_override("SOULSYSTEM_INSTANCE_NAME")
        .or_else(|| crate::config::helpers::env_or_override("SOULSYSTEM_INSTANCE_NAME"))
        .filter(|v| !v.is_empty())
}
...
/// SoulSystem and the external auth proxy for routing and callback lookup.
...
mod tests {
    ...
    fn test_callback_host_env_override() {
        ...
        let original_url = std::env::var("SOULSYSTEM_OAUTH_CALLBACK_URL").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
    ...
    fn test_landing_html_success_contains_key_elements() {
        ...
        assert!(html.contains("SoulSystem"));
    ...
    fn test_use_gateway_callback_false_for_localhost() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("SOULSYSTEM_OAUTH_CALLBACK_URL", "http://127.0.0.1:3001");
        }
        assert!(!crate::auth::oauth::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_use_gateway_callback_false_for_empty() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("SOULSYSTEM_OAUTH_CALLBACK_URL", "");
        }
        assert!(!crate::auth::oauth::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_build_platform_state_with_instance() {
        ...
        let _guard = lock_env();
        let original = std::env::var("SOULSYSTEM_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("SOULSYSTEM_INSTANCE_NAME", "kind-deer");
        }
        ...
        unsafe {
            if let Some(val) = original {
                std::env::set_var("SOULSYSTEM_INSTANCE_NAME", val);
            } else {
                std::env::remove_var("SOULSYSTEM_INSTANCE_NAME");
            }
        }
    }
    ...
    fn test_build_platform_state_without_instance() {
        ...
        let _guard = lock_env();
        let original = std::env::var("SOULSYSTEM_INSTANCE_NAME").ok();
        let original_oc = std::env::var("SOULSYSTEM_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::remove_var("SOULSYSTEM_INSTANCE_NAME");
            std::env::remove_var("SOULSYSTEM_INSTANCE_NAME");
    ...
    #[test]
    fn test_build_platform_state_with_instance_name() {
        use crate::auth::oauth::{build_platform_state, decode_hosted_oauth_state};

        let _guard = lock_env();
        let original_ic = std::env::var("SOULSYSTEM_INSTANCE_NAME").ok();
        let original_oc = std::env::var("SOULSYSTEM_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::remove_var("SOULSYSTEM_INSTANCE_NAME");
            std::env::set_var("SOULSYSTEM_INSTANCE_NAME", "quiet-lion");
        }
    ...
    fn test_oauth_proxy_auth_token_prefers_dedicated_env() {
        ...
        let _proxy_guard = set_env_var(
            "SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN",
    ...
    fn test_oauth_proxy_auth_token_falls_back_to_gateway_token() {
        let _guard = lock_env();
        let _proxy_guard = set_env_var("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN", None);
        let _gateway_guard = set_env_var("GATEWAY_AUTH_TOKEN", Some("gateway-token"));
    ...
    fn test_oauth_proxy_auth_token_whitespace_dedicated_env_falls_back_to_gateway_token() {
        let _guard = lock_env();
        let _proxy_guard = set_env_var("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN", Some("   "));
        let _gateway_guard = set_env_var("GATEWAY_AUTH_TOKEN", Some("gateway-token"));
