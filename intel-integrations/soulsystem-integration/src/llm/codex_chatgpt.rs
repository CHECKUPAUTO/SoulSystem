...
impl CodexChatGptProvider {
    ...
    /// Convert SoulSystem messages to Responses API request JSON.
    ...
    /// `"timestamp": ""`). SoulSystem's tool validation treats these as
    ...
    async fn complete_with_tools(
        ...
        let tool_calls: Vec<ToolCall> = result
            ...
            .map(|tc| {
                ...
                // `"timestamp": ""`), which SoulSystem's tool validation rejects.
