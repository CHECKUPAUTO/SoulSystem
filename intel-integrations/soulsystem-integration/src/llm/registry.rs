...
//! ```text
//!   ┌─────────────────────┐    ┌──────────────────────────┐
//!   │  providers.json     │    │ ~/.soulsystem/providers.json│
...
/// from `~/.soulsystem/providers.json`.
...
impl ProviderRegistry {
    ...
    ///
    /// User providers from `~/.soulsystem/providers.json` are appended,
...
fn user_providers_path() -> Option<std::path::PathBuf> {
    Some(crate::bootstrap::soulsystem_base_dir().join("providers.json"))
}
