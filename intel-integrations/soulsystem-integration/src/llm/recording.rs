...
//!
//! Enable by setting `SOULSYSTEM_RECORD_TRACE=1` at runtime.
...
/// and API keys.
///
/// `soulsystem_safety::LeakDetector` runs on response bodies before they
...
/// `soulsystem_safety::LeakDetector` runs on response bodies upstream in
...
fn unwrap_tool_output(content: &str) -> std::borrow::Cow<'_, str> {
    if let Some(body) = soulsystem_safety::SafetyLayer::unwrap_tool_output(content) {
...
impl RecordingLlm {
    ...
    ///
    /// - `SOULSYSTEM_RECORD_TRACE` — any non-empty value enables recording
    /// - `SOULSYSTEM_TRACE_OUTPUT` — file path (default: `./trace_{timestamp}.json`)
    /// - `SOULSYSTEM_TRACE_MODEL_NAME` — model_name field (default: `recorded-{inner.model_name()}`)
    pub fn from_env(inner: Arc<dyn LlmProvider>) -> Option<Arc<Self>> {
        let enabled = std::env::var("SOULSYSTEM_RECORD_TRACE")
            .ok()
            .filter(|v| !v.is_empty());
        enabled?;

        let output_path = std::env::var("SOULSYSTEM_TRACE_OUTPUT")
            .ok()
            .filter(|v| !v.is_empty())
            .map(PathBuf::from)
            .unwrap_or_else(|| {
        ...
        let model_name = std::env::var("SOULSYSTEM_TRACE_MODEL_NAME")
            .ok()
            .filter(|v| !v.is_empty())
...
mod tests {
    ...
    fn from_env_returns_none_when_unset() {
        ...
        unsafe { std::env::remove_var("SOULSYSTEM_RECORD_TRACE") };
        let stub = Arc::new(StubLlm::new("response"));
