...
//! OAuth flow. Tokens are stored in `~/.soulsystem/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.soulsystem/session.json).
...
impl SessionManager {
    ...
    /// `~/.soulsystem/.env` for survival across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.soulsystem/.env so the key survives restarts
