...
//!
//! This mirrors Agent's Responses API flow translated to Rust.
...
/// with JWT-based auth headers matching Agent's approach.
...
impl OpenAiCodexProvider {
    ...
    /// Build request headers matching Agent's `buildHeaders`.
    async fn build_headers(&self) -> Result<reqwest::header::HeaderMap, LlmError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("soulsystem"),
        );
        headers.insert(
            USER_AGENT,
            HeaderValue::from_static(concat!("soulsystem/", env!("CARGO_PKG_VERSION"))),
        );
...
///
/// Matches Agent's `extractAccountId` which reads:
...
// ---------------------------------------------------------------------------
// Message conversion (matching Agent's convertResponsesMessages)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SSE response parsing (matching Agent's processResponsesStream)
// ---------------------------------------------------------------------------
...
fn parse_sse_response(body: &str) -> Result<ParsedResponse, LlmError> {
    ...
    // Map status to finish reason (matching Agent's mapStopReason)
