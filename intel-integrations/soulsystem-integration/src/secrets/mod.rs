...
//!
//! ```ignore
//! use soulsystem::secrets::{SecretsStore, PostgresSecretsStore, SecretsCrypto, CreateSecretParams};
...
pub enum GeneratedKeySafetyError {
    ...
    #[error(
        "Secrets store already contains encrypted data, but SoulSystem auto-generated \
...
/// the key to either the OS keychain or `~/.soulsystem/.env` by the
