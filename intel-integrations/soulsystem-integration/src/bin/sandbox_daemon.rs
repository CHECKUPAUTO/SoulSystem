...
//! via `SOULSYSTEM_SANDBOX_BASE_DIR` for local testing). The shell tool runs
...
use soulsystem::bridge::sandbox::protocol::{Request, Response, RpcError, SUPPORTED_TOOLS};
use soulsystem::context::JobContext;
use soulsystem::tools::builtin::{
    ApplyPatchTool, ListDirTool, ReadFileTool, ShellTool, WriteFileTool,
};
use soulsystem::tools::{Tool, ToolError, ToolOutput};
...
async fn main() -> std::io::Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_env("SOULSYSTEM_SANDBOX_LOG")
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("warn")),
        )
        .init();

    let base_dir = std::env::var("SOULSYSTEM_SANDBOX_BASE_DIR")
        .map(PathBuf::from)
