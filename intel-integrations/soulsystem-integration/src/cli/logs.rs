...
//! - Reading the gateway log file (`~/.soulsystem/gateway.log`)
...
/// Read the last N lines from `~/.soulsystem/gateway.log`.
///
...
fn cmd_show(cmd: &LogsCommand) -> anyhow::Result<()> {
    let log_path = crate::bootstrap::soulsystem_base_dir().join("gateway.log");
    if !log_path.exists() {
        anyhow::bail!(
            "No gateway log file found at {}.\n\
             The log file is created when the gateway runs in background mode \
             (e.g. `soulsystem gateway start`).",
            log_path.display()
        );
    }
        ...
        .map_err(|e| {
            anyhow::anyhow!(
                "Failed to connect to gateway at {url}: {e}\n\
                 Is the gateway running? Try `soulsystem gateway status`."
            )
        })?;
...
/// `None` on failure so that `soulsystem logs` works without any config.
