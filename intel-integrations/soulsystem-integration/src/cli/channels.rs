...
//!
//! `status` (runtime health) requires connecting to a running SoulSystem instance
...
async fn cmd_list(
    ...
    if !verbose {
        ...
        println!("managed via environment variables. See 'soulsystem onboard --channels-only'.");
    }
