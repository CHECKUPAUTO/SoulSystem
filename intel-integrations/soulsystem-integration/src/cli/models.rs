...
//! persisted to both `config.toml` and `~/.soulsystem/.env` so changes
...
fn config_toml_path() -> std::path::PathBuf {
    crate::bootstrap::soulsystem_base_dir().join("config.toml")
}
        ...
        None => {
            println!(
                "\n  Could not fetch model list (missing credentials or provider unavailable).\
                 \n  Tip: Run `soulsystem doctor` to check your configuration."
            );
        }
    }
}

/// Also update `~/.soulsystem/.env` so changes take effect immediately.
///
...
fn cmd_set_provider(
    ...
    let canonical_id = if provider == "nearai" || provider == "near_ai" || provider == "near" {
        ...
        let def = registry.find(provider).ok_or_else(|| {
            ...
            anyhow::anyhow!(
                "Unknown provider '{}'.\n\nAvailable providers: {}\n\n\
                 Tip: Run `soulsystem models list` to see all providers with descriptions,\n\
                 or `soulsystem onboard --step provider` for interactive setup.",
                provider,
                known.join(", ")
            )
        })?;
        ...
        && let Some(ref env_var) = def.api_key_env
    {
        ...
        if !has_key {
            println!();
            println!(
                "Note: {} requires an API key. Set {} or run `soulsystem onboard --step provider` to configure.",
...
async fn cmd_list_providers(
    ...
    if !verbose {
        ...
        println!();
        println!("To switch provider: soulsystem models set-provider <name>");
        println!("For guided setup:   soulsystem onboard --step provider");
    }
...
mod tests {
    ...
    fn custom_config_does_not_pollute_default_dotenv() {
        ...
        // trying to write to the default ~/.soulsystem/.env.
