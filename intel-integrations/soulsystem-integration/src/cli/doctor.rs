//! `soulsystem doctor` - active health diagnostics.
//!
...
pub async fn run_doctor_command() -> anyhow::Result<()> {
    println!();
    println!("  {}SoulSystem Doctor{}", fmt::bold(), fmt::reset());
...
async fn check_nearai_session(settings: &Settings) -> CheckResult {
    ...
    if !session_path.exists() {
        ...
        return CheckResult::Fail(format!(
            "session file not found at {}. Run `soulsystem onboard`",
        ...
        Ok(config) => {
            ...
            if has_creds {
                ...
                let hint = match config.provider.as_str() {
                    "nearai" => "run `soulsystem onboard` to create a session",
...
async fn check_skills() -> CheckResult {
    let user_dir = soulsystem_base_dir().join("skills");
    let installed_dir = soulsystem_base_dir().join("installed_skills");

    let mut registry = soulsystem_skills::SkillRegistry::new(user_dir.clone());
...
fn check_secrets(settings: &Settings) -> CheckResult {
    match settings.secrets_master_key_source {
        ...
        crate::settings::KeySource::None => {
            CheckResult::Skip("secrets not configured (run `soulsystem onboard`)".into())
        }
    }
}
...
fn check_service_installed() -> CheckResult {
    if cfg!(target_os = "macos") {
        let plist =
            dirs::home_dir().map(|h| h.join("Library/LaunchAgents/com.soulsystem.daemon.plist"));
        match plist {
            Some(path) if path.exists() => {
                CheckResult::Pass(format!("launchd plist installed ({})", path.display()))
            }
            Some(_) => CheckResult::Skip("not installed (run `soulsystem service install`)".into()),
            None => CheckResult::Skip("cannot determine home directory".into()),
        }
    } else if cfg!(target_os = "linux") {
        let unit = dirs::home_dir().map(|h| h.join(".config/systemd/user/soulsystem.service"));
        match unit {
            Some(path) if path.exists() => {
...
mod tests {
    ...
    fn check_binary_skips_nonexistent() {
        match check_binary("__soulsystem_nonexistent_binary__", &["--version"]) {
            CheckResult::Skip(_) => {}
            other => panic!(
