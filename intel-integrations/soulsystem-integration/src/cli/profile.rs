...
fn extract_description(toml_content: &str) -> Option<String> {
    ...
    // Skip the `# SoulSystem Profile: <name>` title line.
    lines.next()?;
...
fn cmd_list(json: bool) -> anyhow::Result<()> {
    ...
    let active_name = std::env::var("SOULSYSTEM_PROFILE")
        .ok()
        .filter(|s| !s.is_empty())
    ...
    if active_name.is_none() {
        ...
        println!("Set SOULSYSTEM_PROFILE=<name> to activate one.");
    }
    println!("\nCustom profiles can be added to ~/.soulsystem/profiles/");
...
mod tests {
    ...
    fn extract_description_from_builtin() {
        let toml = "# SoulSystem Profile: local\n#\n# A short description.\n#\ndatabase_backend = \"libsql\"\n";
        assert_eq!(
    ...
    fn extract_description_skips_blank_comments() {
        let toml = "# SoulSystem Profile: foo\n#\n#\n# Real description.\n";
        assert_eq!(
            extract_description(toml),
