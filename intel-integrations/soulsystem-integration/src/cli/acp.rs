...
async fn list_agents() -> anyhow::Result<()> {
    ...
    if agents.agents.is_empty() {
        ...
        println!("  soulsystem acp add goose --command goose --arg \"--stdio\"");
        return Ok(());
    }
...
async fn test_agent(name: &str) -> anyhow::Result<()> {
    ...
    /// Event sink that prints agent output to stdout during `soulsystem acp test`.
    ...
    // Uses SoulSystemAcpClient with a PrintEventSink so the test exercises
    ...
    let result = local_set
        .run_until(async move {
            ...
            let client = acp_bridge::SoulSystemAcpClient::new(PrintEventSink);
            ...
            let handshake = tokio::time::timeout(
                ...
                conn.initialize(acp_bridge::soulsystem_init_request()),
            )
            .await;
