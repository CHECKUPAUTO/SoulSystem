...
#[derive(Parser, Debug)]
#[command(name = "soulsystem")]
#[command(
    about = "Secure personal AI assistant that protects your data and expands its capabilities"
)]
#[command(long_about = "SoulSystem is a secure AI assistant.\n\n\
     Getting started:\n  \
       soulsystem onboard              # Interactive setup wizard (recommended for first run)\n  \
       soulsystem onboard --quick      # Quick setup: just pick a provider and model\n  \
       soulsystem models set-provider openai  # Switch to a specific provider\n  \
       soulsystem doctor               # Check your configuration\n\n\
     Common commands:\n  \
       soulsystem run                  # Start the agent\n  \
       soulsystem config list          # View all settings\n  \
       soulsystem models status        # Show current provider and model\n  \
       soulsystem models list          # List available providers\n\n\
     Use 'soulsystem <subcommand> --help' for details on any command.")]
#[command(version)]
...
pub enum Command {
    ...
    #[command(
        ...
        long_about = "Starts the SoulSystem agent in default mode.\nExample: soulsystem run"
    )]
    Run,

    /// Interactive onboarding wizard
    #[command(
        about = "Run interactive setup wizard (start here if new to SoulSystem)",
        long_about = "Guides you through configuring SoulSystem step by step.\n\n\
         ...
         Examples:\n  \
           soulsystem onboard                    # Full setup wizard\n  \
           soulsystem onboard --quick            # Quick: just provider + model\n  \
           soulsystem onboard --step provider    # Change only the LLM provider\n  \
           soulsystem onboard --step channels    # Reconfigure messaging channels\n  \
           soulsystem onboard --step provider,model  # Change provider and model"
    )]
    Onboard {
    ...
    #[command(
        ...
        long_about = "View and modify SoulSystem settings (stored in database and config.toml).\n\n\
         For LLM provider/model changes, use `soulsystem models` instead.\n\n\
         Examples:\n  \
           soulsystem config list              # List all settings\n  \
           soulsystem config list -f agent     # Filter by prefix\n  \
           soulsystem config get agent.name    # Get a specific value\n  \
           soulsystem config set agent.name my-bot  # Change a value\n  \
           soulsystem config init              # Generate config.toml\n  \
           soulsystem config path              # Show where settings are stored"
    )]
    ...
    #[command(
        ...
        long_about = "List configured messaging channels.\nExamples:\n  soulsystem channels list\n  soulsystem channels list --verbose\n  soulsystem channels list --json"
    )]
    ...
    #[command(
        ...
        long_about = "List, create, edit, enable/disable, delete, and view history of routines.\nExamples:\n  soulsystem routines list\n  soulsystem routines create --name daily-digest --schedule '0 0 9 * * *' --prompt 'Summarize today'"
    )]
    ...
    #[command(
        ...
        long_about = "Add, auth, list, or test MCP servers.\nExample: soulsystem mcp add notion https://mcp.notion.com"
    )]
    Mcp(Box<McpCommand>),
    ...
    #[command(
        subcommand,
        about = "Manage skills",
        long_about = "List, search, and inspect SKILL.md-based skills.\nExamples:\n  soulsystem skills list\n  soulsystem skills search 'writing'\n  soulsystem skills info my-skill"
    )]
    ...
    #[command(
        ...
        long_about = "List and inspect lifecycle hooks (bundled, plugin, workspace).\nExamples:\n  soulsystem hooks list\n  soulsystem hooks list --verbose\n  soulsystem hooks list --json"
    )]
    Hooks(HooksCommand),
    ...
    #[command(
        ...
        long_about = "List providers, view current configuration, and set active provider/model.\n\n\
         ...
         Examples:\n  \
           soulsystem models list                          # List all providers\n  \
           soulsystem models list openai --verbose         # Show details for OpenAI\n  \
           soulsystem models status                        # Show current provider/model\n  \
           soulsystem models set gpt-4o                    # Change model\n  \
           soulsystem models set-provider anthropic        # Switch to Anthropic\n  \
           soulsystem models set-provider ollama --model llama3  # Switch to local Ollama"
    )]
    ...
    #[command(
        ...
        long_about = "Tail gateway logs, stream live output, or adjust log level.\nExamples:\n  soulsystem logs                 # Show last 200 lines from gateway.log\n  soulsystem logs --follow        # Stream live logs via SSE\n  soulsystem logs --level         # Show current log level\n  soulsystem logs --level debug   # Set log level to debug"
    )]
    Logs(LogsCommand),
    ...
    #[command(
        ...
        long_about = "Generates shell completion scripts.\nExample: soulsystem completion --shell bash > soulsystem.bash"
    )]
    ...
    #[command(
        ...
        long_about = "Migrate data from other AI assistants like Agent.\nExample: soulsystem import agent"
    )]
    ...
    #[command(
        ...
        long_about = "Re-authenticate with an LLM provider.\n\n\
         ...
         For interactive setup: `soulsystem onboard --step provider`\n\n\
         Example:\n  soulsystem login --openai-codex"
    )]
    Login {
    ...
    Worker {
        ...
        #[arg(long, env = "SOULSYSTEM_MAX_ITERATIONS", default_value = "50")]
        max_iterations: u32,
    },
...
-> anyhow::Result<Arc<dyn crate::secrets::SecretsStore + Send + Sync>> {
    ...
    let master_key = config.secrets.master_key().ok_or_else(|| {
        anyhow::anyhow!(
            "SECRETS_MASTER_KEY not set. Run 'soulsystem onboard' first or set it in .env"
        )
    })?;
