...
//! For core skill types, parsing, and registry, import from `soulsystem_skills` directly.
//!
...
//!
//! The `soulsystem_skills` crate itself remains (types, parser, validation, v2 types).

pub mod attenuation;
pub mod bundled;

// Items from `soulsystem_skills` are no longer glob-re-exported.
// Callers should import from `soulsystem_skills` directly.
...
use soulsystem_skills::{LoadedSkill, SkillCredentialLocation, SkillCredentialSpec};
...
fn credential_spec_to_oauth_refresh(spec: &SkillCredentialSpec) -> Option<OAuthRefreshConfig> {
    ...
    match &oauth.refresh {
        soulsystem_skills::ProviderRefreshStrategy::ReauthorizeOnly => return None,
        soulsystem_skills::ProviderRefreshStrategy::Standard => {}
        soulsystem_skills::ProviderRefreshStrategy::Custom {
            refresh_url,
            extra_params,
        } => {
...
pub fn register_skill_credentials(
    ...
    for skill in skills {
        for spec in &skill.manifest.credentials {
            let errors = soulsystem_skills::validation::validate_credential_spec(spec);
            if !errors.is_empty() {
                tracing::warn!(
                    skill = %skill.name(),
...
mod tests {
    ...
    fn test_convert_bearer_location() {
        let loc = soulsystem_skills::SkillCredentialLocation::Bearer;
    ...
    fn test_convert_basic_auth_location() {
        let loc = soulsystem_skills::SkillCredentialLocation::BasicAuth {
    ...
    fn test_convert_header_location() {
        let loc = soulsystem_skills::SkillCredentialLocation::Header {
    ...
    fn test_convert_query_param_location() {
        let loc = soulsystem_skills::SkillCredentialLocation::QueryParam {
            name: "key".to_string(),
        };
        let converted = convert_credential_location(&loc);
        match converted {
    ...
    fn test_credential_spec_to_mapping() {
        let spec = soulsystem_skills::SkillCredentialSpec {
            name: "github_token".to_string(),
            provider: "github".to_string(),
            location: soulsystem_skills::SkillCredentialLocation::Bearer,
    ...
    fn test_register_skill_credentials_valid() {
        use soulsystem_skills::types::*;
        use std::path::PathBuf;

        let skill = soulsystem_skills::LoadedSkill {
