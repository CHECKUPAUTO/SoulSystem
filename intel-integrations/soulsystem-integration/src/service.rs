//! OS service management for running SoulSystem as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.soulsystem.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/soulsystem.service`
//!
//! The installed service runs `soulsystem run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.soulsystem.daemon";
const SYSTEMD_UNIT: &str = "soulsystem.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = soulsystem_logs_dir();
    ...
    println!("  Start with: soulsystem service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `soulsystem service install` first.");
        }
...
fn soulsystem_logs_dir() -> PathBuf {
    soulsystem_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.soulsystem.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/soulsystem.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_soulsystem() {
        let path = soulsystem_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".soulsystem/logs"), "unexpected path: {s}");
    }

    #[test]
    fn macos_plist_sets_cli_enabled_false() {
        let plist = macos_plist_content("/tmp/soulsystem", "/tmp/stdout.log", "/tmp/stderr.log");
