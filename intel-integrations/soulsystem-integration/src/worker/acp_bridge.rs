...
//! SoulSystem's `JobEventPayload` stream and posted to the orchestrator.
//!
...
//! │ Docker Container                              │
//! │                                               │
//! │  soulsystem acp-bridge --job-id <uuid>          │
...
impl AcpBridgeRuntime {
    ...
    ///
    /// Reads `SOULSYSTEM_WORKER_TOKEN` from the environment for auth.
    ...
    async fn run_acp_session(
        ...
        let acp_result = local_set
            .run_until(async move {
                ...
                let soulsystem_client = SoulSystemAcpClient::new(Arc::clone(&client_for_acp));

                let (conn, handle_io) =
                    acp::ClientSideConnection::new(soulsystem_client, outgoing, incoming, |fut| {
                        tokio::task::spawn_local(fut);
                    });
                tokio::task::spawn_local(handle_io);

                conn.initialize(soulsystem_init_request())
                    .await
...
/// command prints them to stdout. Both share the same `SoulSystemAcpClient`.
...
/// SoulSystem's implementation of the ACP Client trait.
///
...
pub(crate) struct SoulSystemAcpClient<S: AcpEventSink> {
    sink: S,
}

impl<S: AcpEventSink> SoulSystemAcpClient<S> {
...
impl<S: AcpEventSink> acp::Client for SoulSystemAcpClient<S> {
...
/// Build the standard SoulSystem ACP initialization request.
pub(crate) fn soulsystem_init_request() -> acp::InitializeRequest {
    acp::InitializeRequest::new(acp::ProtocolVersion::V1).client_info(
        acp::Implementation::new("soulsystem", env!("CARGO_PKG_VERSION")).title("SoulSystem"),
    )
}
...
/// Convert an ACP `SessionUpdate` into an SoulSystem `JobEventPayload`.
