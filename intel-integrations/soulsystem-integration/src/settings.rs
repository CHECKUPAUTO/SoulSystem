...
//!
//! Stores user preferences in `~/.soulsystem` (JSON/TOML) and, for some values,
...
pub struct Settings {
    ...
    /// Stable owner scope for this SoulSystem instance.
    ///
...
fn default_sandbox_image() -> String {
    "soulsystem-worker:latest".to_string()
}
...
impl Settings {
    ...
    /// Get the default settings file path (~/.soulsystem/settings.json).
    pub fn default_path() -> std::path::PathBuf {
        soulsystem_base_dir().join("settings.json")
    }
    ...
    /// Default TOML config file path (~/.soulsystem/config.toml).
    pub fn default_toml_path() -> PathBuf {
        soulsystem_base_dir().join("config.toml")
    }
    ...
    pub fn save_toml(&self, path: &std::path::Path) -> Result<(), String> {
        ...
        let content = format!(
            "# SoulSystem configuration file.\n\
             ...
             # Run `soulsystem config init` to regenerate this file.\n\
             #\n\
             # Documentation: https://github.com/nearai/soulsystem\n\
             \n\
             {raw}"
        );
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("soulsystem".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "soulsystem");
    }
    ...
    fn toml_header_comment_present() {
        ...
        assert!(content.starts_with("# SoulSystem configuration file."));
    ...
    #[test]
    fn default_toml_path_under_soulsystem() {
        let path = Settings::default_toml_path();
        assert!(path.to_string_lossy().contains(".soulsystem"));
    ...
    fn wizard_recovery_step1_overrides_stale_db() {
        ...
        let prior_run = Settings {
            ...
            database_url: Some("postgres://old-host/soulsystem".to_string()),
            llm_backend: Some("anthropic".to_string()),
        ...
        let step1_settings = Settings {
            database_backend: Some("postgres".to_string()),
            database_url: Some("postgres://new-host/soulsystem".to_string()),
            ..Settings::default()
        };
        ...
        assert_eq!(
            ...
            Some("postgres://new-host/soulsystem".to_string()),
    ...
    /// Simulates `soulsystem onboard --provider-only` re-running on a fully
    ...
    fn provider_only_rerun_preserves_unrelated_settings() {
        ...
        let prior = Settings {
            onboard_completed: true,
            database_backend: Some("libsql".to_string()),
            libsql_path: Some("/home/user/.soulsystem/soulsystem.db".to_string()),
            llm_backend: Some("openai".to_string()),
            selected_model: Some("gpt-4o".to_string()),
            embeddings: EmbeddingsSettings {
                enabled: true,
    ...
    /// Simulates `soulsystem onboard --channels-only` re-running on a fully
    ...
    fn switching_db_backend_allows_fresh_connection_settings() {
        ...
        assert_eq!(
            ...
            Some("/home/user/.soulsystem/soulsystem.db")
        );
