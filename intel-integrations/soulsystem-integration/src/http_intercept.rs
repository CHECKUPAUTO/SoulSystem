...
/// `SOULSYSTEM_TEST_HTTP_REMAP` env var sneaks into a debug build, the
...
impl HostRemapHttpInterceptor {
    fn from_env() -> Option<Self> {
        let raw = std::env::var("SOULSYSTEM_TEST_HTTP_REMAP").ok()?;
        let mappings = raw
            .split(',')
            .filter_map(|entry| {
                ...
                if !is_loopback_target(&base_url) {
                    tracing::warn!(
                        host = %host,
                        base_url = %base_url,
                        "SOULSYSTEM_TEST_HTTP_REMAP target is not loopback; refusing to register"
                    );
                    return None;
                }
                Some((host, base_url))
