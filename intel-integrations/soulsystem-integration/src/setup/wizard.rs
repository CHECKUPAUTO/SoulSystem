...
use crate::bootstrap::soulsystem_base_dir;
...
/// Interactive setup wizard for SoulSystem.
...
impl SetupWizard {
    ...
    pub async fn run(&mut self) -> Result<(), SetupError> {
        print_banner();
        print_header("SoulSystem Setup Wizard");
    ...
    async fn reconnect_existing_db(&mut self) -> Result<(), SetupError> {
        ...
        Err(SetupError::Database(
            "No database configured. Run full setup first (soulsystem onboard).".to_string(),
        ))
    }
    ...
    async fn reconnect_postgres(&mut self) -> Result<(), SetupError> {
        let url = std::env::var("DATABASE_URL").map_err(|_| {
            SetupError::Database(
                "DATABASE_URL not set. Run full setup first (soulsystem onboard).".to_string(),
            )
        })?;
    ...
    async fn step_database_libsql(&mut self) -> Result<(), SetupError> {
        ...
        println!();
        print_info("SoulSystem uses an embedded SQLite database (libSQL).");
    ...
    async fn test_database_connection_postgres(&mut self, url: &str) -> Result<(), SetupError> {
        ...
        if major_version < MIN_PG_MAJOR_VERSION {
            return Err(SetupError::Database(format!(
                "PostgreSQL {} detected. SoulSystem requires PostgreSQL {} or later for pgvector support.\n\
        ...
        if pgvector_row.is_none() {
            return Err(SetupError::Database(format!(
                "pgvector extension not found on your PostgreSQL server.\n\n\
                 ...
                 Then restart PostgreSQL and re-run: soulsystem onboard",
                major_version
            )));
        }
    ...
    async fn step_security(&mut self) -> Result<(), SetupError> {
        ...
        match choice {
            ...
            1 => {
                ...
                // Store hex for write_bootstrap_env to persist to ~/.soulsystem/.env.
                self.settings.secrets_master_key_hex = Some(key_hex.clone());

                println!();
                print_info(&format!(
                    "Master key generated and will be saved to {}",
                    crate::bootstrap::soulsystem_env_path().display()
                ));
                println!();
    ...
    ///
    /// Existing `SOULSYSTEM_PROFILE` values are respected because
    ...
    fn step_quick_local_profile(&mut self) -> Result<(), SetupError> {
        if crate::config::env_or_override("SOULSYSTEM_PROFILE").is_some() {
            return Ok(());
        }
        ...
        let choice = select_one("How should SoulSystem run on this machine?", &options)
            .map_err(SetupError::Io)?;
    ...
    fn apply_quick_local_profile(&mut self, profile: &str) -> Result<(), SetupError> {
        ...
        crate::config::set_runtime_env("SOULSYSTEM_PROFILE", profile);
    ...
    async fn setup_github_copilot_device_login(&mut self) -> Result<(), SetupError> {
        ...
        print_info("Authorize SoulSystem with GitHub Copilot in your browser.");
    ...
    async fn step_channels(&mut self) -> Result<(), SetupError> {
        ...
        let channels_dir = soulsystem_base_dir().join("channels");
    ...
    async fn step_extensions(&mut self) -> Result<(), SetupError> {
        let catalog = match load_registry_catalog() {
            ...
            None => {
                ...
                print_info("Install tools manually with: soulsystem tool install <path>");
                return Ok(());
            }
        };
        ...
        print_info("  soulsystem registry install <name>");
        println!();

        // Check which tools are already installed
        let tools_dir = soulsystem_base_dir().join("tools");
        ...
        let installer = crate::registry::installer::RegistryInstaller::new(
            ...
            tools_dir.clone(),
            soulsystem_base_dir().join("channels"),
        );
            ...
            match installer.install_with_source_fallback(tool, false).await {
                Ok(outcome) => {
                        ...
                        && auth.method.is_some()
                    {
                        ...
                        let hint = format!("  {} - soulsystem tool auth {}", provider, tool.name);
                        if !auth_needed
                            .iter()
    ...
    async fn step_docker_sandbox(&mut self) -> Result<(), SetupError> {
        print_info("SoulSystem can execute code, run builds, and use tools inside Docker");
    ...
    async fn ensure_worker_image(&mut self) -> Result<(), SetupError> {
        ...
        // "ghcr.io/nearai/soulsystem-worker:v1"). For those, or when
        ...
        if dockerfile_path.exists() {
            ...
            print_info("or clone the SoulSystem repository and build from source.");
        }
    ...
    /// Write bootstrap environment variables to `~/.soulsystem/.env`.
    ///
    /// Only true chicken-and-egg settings are written here — things needed
    /// before the database is connected: `SOULSYSTEM_PROFILE`, `DATABASE_BACKEND`,
    ...
    fn bootstrap_env_vars(&self) -> Vec<(String, String)> {
        ...
        if let Some(ref profile) = self.selected_deployment_profile {
            env_vars.push(("SOULSYSTEM_PROFILE".to_string(), profile.clone()));
        }
    ...
    /// Save settings to the database and `~/.soulsystem/.env`, then print
    ...
    async fn save_and_summarize(&mut self) -> Result<(), SetupError> {
        ...
        // Title line: checkmark + "soulsystem is ready"
        println!(
            "  {}\u{2713}{} {}soulsystem is ready{}",
            fmt::success(),
            fmt::reset(),
            fmt::bold_accent(),
            fmt::reset(),
        ...
        if self.config.quick {
            print_info(
                "Tip: Run `soulsystem onboard` to configure channels, extensions, embeddings, and more.",
            );
            println!();
        }
...
async fn install_selected_registry_channels(
    ...
    for name in selected_channels {
        ...
        let installer = crate::registry::installer::RegistryInstaller::new(
            repo_root.clone(),
            soulsystem_base_dir().join("tools"),
...
mod tests {
    ...
    fn test_wizard_owner_id_uses_resolved_env_scope() {
        let _guard = lock_env();
        let _owner = EnvGuard::set("SOULSYSTEM_OWNER_ID", " wizard-owner ");
    ...
    fn test_wizard_owner_id_uses_toml_scope() {
        let _guard = lock_env();
        let _owner = EnvGuard::clear("SOULSYSTEM_OWNER_ID");
    ...
    fn test_bootstrap_env_vars_include_selected_deployment_profile() {
        let _guard = lock_env();
        let _profile = EnvGuard::clear("SOULSYSTEM_PROFILE");
        ...
        assert!(
            vars.iter().any(|(key, value)| {
                key == "SOULSYSTEM_PROFILE" && value == QUICK_PROFILE_LOCAL_SANDBOX
            }),
    ...
    fn test_bootstrap_env_vars_do_not_persist_unselected_profile() {
        let _guard = lock_env();
        let _profile = EnvGuard::set("SOULSYSTEM_PROFILE", QUICK_PROFILE_LOCAL);
        ...
        assert!(
            !vars.iter().any(|(key, _)| key == "SOULSYSTEM_PROFILE"),
    ...
    fn test_try_with_config_and_toml_propagates_invalid_owner_env() {
        ...
        let _guard = lock_env();
        let original = std::env::var_os("SOULSYSTEM_OWNER_ID");
        unsafe {
            std::env::set_var("SOULSYSTEM_OWNER_ID", OsString::from_vec(vec![0x66, 0x80]));
        }
        ...
        unsafe {
            if let Some(value) = original {
                std::env::set_var("SOULSYSTEM_OWNER_ID", value);
            } else {
                std::env::remove_var("SOULSYSTEM_OWNER_ID");
            }
        }
    ...
    async fn test_discover_wasm_channels_nonexistent_dir() {
        let channels = discover_wasm_channels(
            &std::env::temp_dir().join("soulsystem_nonexistent_dir_abcxyz123"),
        )
        .await;
        assert!(channels.is_empty());
    }
