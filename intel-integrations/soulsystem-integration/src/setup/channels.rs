...
async fn setup_tunnel_cloudflare() -> Result<TunnelSettings, ChannelSetupError> {
    ...
    let cloudflared_found = soulsystem_skills::gating::binary_exists("cloudflared");
...
/// with SoulSystem's tunnel management.
...
mod tests {
    ...
    async fn test_validate_public_https_url_fails_closed_on_dns_error() {
        ...
        let probe = tokio::time::timeout(
            ...
            tokio::net::lookup_host(("soulsystem-dns-hijack-probe.invalid", 443u16)),
        )
        .await;
    ...
    fn test_http_webhook_secret_hint_reflects_current_behavior() {
        ...
        assert!(!hint.contains("soulsystem secret get"));
    }
}
