...
This document is the authoritative specification for SoulSystem's onboarding
...
3. If Command::Run or no command:
   a. Load .env files (dotenvy::dotenv() then load_soulsystem_env())
...
`DATABASE_BACKEND` live in `~/.soulsystem/.env`.
...
auto_setup_database()    → libsql at ~/.soulsystem/soulsystem.db (zero prompts)
...
Step 2/2: Model Selection     ← interactive unless defaulted by detected provider
       ↓
   save_and_summarize()      → includes tip to run `soulsystem onboard`
```

**Local usage profile:** If `SOULSYSTEM_PROFILE` is already set, quick mode
...
The selected profile is written to `~/.soulsystem/.env` as `SOULSYSTEM_PROFILE`
...
defaults to libsql at `~/.soulsystem/soulsystem.db`, creates the database,
...
The full 9-step wizard remains available via `soulsystem onboard`.
...
**libSQL path:**
1. Offer local path (default: `~/.soulsystem/soulsystem.db`)
...
2. "Allow soulsystem to access this keychain item" (per-app authorization)
...
- **Never probe the keychain in read-only commands** (e.g., `soulsystem status`).
...
- **NEAR AI Chat** path: session token saved to `~/.soulsystem/session.json`.
  Hosting providers can set `NEARAI_SESSION_TOKEN` env var directly (takes
  precedence over file-based tokens).
- **NEAR AI Cloud** path: `NEARAI_API_KEY` saved to `~/.soulsystem/.env`
...
6b. Discover WASM channels from ~/.soulsystem/channels/
...
1. Already installed in `~/.soulsystem/channels/`
...
4. Discover already-installed tools in `~/.soulsystem/tools/`
...
**Layer 1: `~/.soulsystem/.env`** (bootstrap vars)
...
```env
SOULSYSTEM_PROFILE="local"
DATABASE_BACKEND="libsql"
LIBSQL_PATH="/Users/name/.soulsystem/soulsystem.db"
...
DATABASE_URL="postgres://user:pass@localhost/soulsystem"
SECRETS_MASTER_KEY="..."
ONBOARD_COMPLETED="true"
```

**Why separate?** Chicken-and-egg: you need `DATABASE_BACKEND` to know
which database to connect to, `SOULSYSTEM_PROFILE` to apply built-in defaults
...
1. Writes bootstrap vars to `~/.soulsystem/.env` via `write_bootstrap_env()`
...
Bootstrap vars written to `~/.soulsystem/.env` (only true chicken-and-egg vars
that are needed before the DB is connected):
- `SOULSYSTEM_PROFILE` (when quick first-run setup selected a profile)
...
pub struct Settings {
    ...
    // Advanced (not in wizard, set via `soulsystem config set`)
...
- Service name: `"soulsystem"`, account: `"master_key"`
...
1. **NEAR AI Cloud API key (option 4 in auth menu):** Get an API key
   ...
   local listener is needed. The key is saved to `~/.soulsystem/.env`
   and the encrypted secrets store. Uses the OpenAI-compatible
   ChatCompletions API mode.

2. **Custom callback URL:** Set `SOULSYSTEM_OAUTH_CALLBACK_URL` to a
   publicly accessible URL (e.g., via SSH tunnel or reverse proxy) that
   forwards to port 9876 on the server:
   ```bash
   export SOULSYSTEM_OAUTH_CALLBACK_URL=https://myserver.example.com:9876
   ```
...
6. Run the full shipping checklist:
   ...
   ```
7. Test a fresh onboarding: `rm -rf ~/.soulsystem && cargo run`
