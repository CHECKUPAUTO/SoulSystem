//! Application builder for initializing core SoulSystem components.
//!
...
impl AppBuilder {
            ...
            .await
        {
            if self.config.secrets.generated {
                crate::secrets::rollback_generated_key_persistence(
                    ...
                    &crate::bootstrap::soulsystem_env_path(),
                )
                .await;
            }
    ...
    > {
        ...
        // `SOULSYSTEM_TEST_HTTP_REMAP` env var on a release deployment cannot
    ...
    pub async fn build_all(mut self) -> Result<AppComponents, anyhow::Error> {
        ...
        let (skill_registry, skill_catalog) = if self.config.skills.enabled {
            ...
            let catalog = soulsystem_skills::catalog::shared_catalog();
