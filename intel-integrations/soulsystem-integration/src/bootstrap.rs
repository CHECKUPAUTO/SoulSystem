//! Bootstrap helpers for SoulSystem.
//!
...
//!
//! File: `~/.soulsystem/.env` (standard dotenvy format)

use std::path::PathBuf;
use std::sync::LazyLock;

const SOULSYSTEM_BASE_DIR_ENV: &str = "SOULSYSTEM_BASE_DIR";

/// Lazily computed SoulSystem base directory, cached for the lifetime of the process.
static SOULSYSTEM_BASE_DIR: LazyLock<PathBuf> = LazyLock::new(compute_soulsystem_base_dir);

/// Compute the SoulSystem base directory from environment.
///
/// This is the underlying implementation used by both the public
/// `soulsystem_base_dir()` function (which caches the result) and tests
/// (which need to verify different configurations).
pub fn compute_soulsystem_base_dir() -> PathBuf {
    std::env::var(SOULSYSTEM_BASE_DIR_ENV)
        .map(PathBuf::from)
        .map(|path| {
            if path.as_os_str().is_empty() {
                ...
                eprintln!(
                    "Warning: SOULSYSTEM_BASE_DIR is a relative path '{}', resolved against current directory",
                    path.display()
                );
                path
            } else {
...
/// Get the SoulSystem base directory.
///
/// Override with `SOULSYSTEM_BASE_DIR` environment variable.
/// Defaults to `~/.soulsystem` (or `./.soulsystem` if home directory cannot be determined).
///
/// Thread-safe: the value is computed once and cached in a `LazyLock`.
///
/// # Environment Variable Behavior
/// - If `SOULSYSTEM_BASE_DIR` is set to a non-empty path, that path is used.
/// - If `SOULSYSTEM_BASE_DIR` is set to an empty string, it is treated as unset.
/// - If `SOULSYSTEM_BASE_DIR` contains null bytes, a warning is printed and the default is used.
...
/// for existence.
pub fn soulsystem_base_dir() -> PathBuf {
    SOULSYSTEM_BASE_DIR.clone()
}

/// Path to the SoulSystem-specific `.env` file: `~/.soulsystem/.env`.
pub fn soulsystem_env_path() -> PathBuf {
    soulsystem_base_dir().join(".env")
}

/// Load env vars from `~/.soulsystem/.env` (in addition to the standard `.env`).
///
/// Call this **after** `dotenvy::dotenv()` so that the standard `./.env`
/// takes priority over `~/.soulsystem/.env`. dotenvy never overwrites
/// existing env vars, so the effective priority is:
///
///   explicit env vars > `./.env` > `~/.soulsystem/.env` > auto-detect
///
/// If `~/.soulsystem/.env` doesn't exist but the legacy `bootstrap.json` does,
...
/// `DATABASE_BACKEND` is still unset and `~/.soulsystem/soulsystem.db` exists,
/// defaults to `libsql` so cloud instances work out of the box without any
/// manual configuration.
pub fn load_soulsystem_env() {
    let path = soulsystem_env_path();
    ...
    // DATABASE_URL is configured but soulsystem.db is already present.
    if std::env::var("DATABASE_BACKEND").is_err() {
        ...
        if default_db.exists() {
            if tokio::runtime::Handle::try_current().is_ok() {
                ...
                tracing::warn!(
                    "load_soulsystem_env called with active Tokio runtime; \
...
fn migrate_bootstrap_json_to_env(env_path: &std::path::Path) {
    let soulsystem_dir = env_path
        .parent()
        .unwrap_or_else(|| std::path::Path::new("."));
    let bootstrap_path = soulsystem_dir.join("bootstrap.json");
...
/// Write database bootstrap vars to `~/.soulsystem/.env`.
///
...
pub fn save_bootstrap_env(vars: &[(&str, &str)]) -> std::io::Result<()> {
    save_bootstrap_env_to(&soulsystem_env_path(), vars)
}
...
/// Update or add multiple variables in `~/.soulsystem/.env`, preserving existing content.
///
...
pub fn upsert_bootstrap_vars(vars: &[(&str, &str)]) -> std::io::Result<()> {
    upsert_bootstrap_vars_to(&soulsystem_env_path(), vars)
}
...
/// Remove a single variable from `~/.soulsystem/.env`, preserving other lines.
///
...
/// Update or add a single variable in `~/.soulsystem/.env`, preserving existing content.
///
...
pub fn upsert_bootstrap_var(key: &str, value: &str) -> std::io::Result<()> {
    upsert_bootstrap_var_to(&soulsystem_env_path(), key, value)
}
...
/// Write `DATABASE_URL` to `~/.soulsystem/.env`.
///
...
/// One-time migration of legacy `~/.soulsystem/settings.json` into the database.
///
...
pub async fn migrate_disk_to_db(
    ...
    let soulsystem_dir = soulsystem_base_dir();
    let legacy_settings_path = soulsystem_dir.join("settings.json");
    ...
    let session_path = soulsystem_dir.join("session.json");
    ...
    let old_bootstrap = soulsystem_dir.join("bootstrap.json");
...
/// Path to the PID lock file: `~/.soulsystem/soulsystem.pid`.
pub fn pid_lock_path() -> PathBuf {
    soulsystem_base_dir().join("soulsystem.pid")
}

/// A PID-based lock that prevents multiple SoulSystem instances from running
/// simultaneously.
///
...
pub enum PidLockError {
    #[error("Another SoulSystem instance is already running (PID {pid})")]
...
mod tests {
    ...
    fn test_save_and_load_database_url() {
        ...
        let url = "postgres://localhost:5432/soulsystem_test";
        ...
        assert_eq!(
            content,
            "DATABASE_URL=\"postgres://localhost:5432/soulsystem_test\"\n"
        );
    ...
    fn test_save_database_url_with_hash_in_password() {
        ...
        let url = "postgres://user:p%23ss@localhost:5432/soulsystem";
        std::fs::write(&env_path, format!("DATABASE_URL=\"{}\"\n", url)).unwrap();
...
INJECTED="pwned"#;
    ...
    #[test]
    fn test_soulsystem_env_path() {
        // Use compute_soulsystem_base_dir() directly to avoid LazyLock caching,
        // which can be poisoned by whichever test initializes it first.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: Under lock_env(), no concurrent env access.
        unsafe { std::env::remove_var("SOULSYSTEM_BASE_DIR") };

        let path = compute_soulsystem_base_dir().join(".env");
        assert!(
            path.ends_with(".soulsystem/.env"),
            "expected path ending with .soulsystem/.env, got: {}",
            path.display()
        );

        if let Some(val) = old_val {
            unsafe { std::env::set_var("SOULSYSTEM_BASE_DIR", val) };
        }
    }
    ...
    fn test_migrate_bootstrap_json_to_env() {
        ...
        let bootstrap_json = serde_json::json!({
            "database_url": "postgres://localhost/soulsystem_upgrade",
    ...
    fn test_save_bootstrap_env_multiple_vars() {
        ...
        let vars = [
            ...
            ("LIBSQL_PATH", "/home/user/.soulsystem/soulsystem.db"),
        ];
        ...
        assert_eq!(
            ...
            (
                ...
                "/home/user/.soulsystem/soulsystem.db".to_string()
            )
        );
    }
    ...
    fn test_libsql_autodetect_sets_backend_when_db_exists() {
        ...
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("soulsystem.db");
    ...
    #[test]
    fn test_soulsystem_base_dir_default() {
        // This test must run first (or in isolation) before the LazyLock is initialized.
        // It verifies that when SOULSYSTEM_BASE_DIR is not set, the default path is used.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("SOULSYSTEM_BASE_DIR") };
        ...
        let path = compute_soulsystem_base_dir();
        let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
        assert_eq!(path, home.join(".soulsystem"));
    ...
    #[test]
    fn test_soulsystem_base_dir_env_override() {
        // This test verifies that when SOULSYSTEM_BASE_DIR is set,
        // the custom path is used. Must run before LazyLock is initialized.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SOULSYSTEM_BASE_DIR", "/custom/soulsystem/path") };

        // Force re-evaluation by calling the computation function directly
        let path = compute_soulsystem_base_dir();
        assert_eq!(path, std::path::PathBuf::from("/custom/soulsystem/path"));
    ...
    fn test_compute_base_dir_env_path_join() {
        // Verifies that soulsystem_env_path correctly joins .env to the base dir.
        // Uses compute_soulsystem_base_dir directly to avoid LazyLock caching.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SOULSYSTEM_BASE_DIR", "/my/custom/dir") };

        // Test the path construction logic directly
        let base_path = compute_soulsystem_base_dir();
    ...
    #[test]
    fn test_soulsystem_base_dir_empty_env() {
        // Verifies that empty SOULSYSTEM_BASE_DIR falls back to default.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SOULSYSTEM_BASE_DIR", "") };
    ...
    #[test]
    fn test_soulsystem_base_dir_special_chars() {
        // Verifies that paths with special characters are handled correctly.
        let _guard = lock_env();
        let old_val = std::env::var("SOULSYSTEM_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SOULSYSTEM_BASE_DIR", "/tmp/test_with-special.chars") };
    ...
    fn test_pid_lock_acquire_and_drop() {
        let dir = tempdir().unwrap();
        let pid_path = dir.path().join("soulsystem.pid");
    ...
    fn test_pid_lock_creates_parent_dirs() {
        let dir = tempdir().unwrap();
        let pid_path = dir.path().join("nested").join("deep").join("soulsystem.pid");
    ...
    fn test_pid_lock_child_helper_holds_lock() {
        if std::env::var("SOULSYSTEM_PID_LOCK_CHILD").ok().as_deref() != Some("1") {
            return;
        }

        let pid_path = PathBuf::from(
            std::env::var("SOULSYSTEM_PID_LOCK_PATH").expect("SOULSYSTEM_PID_LOCK_PATH missing"),
        );
        let hold_ms = std::env::var("SOULSYSTEM_PID_LOCK_HOLD_MS")
            .ok()
    ...
    fn test_pid_lock_rejects_lock_held_by_other_process() {
        ...
        let mut child = Command::new(current_exe)
            ...
            .env("SOULSYSTEM_PID_LOCK_CHILD", "1")
            .env("SOULSYSTEM_PID_LOCK_PATH", pid_path.display().to_string())
            .env("SOULSYSTEM_PID_LOCK_HOLD_MS", "3000")
            .spawn()
            .unwrap();
