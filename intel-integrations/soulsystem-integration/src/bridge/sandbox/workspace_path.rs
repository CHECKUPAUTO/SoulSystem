...
//! `~/.soulsystem/projects/<user_id>/<project_id>/`. That's the directory the
...
//! intentionally doesn't know about `~/.soulsystem` — that's a host-side concept
//! kept here so the engine stays portable.

use std::io;
use std::path::{Component, Path, PathBuf};

use soulsystem_engine::Project;

use crate::bootstrap::soulsystem_base_dir;

/// Subdirectory under [`soulsystem_base_dir`] that holds per-project workspaces.
...
/// `~/.soulsystem/projects/<user_id>/<project_id>/` — namespaced by user so
...
mod tests {
    ...
    fn empty_user_id_does_not_drop_namespace() {
        ...
        let base = soulsystem_base_dir().join(PROJECTS_SUBDIR);
    ...
    fn different_users_get_different_paths() {
        let id = soulsystem_engine::ProjectId::new();
