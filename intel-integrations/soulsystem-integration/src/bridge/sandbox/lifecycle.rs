...
//! (`soulsystem-sandbox-<project_id>`) so multiple SoulSystem runs against the
...
use soulsystem_engine::{MountError, ProjectId};
use tracing::{debug, warn};

/// Default image. Override with `SOULSYSTEM_SANDBOX_IMAGE`.
pub const DEFAULT_IMAGE: &str = "soulsystem/sandbox:dev";
...
pub fn sandbox_image() -> String {
    std::env::var("SOULSYSTEM_SANDBOX_IMAGE").unwrap_or_else(|_| DEFAULT_IMAGE.to_string())
}

/// Build the deterministic container name for a project.
pub fn container_name_for(project_id: ProjectId) -> String {
    format!("soulsystem-sandbox-{}", project_id.0)
}
...
mod tests {
    ...
    fn container_name_is_deterministic() {
        ...
        assert_eq!(n1, n2);
        assert!(n1.starts_with("soulsystem-sandbox-"));
    }
}
