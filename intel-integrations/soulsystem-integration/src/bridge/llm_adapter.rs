//! LLM bridge adapter — wraps `LlmProvider` as `soulsystem_engine::LlmBackend`.
...
impl LlmBackend for LlmBridgeAdapter {
    async fn complete(
        ...
        let llm_response = if !response.tool_calls.is_empty() {
            let mut calls: Vec<soulsystem_engine::ActionCall> = response
                .tool_calls
                .iter()
                .map(|tc| soulsystem_engine::ActionCall {
                    id: tc.id.clone(),
...
fn build_tool_result_index(messages: &[ThreadMessage]) -> Vec<(String, serde_json::Value)> {
    messages
        .iter()
        .filter(|m| m.role == soulsystem_engine::MessageRole::ActionResult)
        .filter_map(|m| {
...
fn thread_msg_to_chat(msg: &ThreadMessage) -> ChatMessage {
    use soulsystem_engine::MessageRole;
...
mod tests {
    ...
    use soulsystem_engine::{ActionCall, ActionDef, EffectType, LlmResponse, ThreadMessage};
    ...
    async fn config_model_forwards_to_completion_request() {
        ...
        let config = soulsystem_engine::LlmCallConfig {
    ...
    async fn config_without_model_leaves_request_model_none() {
        ...
        adapter
            .complete(
                &[ThreadMessage::user("hi")],
                &[],
                &soulsystem_engine::LlmCallConfig::default(),
            )
            .await
            .unwrap();
