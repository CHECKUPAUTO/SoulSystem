//! Effect bridge adapter — wraps `ToolRegistry` + `SafetyLayer` as `soulsystem_engine::EffectExecutor`.
//!
//! This is the security boundary between the engine and existing SoulSystem
...
pub struct EffectBridgeAdapter {
    ...
    mission_manager: RwLock<Option<Arc<soulsystem_engine::MissionManager>>>,
...
impl EffectBridgeAdapter {
    ...
    pub async fn set_mission_manager(&self, mgr: Arc<soulsystem_engine::MissionManager>) {
        *self.mission_manager.write().await = Some(mgr);
    }

    /// Get the mission manager if available.
    pub async fn mission_manager(&self) -> Option<Arc<soulsystem_engine::MissionManager>> {
            ...
            "awaiting_authorization" | "awaiting_token" => Some(Self::gate_paused(
                ...
                soulsystem_engine::ResumeKind::Authentication {
                    ...
                    credential_name: output_value
                        .get("credential_name")
                        .and_then(|v| v.as_str())
                        .and_then(|raw| soulsystem_common::CredentialName::new(raw).ok())
                        .or_else(|| soulsystem_common::CredentialName::new(name).ok())
                        .unwrap_or_else(|| {
                            soulsystem_common::CredentialName::from_trusted(name.to_string())
                        }),
        ...
        let result = match action_name {
            "mission_create" => {
                    ...
                    params.get("project_id").and_then(|v| v.as_str())
                {
                    match uuid::Uuid::parse_str(pid_str) {
                        Ok(uuid) => {
                            let pid = soulsystem_engine::ProjectId(uuid);
                ...
                .await
            {
                Ok(missions) => {
                    let list: Vec<serde_json::Value> = missions
                        ...
                        .map(|m| {
                            let timezone =
                                if let soulsystem_engine::types::mission::MissionCadence::Cron {
                                    timezone: Some(tz),
                                    ..
                                } = &m.cadence
                                {
            ...
            "mission_get" => {
                ...
                let id = uuid::Uuid::parse_str(id_str)
                    .map(soulsystem_engine::MissionId)
                            ...
                            for tid in recent_thread_ids {
                                if let Ok(Some(thread)) = store.load_thread(*tid).await {
                                    let last_response = thread
                                        ...
                                        .iter()
                                        .rev()
                                        .find(|m| m.role == soulsystem_engine::MessageRole::Assistant)
            ...
            "mission_update" => {
                ...
                match id {
                    Ok(id) => {
                        let mut updates = soulsystem_engine::MissionUpdate::default();
...
fn extract_guardrails(
    params: &serde_json::Value,
    base: &mut soulsystem_engine::MissionUpdate,
) -> Result<(), String> {
...
fn parse_cadence(
    s: &str,
    timezone: Option<soulsystem_engine::ValidTimezone>,
) -> Result<soulsystem_engine::types::mission::MissionCadence, String> {
    ...
    if lower == "manual" {
        ...
        if let Err(e) = regex::RegexBuilder::new(pattern)
            .size_limit(soulsystem_engine::runtime::mission::MAX_EVENT_REGEX_SIZE)
            .build()
        {
            return Err(format!(
...
struct RoutineMissionAlias {
    ...
    post_create_update: Option<soulsystem_engine::MissionUpdate>,
}
...
fn routine_to_mission_alias(
    ...
    match action_name {
        "routine_create" => {
            ...
            let mut updates = soulsystem_engine::MissionUpdate {
...
fn parse_routine_request(
    params: &serde_json::Value,
) -> soulsystem_engine::types::mission::MissionCadence {
    use soulsystem_engine::types::mission::MissionCadence;
    ...
    match kind {
        "cron" => MissionCadence::Cron {
            ...
            timezone: request
                ...
                .and_then(|v| v.as_str())
                .and_then(soulsystem_common::ValidTimezone::parse),
        },
...
fn cadence_to_round_trip_string(
    cadence: &soulsystem_engine::types::mission::MissionCadence,
) -> String {
    use soulsystem_engine::types::mission::MissionCadence;
    match cadence {
...
mod tests {
    ...
    fn make_adapter() -> EffectBridgeAdapter {
        use soulsystem_safety::SafetyConfig;
    ...
    fn lease() -> soulsystem_engine::CapabilityLease {
        soulsystem_engine::CapabilityLease {
            id: soulsystem_engine::types::capability::LeaseId::new(),
            thread_id: soulsystem_engine::ThreadId::new(),
            capability_name: "tools".into(),
            granted_actions: soulsystem_engine::GrantedActions::All,
    ...
    fn exec_ctx(
        ...
        call_id: Option<&str>,
    ) -> soulsystem_engine::ThreadExecutionContext {
        soulsystem_engine::ThreadExecutionContext {
            thread_id,
            thread_type: soulsystem_engine::types::thread::ThreadType::Foreground,
            project_id: soulsystem_engine::ProjectId::new(),
    ...
    async fn memory_write_orchestrator_target_paused_for_approval_when_self_modify_enabled() {
        ...
        let _guard = soulsystem_engine::runtime::SelfModifyTestGuard::enable();
    ...
    async fn memory_write_orchestrator_target_refused_when_self_modify_disabled() {
        ...
        let _guard = soulsystem_engine::runtime::SelfModifyTestGuard::disable();
    ...
    /// Regression for nearai/soulsystem#2206: a `tool_activate`/`tool_auth`
    ...
    async fn auth_gate_strips_non_https_auth_url_from_tool_activate_output() {
        ...
        match result {
            Err(EngineError::GatePaused {
                ...
                match *resume_kind {
                    soulsystem_engine::ResumeKind::Authentication { auth_url, .. } => {
                        assert!(
                            auth_url.is_none(),
    ...
    fn routine_create_alias_translates_cron_with_full_field_set() {
        ...
        match updates.cadence.as_ref().expect("cadence in updates") {
            soulsystem_engine::types::mission::MissionCadence::Cron {
                expression,
                timezone,
            } => {
    ...
    fn routine_create_alias_translates_message_event_with_channel_filter() {
        ...
        match updates.cadence.as_ref().expect("cadence") {
            soulsystem_engine::types::mission::MissionCadence::OnEvent {
                event_pattern,
                channel,
            } => {
    ...
    fn routine_create_alias_translates_system_event_with_filters() {
        let params = serde_json::json!({
            ...
            "request": {
                ...
                "filters": {
                    "repository_name": "nearai/soulsystem",
        ...
        let updates = alias.post_create_update.expect("updates");
        match updates.cadence.as_ref().expect("cadence") {
            soulsystem_engine::types::mission::MissionCadence::OnSystemEvent {
                source,
                event_type,
                filters,
            } => {
    ...
    fn routine_create_alias_translates_webhook() {
        ...
        let updates = alias.post_create_update.expect("updates");
        match updates.cadence.as_ref().expect("cadence") {
            soulsystem_engine::types::mission::MissionCadence::Webhook { path, secret } => {
    ...
    fn parse_cadence_event_prefix_wins_over_cron_heuristic() {
        ...
        assert!(matches!(
            cadence,
            soulsystem_engine::types::mission::MissionCadence::Webhook { .. }
        ));
    ...
    fn routine_create_alias_defaults_to_manual_when_request_missing() {
        ...
        let updates = alias.post_create_update.expect("updates");
        match updates.cadence.as_ref().expect("cadence") {
            soulsystem_engine::types::mission::MissionCadence::Manual => {}
    ...
    async fn preflight_gate_blocks_missing_credential() {
        ...
        let params = serde_json::json!({
            "url": "https://api.github.com/repos/nearai/soulsystem/issues",
            "method": "GET"
        });
        let lease = soulsystem_engine::CapabilityLease {
            id: soulsystem_engine::types::capability::LeaseId::new(),
            thread_id: soulsystem_engine::ThreadId::new(),
            capability_name: "tools".into(),
            granted_actions: soulsystem_engine::GrantedActions::All,
        ...
        let ctx = soulsystem_engine::ThreadExecutionContext {
            thread_id: soulsystem_engine::ThreadId::new(),
            thread_type: soulsystem_engine::types::thread::ThreadType::Foreground,
            project_id: soulsystem_engine::ProjectId::new(),
            user_id: "test_user".to_string(),
    ...
    async fn tool_activate_awaiting_authorization_becomes_auth_gate() {
        ...
        let adapter = EffectBridgeAdapter::new(
            tools,
            Arc::new(SafetyLayer::new(&soulsystem_safety::SafetyConfig {
                max_output_length: 10_000,
                injection_check_enabled: false,
            })),
            Arc::new(HookRegistry::default()),
    ...
    mod mission_store {
        use soulsystem_engine::types::mission::{Mission, MissionId, MissionStatus};
        use soulsystem_engine::types::thread::{Thread, ThreadId, ThreadState};
        use soulsystem_engine::{EngineError, ProjectId};
        ...
        #[async_trait::async_trait]
        impl soulsystem_engine::Store for TestStore {
            ...
            async fn save_step(&self, _: &soulsystem_engine::Step) -> Result<(), EngineError> {
                Ok(())
            }
            async fn load_steps(
                &self,
                _: ThreadId,
            ) -> Result<Vec<soulsystem_engine::Step>, EngineError> {
                Ok(vec![])
            }
            ...
            async fn load_events(
                &self,
                _: ThreadId,
            ) -> Result<Vec<soulsystem_engine::ThreadEvent>, EngineError> {
                Ok(vec![])
            }
            async fn save_project(&self, _: &soulsystem_engine::Project) -> Result<(), EngineError> {
                Ok(())
            }
            async fn load_project(
                &self,
                _: ProjectId,
            ) -> Result<Option<soulsystem_engine::Project>, EngineError> {
                Ok(None)
            }
            async fn save_memory_doc(
                &self,
                _: &soulsystem_engine::MemoryDoc,
            ) -> Result<(), EngineError> {
                Ok(())
            }
            async fn load_memory_doc(
                &self,
                _: soulsystem_engine::DocId,
            ) -> Result<Option<soulsystem_engine::MemoryDoc>, EngineError> {
                Ok(None)
            }
            async fn list_memory_docs(
                ...
                _: ProjectId,
                _: &str,
            ) -> Result<Vec<soulsystem_engine::MemoryDoc>, EngineError> {
                Ok(vec![])
            }
            async fn save_lease(
                &self,
            ...
            async fn load_active_leases(
                &self,
                _: ThreadId,
            ) -> Result<Vec<soulsystem_engine::CapabilityLease>, EngineError> {
                Ok(vec![])
            }
            async fn revoke_lease(
                &self,
                _: soulsystem_engine::types::capability::LeaseId,
                _: &str,
            ) -> Result<(), EngineError> {
                Ok(())
            }
    ...
    async fn make_adapter_with_missions() -> EffectBridgeAdapter {
        use soulsystem_engine::{CapabilityRegistry, LeaseManager, PolicyEngine, ThreadManager};
        use soulsystem_safety::SafetyConfig;
        ...
        impl soulsystem_engine::LlmBackend for NoopLlm {
            async fn complete(
                ...
                Ok(soulsystem_engine::LlmOutput {
                    response: soulsystem_engine::types::step::LlmResponse::Text("done".into()),
                    usage: soulsystem_engine::types::step::TokenUsage::default(),
                })
            }
        ...
        impl soulsystem_engine::EffectExecutor for NoopEffects {
            async fn execute_action(
                ...
                Ok(soulsystem_engine::ActionResult {
                    call_id: String::new(),
        ...
        let store: Arc<dyn soulsystem_engine::Store> = Arc::new(mission_store::TestStore::new());
        ...
        let mgr = soulsystem_engine::MissionManager::new(Arc::clone(&store), thread_manager);
    ...
    async fn mission_create_missing_cadence_returns_error_via_execute_action() {
        ...
        let result = adapter
            .execute_action(
                ...
                &lease(),
                &exec_ctx(soulsystem_engine::ThreadId::new(), Some("c1")),
            )
            .await
    ...
    async fn mission_create_malformed_cadence_returns_error_via_execute_action() {
        ...
        let result = adapter
            .execute_action(
                ...
                &lease(),
                &exec_ctx(soulsystem_engine::ThreadId::new(), Some("c2")),
            )
            .await
    ...
    async fn mission_create_string_guardrails_rejected_via_execute_action() {
        ...
        let result = adapter
            .execute_action(
                ...
                &lease(),
                &exec_ctx(soulsystem_engine::ThreadId::new(), Some("c3")),
            )
            .await
            .expect("should return Ok with is_error=true");
    ...
    async fn mission_create_valid_params_succeeds_via_execute_action() {
        ...
        let result = adapter
            .execute_action(
                ...
                &lease(),
                &exec_ctx(soulsystem_engine::ThreadId::new(), Some("c4")),
            )
            .await
    ...
    async fn mission_update_string_guardrails_rejected_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("u1"));
    ...
    async fn system_event_cadence_round_trips_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("rt1"));
    ...
    async fn mission_complete_returns_completed_status_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("d1"));
    ...
    async fn mission_full_lifecycle_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("lc1"));
    ...
    async fn mission_fire_returns_thread_id_for_manual_cadence_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("fire1"));
    ...
    async fn mission_list_returns_all_user_missions_via_execute_action() {
        let adapter = make_adapter_with_missions().await;
        let ctx = exec_ctx(soulsystem_engine::ThreadId::new(), Some("list1"));
    ...
    fn mission_capability() -> soulsystem_engine::Capability {
        soulsystem_engine::Capability {
            name: "missions".into(),
    ...
    fn mission_lease(granted: &[&str]) -> soulsystem_engine::CapabilityLease {
        soulsystem_engine::CapabilityLease {
            id: soulsystem_engine::types::capability::LeaseId::new(),
            thread_id: soulsystem_engine::ThreadId::new(),
            capability_name: "missions".into(),
            granted_actions: soulsystem_engine::GrantedActions::Specific(
    ...
    async fn available_actions_filters_v1_denylisted_names_from_engine_capabilities() {
        ...
        registry.register(soulsystem_engine::Capability {
            name: "rogue".into(),
        ...
        let rogue_lease = soulsystem_engine::CapabilityLease {
            id: soulsystem_engine::types::capability::LeaseId::new(),
            thread_id: soulsystem_engine::ThreadId::new(),
