...
fn self_modify_enabled() -> bool {
    soulsystem_engine::runtime::self_modify_enabled()
}
...
fn synthesize_orchestrator_doc_from_py(path: &str, content: &str) -> Option<MemoryDoc> {
    ...
    Some(MemoryDoc {
        ...
        user_id: soulsystem_engine::types::shared_owner_id().to_string(),
...
fn validate_orchestrator_content(doc: &MemoryDoc) -> Result<(), EngineError> {
    if doc.title.starts_with("orchestrator:")
        && doc.title != ORCHESTRATOR_FAILURES_TITLE
        && let Err(reason) = soulsystem_engine::executor::validate_python_syntax(&doc.content)
    {
...
fn compact_thread_summary(thread: &Thread) -> ThreadArchiveSummary {
    ...
    let outcome = thread
        ...
        .iter()
        .rev()
        .find(|m| matches!(m.role, soulsystem_engine::MessageRole::Assistant))
...
fn thread_from_archive(summary: &ThreadArchiveSummary) -> Option<Thread> {
    ...
    Some(Thread {
        ...
        thread_type: soulsystem_engine::ThreadType::Mission,
        state,
        project_id: soulsystem_engine::ProjectId(uuid::Uuid::nil()),
        user_id: "default".to_string(),
        parent_id: None,
        config: soulsystem_engine::ThreadConfig::default(),
        messages: Vec::new(),
...
impl Store for HybridStore {
    ...
    async fn save_memory_doc(&self, doc: &MemoryDoc) -> Result<(), EngineError> {
        ...
        if is_protected_orchestrator_doc(doc) {
            let trusted = soulsystem_engine::runtime::is_trusted_internal_write_active();
        ...
        if is_globally_shared(&stamped)
            && soulsystem_engine::types::is_shared_owner(&stamped.user_id)
    ...
    async fn list_shared_memory_docs(
        ...
        let mut out: Vec<MemoryDoc> = docs
            .values()
            .filter(|doc| {
                let owner_shared = soulsystem_engine::types::is_shared_owner(&doc.user_id);
                if !owner_shared {
                    return false;
                }
...
mod tests {
    use super::*;
    use soulsystem_engine::types::shared_owner_id;
    ...
    /// same normalization logic (one lives in the `soulsystem` bridge layer,
    ...
    async fn save_allows_trusted_seed_when_self_modify_disabled() {
        use soulsystem_engine::Store;
        use soulsystem_engine::runtime::with_trusted_internal_writes;
    ...
    fn archive_thread_fixture(cost: f64) -> soulsystem_engine::Thread {
        let mut thread = soulsystem_engine::Thread::new(
            "archive-round-trip",
            soulsystem_engine::ThreadType::Mission,
        ...
        thread.state = soulsystem_engine::ThreadState::Completed;
        thread
    }
