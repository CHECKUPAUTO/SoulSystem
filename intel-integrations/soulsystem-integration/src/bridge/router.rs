...
use soulsystem_common::AppEvent;
use soulsystem_engine::types::{is_shared_owner, shared_owner_id};
...
async fn resolve_extension_for_action(
    ...
    user_id: &str,
) -> soulsystem_common::ExtensionName {
...
async fn resolve_auth_gate_extension_name(
    ...
    tools: &crate::tools::ToolRegistry,
    pending: &PendingGate,
) -> Option<soulsystem_common::ExtensionName> {
    let soulsystem_engine::ResumeKind::Authentication {
        credential_name, ..
    } = &pending.resume_kind
    else {
        return None;
...
async fn send_pending_gate_status(
    ...
    pending: &PendingGate,
    extension_name: Option<&soulsystem_common::ExtensionName>,
) {
    let display_parameters = gate_display_parameters(pending);

    match &pending.resume_kind {
        soulsystem_engine::ResumeKind::Approval { allow_always } => {
            let _ = agent
                .channels
                .send_status(
                    &message.channel,
        ...
        soulsystem_engine::ResumeKind::Authentication {
            instructions,
            auth_url,
            ..
        } => {
        ...
        soulsystem_engine::ResumeKind::External { .. } => {}
    }
}
...
) -> soulsystem_engine::ThreadMessage {
    let rendered = serde_json::to_string_pretty(output).unwrap_or_else(|_| output.to_string());
    soulsystem_engine::ThreadMessage::action_result(call_id, action_name, rendered)
}
...
fn resolved_call_id_for_pending_action(
    thread: &soulsystem_engine::Thread,
    pending: &PendingGate,
) -> Option<String> {
    ...
    let resolved_ids: HashSet<&str> = all_messages
        .clone()
        .filter_map(|message| {
            (message.role == soulsystem_engine::types::message::MessageRole::ActionResult)
    ...
    all_messages.rev().find_map(|message| {
        if message.role != soulsystem_engine::types::message::MessageRole::Assistant {
            return None;
        }
...
async fn notify_pending_gate(
    ...
    if let soulsystem_engine::ResumeKind::External { callback_id } = &pending.resume_kind {
        tracing::debug!(
            gate = %pending.gate_name,
...
async fn requeue_auth_pending_gate(
    ...
    let next_pending = PendingGate {
        ...
        description: pending.description.clone(),
        resume_kind: soulsystem_engine::ResumeKind::Authentication {
            credential_name,
            instructions,
            auth_url,
        },
...
fn pairing_pending_gate_from_auth(pending: &PendingGate, extension_name: &str) -> PendingGate {
    PendingGate {
        ...
        resume_kind: soulsystem_engine::ResumeKind::External {
...
fn snapshot_lease_still_valid(
    lease: &soulsystem_engine::CapabilityLease,
    pending: &PendingGate,
) -> bool {
...
async fn resume_lease_for_pending_gate(
    pending: &PendingGate,
    leases: &soulsystem_engine::LeaseManager,
) -> Option<soulsystem_engine::CapabilityLease> {
...
async fn execute_pending_gate_action(
    ...
    let exec_ctx = soulsystem_engine::ThreadExecutionContext {
        thread_id: pending.thread_id,
        thread_type: thread.thread_type,
        project_id: thread.project_id,
        user_id: thread.user_id.clone(),
        step_id: soulsystem_engine::StepId::new(),
        ...
        user_timezone: thread
            ...
            .get("user_timezone")
            .and_then(|v| v.as_str())
            .and_then(soulsystem_engine::ValidTimezone::parse),
    };
        ...
        .await
    {
        ...
        Err(soulsystem_engine::EngineError::GatePaused {
            gate_name,
            action_name,
            call_id,
            parameters,
            ...
            let pending_gate = PendingGate {
                ...
                approval_already_granted: approval_already_granted
                    || matches!(
                        pending.resume_kind,
                        soulsystem_engine::ResumeKind::Approval { .. }
                    ),
            };
...
async fn resolve_user_project(
    store: &Arc<dyn Store>,
    user_id: &str,
    fallback: soulsystem_engine::ProjectId,
) -> Result<soulsystem_engine::ProjectId, Error> {
    ...
    let project = soulsystem_engine::Project::new(user_id, "default", "Default project");
    let pid = project.id;
    store
...
struct EngineState {
    ...
    store: Arc<dyn Store>,
    default_project_id: soulsystem_engine::ProjectId,
...
fn parse_engine_thread_id(scope: Option<&str>) -> Option<soulsystem_engine::ThreadId> {
    scope
        .and_then(|s| uuid::Uuid::parse_str(s).ok())
        .map(soulsystem_engine::ThreadId)
}
...
async fn reconcile_pending_gate_state(
    ...
    for gate in &restored_gates {
        ...
        if thread.state != soulsystem_engine::ThreadState::Waiting
    ...
    for project in projects {
        ...
        for mut thread in threads {
            ...
            if let Err(e) = thread.transition_to(
                soulsystem_engine::ThreadState::Failed,
...
async fn fail_orphaned_waiting_thread_if_needed(
    state: &EngineState,
    user_id: &str,
    thread_id: soulsystem_engine::ThreadId,
) -> Result<bool, Error> {
    if state
        .pending_gates
        .peek(&PendingGateKey {
...
async fn fail_waiting_thread(
    ...
    if !thread.is_owned_by(user_id) || thread.state != soulsystem_engine::ThreadState::Waiting {
        return Ok(false);
    }

    thread
        .transition_to(soulsystem_engine::ThreadState::Failed, Some(reason.into()))
...
pub async fn init_engine(agent: &Agent) -> Result<(), Error> {
    ...
    if let Some(workspace) = agent.workspace().cloned() {
        let reader: Arc<dyn soulsystem_engine::WorkspaceReader> =
    ...
    let budget_gate: Arc<dyn soulsystem_engine::BudgetGate> =
    ...
    // backend the mount table returns.
    {
        ...
        use soulsystem_engine::{MountError, ProjectMountFactory, WorkspaceMounts};
...
pub async fn handle_approval(
    ...
    resolve_gate(
        ...
        request_id,
        if approved {
            soulsystem_engine::GateResolution::Approved { always }
        } else {
            soulsystem_engine::GateResolution::Denied { reason: None }
        },
    )
    .await
}
...
pub async fn handle_external_callback(
    ...
    let resolution = soulsystem_engine::GateResolution::ExternalCallback {
...
pub async fn handle_auth_gate_resolution(
    ...
    let gate_resolution = match resolution {
        crate::agent::submission::AuthGateResolution::CredentialProvided { token } => {
            soulsystem_engine::GateResolution::CredentialProvided { token }
        }
        crate::agent::submission::AuthGateResolution::Cancelled => {
            soulsystem_engine::GateResolution::Cancelled
        }
    };
...
) -> Result<Option<soulsystem_engine::ThreadId>, Error> {
...
pub async fn resolve_gate(
    ...
    thread_id: soulsystem_engine::ThreadId,
    request_id: uuid::Uuid,
    resolution: soulsystem_engine::GateResolution,
) -> Result<BridgeOutcome, Error> {
    init_engine(agent).await?;
    ...
    match resolution {
        ...
        soulsystem_engine::GateResolution::Denied { reason } => {
            if let Some(ref sse) = state.sse {
                sse.broadcast_for_user(
                    &message.user_id,
                    AppEvent::GateResolved {
            ...
            let deny_msg = soulsystem_engine::ThreadMessage::user(format!(
        ...
        soulsystem_engine::GateResolution::CredentialProvided { token } => {
            ...
            if let soulsystem_engine::ResumeKind::Authentication {
                ref credential_name,
                ..
            } = pending.resume_kind
            {
                    ...
                    Ok(PendingAuthCredentialSubmission::Stored(result)) => match crate::channels::web::onboarding::classify_configure_result(
                        ...
                        crate::channels::web::onboarding::ConfigureFlowOutcome::PairingRequired {
                            ...
                            if let Some(ref sse) = state.sse {
                                sse.broadcast_for_user(
                                    &message.user_id,
                                    soulsystem_common::OnboardingStateDto::pairing_required(
                                        display_name.clone(),
        ...
        soulsystem_engine::GateResolution::ExternalCallback { .. } => {
            if let Some(ref sse) = state.sse {
                sse.broadcast_for_user(
                    &message.user_id,
                    AppEvent::GateResolved {
...
pub async fn handle_expected(
    ...
    let tool_events: Vec<serde_json::Value> = thread
        ...
        .filter_map(|e| match &e.kind {
            soulsystem_engine::EventKind::ActionExecuted {
                action_name,
                params_summary,
                ..
            ...
            soulsystem_engine::EventKind::ActionFailed {
                action_name, error, ..
            } => Some(serde_json::json!({
                "tool": action_name,
                "error": error,
...
async fn find_most_recent_thread(
    state: &EngineState,
    conv: &Option<soulsystem_engine::ConversationSurface>,
    user_id: &str,
) -> Option<soulsystem_engine::Thread> {
...
async fn handle_with_engine_inner(
            ...
            ) =>
        {
            ...
            let resolution =
                if content.trim().is_empty() || content.trim().eq_ignore_ascii_case("cancel") {
                    ...
                    soulsystem_engine::GateResolution::CredentialProvided {
    ...
    if violations
        .iter()
        .any(|rule| rule.action == soulsystem_safety::PolicyAction::Block)
    {
        return Ok(BridgeOutcome::Respond(
...
async fn await_thread_outcome(
    ...
    state: &EngineState,
    message: &IncomingMessage,
    conv_id: soulsystem_engine::ConversationId,
    thread_id: soulsystem_engine::ThreadId,
) -> Result<BridgeOutcome, Error> {
        ...
        ThreadOutcome::Completed { response } => {
                ...
                && text.contains("authentication_required")
            {
                ...
                let pending = PendingGate {
                    ...
                    resume_kind: soulsystem_engine::ResumeKind::Authentication {
                        credential_name: soulsystem_common::CredentialName::from_trusted(
                            cred_name.clone(),
                        ),
                ...
                let _ = agent
                    ...
                    .send_status(
                        &message.channel,
                        StatusUpdate::AuthRequired {
                            extension_name: soulsystem_common::ExtensionName::from_trusted(
                                cred_name.clone(),
                            ),
...
pub(crate) async fn handle_mission_notification(
    notif: &soulsystem_engine::MissionNotification,
    channels: &std::sync::Arc<crate::channels::ChannelManager>,
    sse: Option<&Arc<SseManager>>,
    db: Option<&Arc<dyn Database>>,
    conv_mgr: Option<&soulsystem_engine::ConversationManager>,
) {
...
async fn persist_v2_tool_calls(
    ...
    for msg in &thread.internal_messages {
        if msg.role != soulsystem_engine::MessageRole::ActionResult {
            continue;
        }
...
async fn forward_event_to_channel(
    event: &soulsystem_engine::ThreadEvent,
    channels: &std::sync::Arc<crate::channels::ChannelManager>,
    channel_name: &str,
    metadata: &serde_json::Value,
) {
    use soulsystem_engine::EventKind;
            ...
            if error.contains("authentication_required") {
                ...
                let _ = channels
                    .send_status(
                        channel_name,
                        StatusUpdate::AuthRequired {
                            extension_name: soulsystem_common::ExtensionName::from_trusted(cred_name),
                            instructions: Some(
                                "Store the credential with: soulsystem secret set <name> <value>"
                                    .into(),
                            ),
                            auth_url: None,
                            setup_url: None,
...
pub struct EngineProjectInfo {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub metrics: Vec<soulsystem_engine::ProjectMetric>,
    pub created_at: String,
}
...
fn cadence_type_label(cadence: &soulsystem_engine::types::mission::MissionCadence) -> &'static str {
    use soulsystem_engine::types::mission::MissionCadence;
    match cadence {
...
fn cadence_description(cadence: &soulsystem_engine::types::mission::MissionCadence) -> String {
    use soulsystem_engine::types::mission::MissionCadence;
    match cadence {
        MissionCadence::Cron {
            expression,
...
fn thread_to_info(t: &soulsystem_engine::Thread) -> EngineThreadInfo {
    EngineThreadInfo {
        id: t.id.to_string(),
        goal: t.goal.clone(),
...
pub async fn list_engine_threads(
    ...
    let pid = match project_id {
        Some(id) => {
            ...
            soulsystem_engine::ProjectId(uuid)
        }
...
pub async fn get_engine_thread(
    ...
    let tid = soulsystem_engine::ThreadId(tid);
...
pub async fn list_engine_thread_steps(
    ...
    if let Some(thread) = state
        .store
        .load_thread(soulsystem_engine::ThreadId(tid))
        .await
        .map_err(|e| engine_err("load thread", e))?
    {
        if !thread.is_owned_by(user_id) {
    ...
    let steps = state
        .store
        .load_steps(soulsystem_engine::ThreadId(tid))
        .await
...
pub async fn list_engine_thread_events(
    ...
    let events = state
        .store
        .load_events(soulsystem_engine::ThreadId(tid))
        .await
...
pub async fn get_engine_project(
    ...
    let project = state
        .store
        .load_project(soulsystem_engine::ProjectId(pid))
        .await
    ...
    for (project, (threads, missions)) in projects.iter().zip(project_data) {
        ...
        let active_missions = missions
            ...
            .filter(|m| {
                matches!(
                    m.status,
                    soulsystem_engine::types::mission::MissionStatus::Active
                )
            })
            .count() as u64;
        ...
        let failures_24h = threads
            .iter()
            .filter(|t| {
                matches!(t.state, soulsystem_engine::types::thread::ThreadState::Failed)
        ...
        for thread in &threads {
            if matches!(
                thread.state,
                soulsystem_engine::types::thread::ThreadState::Failed
        ...
        let health = if failures_24h > 0 || pending_gate_count > 0 {
            ...
            matches!(
                m.status,
                soulsystem_engine::types::mission::MissionStatus::Paused
            )
        }) {
            "yellow"
        } else {
...
pub async fn get_engine_mission(
    ...
    let mission = state
        .store
        .load_mission(soulsystem_engine::MissionId(mid))
        .await
...
pub async fn fire_engine_mission(mission_id: &str, user_id: &str) -> Result<Option<String>, Error> {
    ...
    let mid = uuid::Uuid::parse_str(mission_id).map_err(|e| engine_err("parse mission_id", e))?;
    let mid = soulsystem_engine::MissionId(mid);
...
pub async fn pause_engine_mission(
    ...
    mgr.pause_mission(soulsystem_engine::MissionId(mid), &effective_user_id)
        .await
...
pub async fn resume_engine_mission(
    ...
    let effective_user_id = resolve_mission_user_id(&state.store, mid, user_id, is_admin).await?;
    mgr.resume_mission(soulsystem_engine::MissionId(mid), &effective_user_id)
        .await
...
-> Vec<soulsystem_engine::executor::trace::ExecutionTrace> {
    let Some(lock) = ENGINE_STATE.get() else {
        return Vec::new();
    };
    let guard = lock.read().await;
    ...
    for project in projects {
        ...
        for mut thread in threads {
            ...
            out.push(soulsystem_engine::executor::trace::build_trace(&thread));
        }
    }
    out
}
...
async fn resolve_mission_user_id(
    store: &Arc<dyn soulsystem_engine::Store>,
    mid: uuid::Uuid,
    user_id: &str,
    is_admin: bool,
) -> Result<String, Error> {
    if let Ok(Some(mission)) = store.load_mission(soulsystem_engine::MissionId(mid)).await
...
async fn migrate_legacy_user_ids(store: &Arc<dyn soulsystem_engine::Store>, owner_id: &str) {
    // Projects
    ...
    let all_projects: Vec<soulsystem_engine::Project> =
    ...
    for project in &all_projects {
        ...
        if let Ok(legacy) = store.list_memory_docs(pid, "legacy").await {
            for mut doc in legacy {
                doc.user_id = if doc.doc_type == soulsystem_engine::DocType::Skill {
                    soulsystem_engine::types::shared_owner_id().to_string()
                } else {
                    owner_id.to_string()
                };
    ...
    if let Some(default_project) = all_projects.first() {
        let nil_pid = soulsystem_engine::ProjectId(uuid::Uuid::nil());
...
fn clamp_always_to_resume_kind(always: bool, resume_kind: &soulsystem_engine::ResumeKind) -> bool {
    always
        && matches!(
            resume_kind,
            soulsystem_engine::ResumeKind::Approval { allow_always: true }
        )
}
...
mod tests {
    ...
    use soulsystem_safety::SafetyLayer;
    use rust_decimal::Decimal;

    static ENGINE_STATE_TEST_LOCK: LazyLock<TokioMutex<()>> = LazyLock::new(|| TokioMutex::new(()));

    struct TestStore {
        conversations: TokioRwLock<Vec<soulsystem_engine::ConversationSurface>>,
        threads: TokioRwLock<HashMap<soulsystem_engine::ThreadId, soulsystem_engine::Thread>>,
        docs: TokioRwLock<Vec<soulsystem_engine::MemoryDoc>>,
        projects: TokioRwLock<Vec<soulsystem_engine::Project>>,
    }
    ...
    impl Store for TestStore {
        async fn save_thread(
            &self,
            thread: &soulsystem_engine::Thread,
        ) -> Result<(), soulsystem_engine::EngineError> {
            self.threads.write().await.insert(thread.id, thread.clone());
            Ok(())
        }
        async fn load_thread(
            &self,
            id: soulsystem_engine::ThreadId,
        ) -> Result<Option<soulsystem_engine::Thread>, soulsystem_engine::EngineError> {
            Ok(self.threads.read().await.get(&id).cloned())
        }
        async fn list_threads(
            &self,
            _project_id: soulsystem_engine::ProjectId,
            _user_id: &str,
        ) -> Result<Vec<soulsystem_engine::Thread>, soulsystem_engine::EngineError> {
            Ok(self.threads.read().await.values().cloned().collect())
        }
        async fn update_thread_state(
            &self,
            _id: soulsystem_engine::ThreadId,
            _state: soulsystem_engine::ThreadState,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn save_step(
            &self,
            _: &soulsystem_engine::Step,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn load_steps(
            &self,
            _: soulsystem_engine::ThreadId,
        ) -> Result<Vec<soulsystem_engine::Step>, soulsystem_engine::EngineError> {
            Ok(vec![])
        }
        async fn append_events(
            &self,
            _: &[soulsystem_engine::ThreadEvent],
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn load_events(
            &self,
            _: soulsystem_engine::ThreadId,
        ) -> Result<Vec<soulsystem_engine::ThreadEvent>, soulsystem_engine::EngineError> {
            Ok(vec![])
        }
        async fn save_project(
            &self,
            project: &soulsystem_engine::Project,
        ) -> Result<(), soulsystem_engine::EngineError> {
        ...
        async fn load_project(
            &self,
            id: soulsystem_engine::ProjectId,
        ) -> Result<Option<soulsystem_engine::Project>, soulsystem_engine::EngineError> {
            Ok(self
                .projects
                .read()
                .await
        ...
        async fn list_projects(
            &self,
            user_id: &str,
        ) -> Result<Vec<soulsystem_engine::Project>, soulsystem_engine::EngineError> {
            Ok(self
                .projects
                .read()
                .await
        ...
        async fn save_conversation(
            &self,
            conversation: &soulsystem_engine::ConversationSurface,
        ) -> Result<(), soulsystem_engine::EngineError> {
        ...
        async fn load_conversation(
            &self,
            id: soulsystem_engine::ConversationId,
        ) -> Result<Option<soulsystem_engine::ConversationSurface>, soulsystem_engine::EngineError>
        {
            Ok(self
                .conversations
                .read()
        ...
        async fn list_conversations(
            &self,
            user_id: &str,
        ) -> Result<Vec<soulsystem_engine::ConversationSurface>, soulsystem_engine::EngineError>
        {
            Ok(self
                .conversations
                .read()
        ...
        async fn save_memory_doc(
            &self,
            doc: &soulsystem_engine::MemoryDoc,
        ) -> Result<(), soulsystem_engine::EngineError> {
        ...
        async fn load_memory_doc(
            &self,
            id: soulsystem_engine::DocId,
        ) -> Result<Option<soulsystem_engine::MemoryDoc>, soulsystem_engine::EngineError> {
            Ok(self.docs.read().await.iter().find(|d| d.id == id).cloned())
        }
        async fn list_memory_docs(
            &self,
            project_id: soulsystem_engine::ProjectId,
            user_id: &str,
        ) -> Result<Vec<soulsystem_engine::MemoryDoc>, soulsystem_engine::EngineError> {
            Ok(self
                .docs
                .read()
                .await
        ...
        async fn save_lease(
            &self,
            _: &soulsystem_engine::CapabilityLease,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn load_active_leases(
            &self,
            _: soulsystem_engine::ThreadId,
        ) -> Result<Vec<soulsystem_engine::CapabilityLease>, soulsystem_engine::EngineError> {
            Ok(vec![])
        }
        async fn revoke_lease(
            &self,
            _: soulsystem_engine::LeaseId,
            _: &str,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn save_mission(
            &self,
            _: &soulsystem_engine::Mission,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
        async fn load_mission(
            &self,
            _: soulsystem_engine::MissionId,
        ) -> Result<Option<soulsystem_engine::Mission>, soulsystem_engine::EngineError> {
            Ok(None)
        }
        async fn list_missions(
            &self,
            _: soulsystem_engine::ProjectId,
            _user_id: &str,
        ) -> Result<Vec<soulsystem_engine::Mission>, soulsystem_engine::EngineError> {
            Ok(vec![])
        }
        async fn update_mission_status(
            &self,
            _: soulsystem_engine::MissionId,
            _: soulsystem_engine::MissionStatus,
        ) -> Result<(), soulsystem_engine::EngineError> {
            Ok(())
        }
    }
    ...
    fn sample_pending_gate(
        user_id: &str,
        thread_id: soulsystem_engine::ThreadId,
        resume_kind: soulsystem_engine::ResumeKind,
    ) -> PendingGate {
    ...
    fn sample_pending_gate_with_request_id(
        ...
        PendingGate {
            ...
            conversation_id: soulsystem_engine::ConversationId::new(),
    ...
    async fn make_router_test_agent(
        ...
        let deps = crate::agent::AgentDeps {
            ...
            cheap_llm: None,
            safety: Arc::new(soulsystem_safety::SafetyLayer::new(
                &soulsystem_safety::SafetyConfig {
    ...
    async fn insert_and_notify_pending_gate_sends_status_no_text() {
        ...
        let thread_id = soulsystem_engine::ThreadId::new();
        let expected_extension_name = "google_oauth_token".to_string();
        let pending = sample_pending_gate(
            "alice",
            thread_id,
            soulsystem_engine::ResumeKind::Authentication {
                credential_name: soulsystem_common::CredentialName::new(&expected_extension_name)
                    .unwrap(),
    ...
    async fn insert_and_notify_pending_gate_uses_extension_manager_for_auth_display_name() {
        ...
        let thread_id = soulsystem_engine::ThreadId::new();
        let credential = soulsystem_common::CredentialName::new("test_channel_token").unwrap();
        let pending = PendingGate {
    ...
    async fn resolve_pending_gate_is_thread_scoped() {
        ...
        let thread_a = soulsystem_engine::ThreadId::new();
        let thread_b = soulsystem_engine::ThreadId::new();
        store
    ...
    async fn resolve_pending_gate_detects_ambiguity_without_thread_hint() {
        ...
        store
            .insert(sample_pending_gate(
                "alice",
                soulsystem_engine::ThreadId::new(),
                soulsystem_engine::ResumeKind::Approval { allow_always: true },
            ))
            .await
            .unwrap();
    ...
    async fn resolve_pending_gate_filters_by_kind() {
        ...
        store
            .insert(sample_pending_gate(
                ...
                thread_id,
                soulsystem_engine::ResumeKind::Authentication {
                    credential_name: soulsystem_common::CredentialName::new("github").unwrap(),
    ...
    async fn handle_approval_ignores_pending_gate_from_different_thread() {
        ...
        *lock.write().await = None;
        ...
        let outcome = async {
            ...
            let pending_thread_id = soulsystem_engine::ThreadId::new();
            let active_thread_id = soulsystem_engine::ThreadId::new();
            let pending = sample_pending_gate(
                "alice",
                pending_thread_id,
                soulsystem_engine::ResumeKind::Approval { allow_always: true },
    ...
    fn resolved_call_id_prefers_stored_id_for_parallel_same_name_calls() {
        let mut thread = soulsystem_engine::Thread::new(
            "goal",
            soulsystem_engine::ThreadType::Foreground,
            soulsystem_engine::ProjectId::new(),
            "alice",
            soulsystem_engine::ThreadConfig::default(),
        );
        thread.add_message(soulsystem_engine::ThreadMessage::assistant_with_actions(
            Some("parallel shell calls".to_string()),
            vec![
                soulsystem_engine::ActionCall {
                    id: "call-1".to_string(),
    ...
    async fn forward_event_to_channel_preserves_call_id_for_action_events() {
        ...
        let event = soulsystem_engine::ThreadEvent::new(
            soulsystem_engine::ThreadId::new(),
            soulsystem_engine::EventKind::ActionExecuted {
                step_id: soulsystem_engine::StepId::new(),
    ...
    fn resolved_call_id_legacy_fallback_uses_last_unresolved_parallel_call() {
        ...
        thread.add_message(soulsystem_engine::ThreadMessage::action_result(
            "call-1",
            "shell",
            "{\"ok\":true}",
        ));
    ...
    fn resolved_call_id_legacy_fallback_scans_internal_messages() {
        ...
        thread.add_internal_message(soulsystem_engine::ThreadMessage::assistant_with_actions(
            Some("parallel shell calls".to_string()),
            vec![
                soulsystem_engine::ActionCall {
                    id: "call-1".to_string(),
        ...
        thread.add_internal_message(soulsystem_engine::ThreadMessage::action_result(
            "call-1",
            "shell",
            "{\"ok\":true}",
        ));
    ...
    fn resolved_call_id_returns_none_when_no_history_match() {
        let thread = soulsystem_engine::Thread::new(
            "goal",
            soulsystem_engine::ThreadType::Foreground,
            soulsystem_engine::ProjectId::new(),
            "alice",
    ...
    async fn clear_engine_pending_auth_scopes_to_thread_when_hint_provided() {
        ...
        state
            ...
            .insert(sample_pending_gate(
                ...
                thread_a,
                soulsystem_engine::ResumeKind::Authentication {
                    credential_name: soulsystem_common::CredentialName::new("github_token").unwrap(),
                    instructions: "paste token".into(),
                    auth_url: None,
                },
            ))
        ...
        state
            ...
            .insert(sample_pending_gate(
                ...
                thread_b,
                soulsystem_engine::ResumeKind::Authentication {
                    credential_name: soulsystem_common::CredentialName::new("linear_token").unwrap(),
                    instructions: "paste token".into(),
                    auth_url: None,
                },
            ))
    ...
    async fn discard_engine_pending_auth_request_discards_only_matching_auth_gate() {
        ...
        state
            ...
            .insert(sample_pending_gate_with_request_id(
                ...
                auth_request_id,
                soulsystem_engine::ResumeKind::Authentication {
                    credential_name: soulsystem_common::CredentialName::new("telegram_bot_token")
                        .unwrap(),
                    instructions: "paste token".into(),
                    auth_url: None,
                },
    ...
    fn make_expected_test_state(store: Arc<TestStore>) -> EngineState {
        ...
        struct NoopLlm;
        #[async_trait::async_trait]
        impl soulsystem_engine::LlmBackend for NoopLlm {
            async fn complete(
                &self,
                _: &[soulsystem_engine::ThreadMessage],
                _: &[soulsystem_engine::ActionDef],
                _: &soulsystem_engine::LlmCallConfig,
            ) -> Result<soulsystem_engine::LlmOutput, soulsystem_engine::EngineError> {
                Ok(soulsystem_engine::LlmOutput {
                    response: soulsystem_engine::LlmResponse::Text("done".into()),
                    usage: soulsystem_engine::TokenUsage::default(),
                })
            }
            fn model_name(&self) -> &str {
                "noop"
        ...
        struct NoopEffects;
        #[async_trait::async_trait]
        impl soulsystem_engine::EffectExecutor for NoopEffects {
            async fn execute_action(
                &self,
                _: &str,
                _: serde_json::Value,
                _: &soulsystem_engine::CapabilityLease,
                _: &soulsystem_engine::ThreadExecutionContext,
            ) -> Result<soulsystem_engine::ActionResult, soulsystem_engine::EngineError> {
                unreachable!()
            }
            async fn available_actions(
                &self,
                _: &[soulsystem_engine::CapabilityLease],
            ) -> Result<Vec<soulsystem_engine::ActionDef>, soulsystem_engine::EngineError> {
                Ok(vec![])
            }
        }
        ...
        let effect_adapter = Arc::new(EffectBridgeAdapter::new(
            ...
            Arc::new(soulsystem_safety::SafetyLayer::new(
                &soulsystem_safety::SafetyConfig {
        ...
        EngineState {
            ...
            effect_adapter,
            store: store_dyn,
            default_project_id: soulsystem_engine::ProjectId::new(),
    ...
    fn make_expected_test_state_with_llm(
        store: Arc<TestStore>,
        llm: Arc<dyn soulsystem_engine::LlmBackend>,
    ) -> EngineState {
        use soulsystem_engine::{
            CapabilityRegistry, ConversationManager, LeaseManager, PolicyEngine, ThreadManager,
        };
    ...
    async fn resolve_gate_repairs_call_id_for_resume_output_auth_resume() {
        ...
        #[async_trait::async_trait]
        impl soulsystem_engine::LlmBackend for InspectingLlm {
            async fn complete(
                &self,
                messages: &[soulsystem_engine::ThreadMessage],
                _: &[soulsystem_engine::ActionDef],
                _: &soulsystem_engine::LlmCallConfig,
            ) -> Result<soulsystem_engine::LlmOutput, soulsystem_engine::EngineError> {
                let matched = messages.iter().any(|message| {
                    message.role == soulsystem_engine::MessageRole::ActionResult
                        && message.action_name.as_deref() == Some("shell")
                        && message.action_call_id.as_deref() == Some(self.expected_call_id.as_str())
                });

                Ok(soulsystem_engine::LlmOutput {
                    response: soulsystem_engine::LlmResponse::Text(if matched {
                        "paired".into()
                    } else {
        ...
        *lock.write().await = None;
        ...
        let outcome = async {
            let store = Arc::new(TestStore::new());
            let llm: Arc<dyn soulsystem_engine::LlmBackend> = Arc::new(InspectingLlm {
            ...
            thread.state = soulsystem_engine::ThreadState::Waiting;
            store
                .save_thread(&thread)
                .await
                .expect("save waiting thread");

            let mut conversation = soulsystem_engine::ConversationSurface::new("web", "alice");
            ...
            let pending = PendingGate {
                ...
                ..sample_pending_gate(
                    ...
                    thread.id,
                    soulsystem_engine::ResumeKind::Authentication {
                        credential_name: soulsystem_common::CredentialName::new("github_token")
                            .unwrap(),
                        instructions: "paste token".into(),
                        auth_url: None,
                    },
        ...
        *lock.write().await = None;
        ...
        let outcome = async {
            ...
            thread.add_message(soulsystem_engine::ThreadMessage::assistant_with_actions(
                ...
                vec![soulsystem_engine::ActionCall {
    ...
    async fn resolve_gate_fails_waiting_thread_when_no_auth_backend_and_no_resume_output() {
        ...
        *lock.write().await = None;
        ...
        let outcome = async {
            ...
            let credential = soulsystem_common::CredentialName::new("github_token").unwrap();
            let pending = PendingGate {
                conversation_id,
                action_name: "shell".into(),
                parameters: serde_json::json!({"cmd": "ls"}),
            ...
            *lock.write().await = Some(state);
            ...
            assert_eq!(saved.state, soulsystem_engine::ThreadState::Failed);
    ...
    async fn find_recent_thread_returns_active() {
        ...
        thread.add_message(soulsystem_engine::ThreadMessage::user("hello"));
        thread.add_message(soulsystem_engine::ThreadMessage::assistant("hi there"));
        let tid = thread.id;
        store.save_thread(&thread).await.unwrap();

        let mut conv = soulsystem_engine::ConversationSurface::new("web", "alice");
    ...
    async fn find_recent_thread_empty_conv_returns_none() {
        ...
        let conv = Some(soulsystem_engine::ConversationSurface::new("web", "alice"));
    ...
    async fn find_recent_thread_falls_back_to_entries() {
        ...
        thread.add_message(soulsystem_engine::ThreadMessage::user("do something"));
        thread.add_message(soulsystem_engine::ThreadMessage::assistant("done"));
        let tid = thread.id;
        store.save_thread(&thread).await.unwrap();

        // Conversation with no active threads, but an entry referencing the thread
        let mut conv = soulsystem_engine::ConversationSurface::new("web", "alice");
        conv.add_entry(soulsystem_engine::ConversationEntry::agent(tid, "done"));
    ...
    async fn migrate_legacy_user_ids_preserves_shared_ownership_for_skills() {
        ...
        let project = soulsystem_engine::Project::new("admin", "default", "test");
        store.save_project(&project).await.unwrap();

        // Seed legacy docs: a Skill and a Note, both with user_id = "legacy"
        // (simulating pre-PR deserialization fallback).
        let mut skill_doc = soulsystem_engine::MemoryDoc::new(
            project.id,
            "legacy",
            soulsystem_engine::DocType::Skill,
            "skill:bundled-tool",
        ...
        let mut note_doc = soulsystem_engine::MemoryDoc::new(
            project.id,
            "legacy",
            soulsystem_engine::DocType::Note,
            "note:scratch",
            "Some scratch notes",
        );
        ...
        // Run the migration.
        let store_dyn: Arc<dyn soulsystem_engine::Store> = store.clone();
        ...
        assert_eq!(
            skill.user_id,
            soulsystem_engine::types::shared_owner_id(),
    ...
    fn make_persistence_test_state(
        ...
        impl soulsystem_engine::LlmBackend for NoopLlm {
            async fn complete(
                ...
                Ok(soulsystem_engine::LlmOutput {
                    response: soulsystem_engine::LlmResponse::Text("ok".into()),
                    usage: soulsystem_engine::TokenUsage::default(),
                })
            }
            fn model_name(&self) -> &str {
        ...
        let store: Arc<dyn soulsystem_engine::Store> = Arc::new(TestStore::new());
    ...
    async fn test_persist_always_allow_writes_to_settings() {
        ...
        let tid = soulsystem_engine::ThreadId::new();
        let pending = sample_pending_gate(
            "user1",
            tid,
            soulsystem_engine::ResumeKind::Approval { allow_always: true },
    ...
    fn clamp_approval_with_allow_always_passes_through() {
        let rk = soulsystem_engine::ResumeKind::Approval { allow_always: true };
    ...
    fn clamp_approval_without_allow_always_clamps_to_false() {
        ...
        let rk = soulsystem_engine::ResumeKind::Approval {
            allow_always: false,
        };
        assert!(!super::clamp_always_to_resume_kind(true, &rk));
        assert!(!super::clamp_always_to_resume_kind(false, &rk));
    ...
    fn clamp_auth_resume_kind_clamps_to_false() {
        ...
        let rk = soulsystem_engine::ResumeKind::Authentication {
            credential_name: soulsystem_common::CredentialName::new("github_token").unwrap(),
    ...
    fn clamp_external_callback_clamps_to_false() {
        let rk = soulsystem_engine::ResumeKind::External {
    ...
    async fn persist_v2_tool_calls_skips_when_no_action_results() {
        ...
        thread.add_internal_message(soulsystem_engine::ThreadMessage::user("hello"));
        let thread_id = thread.id;
        store.save_thread(&thread).await.unwrap();
    ...
    fn sample_lease_for_pending(pending: &PendingGate) -> soulsystem_engine::CapabilityLease {
        soulsystem_engine::CapabilityLease {
            id: soulsystem_engine::types::capability::LeaseId::new(),
            thread_id: pending.thread_id,
            capability_name: "tools".into(),
            granted_actions: soulsystem_engine::GrantedActions::Specific(vec![
    ...
    async fn resume_lease_prefers_snapshot_even_when_lease_manager_empty() {
        ...
        let leases = soulsystem_engine::LeaseManager::new();
    ...
    async fn resume_lease_rejects_revoked_snapshot_and_falls_back() {
        ...
        let live_lease = leases
            .grant(
                thread_id,
                "tools",
                soulsystem_engine::GrantedActions::Specific(vec!["shell".into()]),
                None,
                None,
            )
            .await
    ...
    async fn resume_lease_rejects_snapshot_with_wrong_thread_id() {
        ...
        mismatched_snapshot.thread_id = soulsystem_engine::ThreadId::new(); // different thread
    ...
    async fn resume_lease_rejects_snapshot_missing_action_coverage() {
        ...
        mismatched_snapshot.granted_actions =
            soulsystem_engine::GrantedActions::Specific(vec!["unrelated_tool".into()]);
        pending.paused_lease = Some(mismatched_snapshot);
