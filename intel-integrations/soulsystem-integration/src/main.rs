...
#[cfg(unix)]
use soulsystem::channels::ChannelSecretUpdater;
#[cfg(any(feature = "postgres", feature = "libsql"))]
use soulsystem::setup::{SetupConfig, SetupWizard};
...
fn main() -> anyhow::Result<()> {
    ...
    soulsystem::bootstrap::load_soulsystem_env();
...
fn format_top_level_error(err: &anyhow::Error) {
    ...
    {
        Some("run `soulsystem onboard` or set DATABASE_URL in .env")
        ...
        Some("run `soulsystem onboard` to set up authentication")
    } else if lower.contains("secrets_master_key") {
        Some("run `soulsystem onboard` or set SECRETS_MASTER_KEY in .env")
        ...
        Some("run `soulsystem onboard` to complete setup")
    } else {
        None
    };
    ...
    S: AsRef<str>,
{
    ...
    for name in names {
        match soulsystem_common::ExtensionName::new(name.as_ref()) {
            Ok(ext_name) => {
...
async fn persisted_active_wasm_channel_names(
    ext_mgr: &soulsystem::extensions::ExtensionManager,
    user_id: &str,
...
async fn async_main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_config_command(config_cmd.clone()).await;
        }
        Some(Command::Registry(registry_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_registry_command(registry_cmd.clone()).await;
        }
        Some(Command::Channels(channels_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_channels_command(
                channels_cmd.clone(),
                cli.config.as_deref(),
            )
            .await;
        ...
        Some(Command::Routines(routines_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_routines_cli(routines_cmd, cli.config.as_deref()).await;
        }
        ...
        Some(Command::Memory(mem_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_memory_command(mem_cmd).await;
        }
        ...
        Some(Command::Skills(skills_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_skills_command(skills_cmd.clone(), cli.config.as_deref())
                .await;
        }
        Some(Command::Hooks(hooks_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_hooks_command(hooks_cmd.clone(), cli.config.as_deref())
                .await;
        }
        Some(Command::Logs(logs_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_logs_command(logs_cmd.clone(), cli.config.as_deref()).await;
        }
        Some(Command::Models(models_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_models_command(models_cmd.clone(), cli.config.as_deref())
                .await;
        }
        Some(Command::Doctor) => {
            init_cli_tracing();
            return soulsystem::cli::run_doctor_command().await;
        }
        Some(Command::Status) => {
            init_cli_tracing();
        ...
        Some(Command::Import(import_cmd)) => {
            init_cli_tracing();
            let config = soulsystem::config::Config::from_env().await?;
            return soulsystem::cli::run_import_command(import_cmd, &config).await;
        }
        Some(Command::Acp(acp_cmd)) => {
            init_cli_tracing();
            return soulsystem::cli::run_acp_command(acp_cmd.clone()).await;
        }
        Some(Command::Worker {
            job_id,
            orchestrator_url,
            ...
            init_worker_tracing();
            return soulsystem::worker::run_worker(*job_id, orchestrator_url, *max_iterations).await;
        }
        Some(Command::ClaudeBridge {
            ...
            init_worker_tracing();
            return soulsystem::worker::run_claude_bridge(
                *job_id,
                orchestrator_url,
                *max_turns,
                model,
        ...
        Some(Command::AcpBridge {
            ...
            init_worker_tracing();
            return soulsystem::worker::run_acp_bridge(*job_id, orchestrator_url).await;
        }
            ...
            if *openai_codex {
                ...
                let codex_config = {
                    ...
                    config.llm.openai_codex.unwrap_or_else(|| {
                        use soulsystem::llm::OpenAiCodexConfig;
                ...
                let mgr = soulsystem::llm::OpenAiCodexSessionManager::new(codex_config)
                    .map_err(|e| anyhow::anyhow!("{}", e))?;
                ...
                println!("  soulsystem login --openai-codex   (ChatGPT subscription)");
            }
            return Ok(());
        }
    ...
    let _pid_lock = match soulsystem::bootstrap::PidLock::acquire() {
        Ok(lock) => Some(lock),
        Err(soulsystem::bootstrap::PidLockError::AlreadyRunning { pid }) => {
            anyhow::bail!(
                "Another SoulSystem instance is already running (PID {}). \
                 If this is incorrect, remove the stale PID file: {}",
                pid,
                soulsystem::bootstrap::pid_lock_path().display()
            );
        }
        Err(e) => {
    ...
    #[cfg(any(feature = "postgres", feature = "libsql"))]
    if !cli.no_onboard
        && let Some(reason) = soulsystem::setup::check_onboard_needed()
    {
    ...
    if cli.auto_approve {
        soulsystem::config::set_runtime_env("AGENT_AUTO_APPROVE_TOOLS", "true");
    }
    ...
    let config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(soulsystem::error::ConfigError::MissingRequired { key, hint }) => {
            anyhow::bail!(
                "Configuration error: Missing required setting '{}'. {}. \
                 Run 'soulsystem onboard' to configure, or set the required environment variables.",
                key,
                hint
            );
        }
    ...
    let log_level_handle = soulsystem::channels::web::log_layer::init_tracing(
        Arc::clone(&log_broadcaster),
        suppress_stderr,
    );

    tracing::debug!("Starting SoulSystem...");
    ...
    let (config, active_tunnel) = if enable_non_cli {
        soulsystem::tunnel::start_managed_tunnel(config).await
    } else {
        (config, None)
    };
    ...
    let (container_job_manager, job_event_tx, prompt_queue, docker_status) = if enable_non_cli {
        let orch = soulsystem::orchestrator::setup_orchestrator(
            &config,
            &components.llm,
        ...
        (
            ...
            soulsystem::sandbox::DockerStatus::Disabled,
        )
    };
    ...
    if tui_mode && cli.message.is_none() {
        ...
        let tool_categories = soulsystem::channels::tui::group_tools_by_prefix(tool_names);
        ...
        let skill_categories = if let Some(ref registry) = components.skill_registry {
            ...
            let skill_data: Vec<(String, Vec<String>)> = registry
                ...
                .collect();
            soulsystem::channels::tui::group_skills_by_tag(&skill_data)
        } else {
            Vec::new()
        };
        ...
        let layout = if let Some(ref tui_config) = config.channels.tui {
            soulsystem::channels::tui::resolve_tui_layout(tui_config, &workspace_root)
        } else {
            soulsystem_tui::TuiLayout::default()
        };
        ...
        let tui_channel = soulsystem::channels::TuiChannel::new(
    ...
    let shared_routine_engine_slot: soulsystem::channels::web::server::RoutineEngineSlot =
    ...
    {
        let wasm_result = soulsystem::channels::wasm::setup_wasm_channels(
            &config,
    ...
    #[cfg(unix)]
    let mut http_channel_state: Option<Arc<soulsystem::channels::HttpChannelState>> = None;
    ...
    let scheduler_slot: soulsystem::tools::builtin::SchedulerSlot =
        Arc::new(tokio::sync::RwLock::new(None));
    ...
    let mut sse_manager: Option<std::sync::Arc<soulsystem::channels::web::sse::SseManager>> = None;
    if enable_non_cli && let Some(ref gw_config) = config.channels.gateway {
        ...
        if let Some(ref db) = components.db {
            let emb_cache_config = soulsystem::workspace::EmbeddingCacheConfig {
                max_entries: config.embeddings.cache_size,
            };
            let pool = Arc::new(soulsystem::channels::web::server::WorkspacePool::new(
                Arc::clone(db),
        ...
        if let Some(ref db) = components.db {
            let dispatcher = Arc::new(soulsystem::tools::dispatch::ToolDispatcher::new(
        ...
        if let Some(ref d) = components.db {
            ...
            let pairing_store = Arc::new(soulsystem::pairing::PairingStore::new(
                Arc::clone(d),
                Arc::clone(&components.ownership_cache),
            ));
            ...
            if let Ok(false) = d.has_any_users().await {
                ...
                let user = soulsystem::db::UserRecord {
                ...
                if auth_token.is_empty() {
                    ...
                    use soulsystem::channels::web::auth::hash_token;
        ...
        gw = gw.with_oauth(config.oauth.clone(), gw_config.port);
        {
            ...
            gw = gw.with_active_config(soulsystem::channels::web::server::ActiveConfigSnapshot {
        ...
        if gw_config.auth_token.is_none() {
            ...
            tokio::spawn(async move {
                if let Err(e) = soulsystem::bootstrap::upsert_bootstrap_var(
                    "GATEWAY_AUTH_TOKEN",
                    &token_to_persist,
                ) {
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = soulsystem::boot_screen::BootInfo {
        ...
        soulsystem::boot_screen::print_boot_screen(&boot_info);
    }
        ...
        for name in &persisted_active_wasm_channels {
            ...
            match ext_mgr
                .ensure_extension_ready(
                    name,
                    &ext_user_id,
                    soulsystem::extensions::EnsureReadyIntent::ExplicitActivate,
                )
                .await
            {
                Ok(soulsystem::extensions::EnsureReadyOutcome::Ready { activation, .. }) => {
                ...
                Ok(soulsystem::extensions::EnsureReadyOutcome::NeedsAuth { auth, .. }) => {
                    tracing::warn!(
                        channel = %name,
                ...
                Ok(soulsystem::extensions::EnsureReadyOutcome::NeedsSetup {
                    instructions, ..
                }) => {
                    tracing::warn!(
                        channel = %name,
    ...
    let http_interceptor = soulsystem::http_intercept::chain(
        [
    ...
    #[cfg(unix)]
    let sighup_settings_store: Option<Arc<dyn soulsystem::db::SettingsStore>> = components
        .settings_store
        .as_ref()
        .map(|s| Arc::clone(s) as Arc<dyn soulsystem::db::SettingsStore>)
        .or_else(|| {
            components
                .db
                .as_ref()
                .map(|db| Arc::clone(db) as Arc<dyn soulsystem::db::SettingsStore>)
        });
    #[cfg(unix)]
    ...
    let auth_manager = components.tools.secrets_store().cloned().map(|secrets| {
        Arc::new(soulsystem::bridge::auth_manager::AuthManager::new(
            secrets,
    ...
    let deps = AgentDeps {
        ...
        transcription: config.transcription.create_provider().map(|p| {
            Arc::new(soulsystem::llm::transcription::TranscriptionMiddleware::new(
                p,
            ))
        }),
        document_extraction: Some(Arc::new(
            soulsystem::document_extraction::DocumentExtractionMiddleware::new(),
        )),
        sandbox_readiness: if !config.sandbox.enabled
            || matches!(docker_status, soulsystem::sandbox::DockerStatus::Disabled)
        {
            soulsystem::agent::routine_engine::SandboxReadiness::DisabledByConfig
        } else if docker_status.is_ok() {
            soulsystem::agent::routine_engine::SandboxReadiness::Available
        } else {
            soulsystem::agent::routine_engine::SandboxReadiness::DockerUnavailable
        },
        builder: components.builder,
        llm_backend: config.llm.backend.clone(),
        tenant_rates: Arc::new(soulsystem::tenant::TenantRateRegistry::new(
    ...
    *scheduler_slot.write().await = Some(agent.scheduler());
            ...
            loop {
                ...
                if let Some(ref secrets_store) = sighup_secrets_store {
                        ...
                        .await
                    {
                        ...
                        soulsystem::config::inject_single_var(
                            "HTTP_WEBHOOK_SECRET",
                ...
                let new_config = match &sighup_settings_store_clone {
                    Some(store) => {
                        soulsystem::config::Config::from_db(store.as_ref(), &sighup_owner_id).await
                    }
                    None => soulsystem::config::Config::from_env().await,
                };
    ...
    if let Some(warning) = docker_user_warning {
        ...
        tokio::spawn(async move {
            ...
            let response = soulsystem::channels::OutgoingResponse {
...
mod tests {
    ...
    /// Regression test for <https://github.com/nearai/soulsystem/issues/1840>:
    ...
    ///
    /// Regression coverage for <https://github.com/nearai/soulsystem/issues/1840>.
    #[test]
