//! TUI channel — bridges the `Channel` trait to `soulsystem_tui`.
//!
//! The TUI crate owns the terminal and event loop. This module translates
//! between the agent's `Channel` trait and `soulsystem_tui`'s event/message
//! channels.
...
use soulsystem_tui::{SkillCategory, ToolCategory, TuiAppConfig, TuiEvent, TuiLayout, start_tui};
...
fn build_tui_incoming_message(
    user_msg: soulsystem_tui::TuiUserMessage,
    user_id: &str,
    sys_tz: &str,
) -> IncomingMessage {
...
fn build_engine_thread_detail_event(detail: crate::bridge::EngineThreadDetail) -> TuiEvent {
    let messages = detail
        .messages
        .into_iter()
        .map(|message| soulsystem_tui::EngineThreadMessageEntry {
            role: message
                .get("role")
    ...
    TuiEvent::EngineThreadDetail {
        detail: soulsystem_tui::EngineThreadDetailEntry {
            id: detail.info.id,
...
/// TUI channel backed by `soulsystem_tui`.
...
impl Channel for TuiChannel {
    ...
    async fn start(&self) -> Result<MessageStream, ChannelError> {
        ...
        let soulsystem_tui::TuiAppHandle {
            event_tx,
            mut msg_rx,
            join_handle: _join,
        } = start_tui(config);
        ...
        tokio::spawn(async move {
            while let Some(user_msg) = msg_rx.recv().await {
                if let Some(action) = user_msg.ui_action {
                    match action {
                        soulsystem_tui::TuiUiAction::OpenEngineThreadDetail { thread_id } => {
    ...
    async fn send_status(
        ...
        let event = match status {
            ...
            StatusUpdate::ThreadList { threads } => TuiEvent::ThreadList {
                threads: threads
                    .into_iter()
                    .map(|t| soulsystem_tui::ThreadEntry {
                        id: t.id,
                        title: t.title,
            ...
            StatusUpdate::EngineThreadList { threads } => TuiEvent::EngineThreadList {
                threads: threads
                    .into_iter()
                    .map(|t| soulsystem_tui::EngineThreadEntry {
                        id: t.id,
                        goal: t.goal,
            ...
            StatusUpdate::ConversationHistory {
                ...
                messages: messages
                    .into_iter()
                    .map(|m| soulsystem_tui::HistoryMessage {
                        role: m.role,
                        content: m.content,
                ...
                pending_approval: pending_approval.map(|approval| {
                    soulsystem_tui::HistoryApprovalRequest {
...
#[cfg(test)]
mod tests {
    use soulsystem_tui::TuiUserMessage;
