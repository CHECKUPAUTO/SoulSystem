...
pub use soulsystem_common::truncate_preview;
...
pub fn tool_error_for_display(error: &str) -> String {
    soulsystem_safety::SafetyLayer::unwrap_tool_output(error).unwrap_or_else(|| error.to_string())
}
...
pub fn tool_result_for_display(result: &serde_json::Value) -> Option<String> {
    ...
    let content = match result {
        serde_json::Value::String(s) => {
            soulsystem_safety::SafetyLayer::unwrap_tool_output(s).unwrap_or_else(|| s.clone())
        }
