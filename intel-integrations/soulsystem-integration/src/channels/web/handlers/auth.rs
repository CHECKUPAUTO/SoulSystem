...
async fn handle_callback(
    ...
    if !state.oauth_allowed_domains.is_empty() {
        ...
        if let Err(msg) = crate::channels::web::platform::auth::check_email_domain(
            profile.email.as_deref(),
            &state.oauth_allowed_domains,
        ) {
            tracing::warn!(
                provider = %provider_name,
                email = ?profile.email,
...
pub async fn near_challenge_handler(
    ...
    let message = format!("Sign in to SoulSystem\nNonce: {nonce}");

    Ok(Json(serde_json::json!({
        "nonce": nonce,
        "message": message,
        "recipient": "soulsystem",
    })))
}
...
pub async fn near_verify_handler(
    ...
    let message_str = format!("Sign in to SoulSystem\nNonce: {}", body.nonce);
