...
pub async fn skills_search_handler(
    ...
    let catalog_json: Vec<serde_json::Value> = entries
        .into_iter()
        .map(|e| {
            let is_installed = soulsystem_skills::catalog::catalog_entry_is_installed(
                &e.slug,
                &e.name,
                &installed_names,
            );
...
pub async fn skills_install_handler(
    ...
    let content = if let Some(ref raw) = req.content {
        ...
        let download_key = if let Some(slug) = req.slug.as_deref().filter(|s| !s.is_empty()) {
            ...
            match soulsystem_skills::catalog::resolve_catalog_slug_for_name(
                &req.name,
                &outcome.results,
            ) {
        ...
        let url =
            soulsystem_skills::catalog::skill_download_url(catalog.registry_url(), &download_key);
...
mod tests {
    ...
    fn catalog_entry_matches_installed_slug_suffix() {
        ...
        assert!(soulsystem_skills::catalog::catalog_entry_is_installed(
