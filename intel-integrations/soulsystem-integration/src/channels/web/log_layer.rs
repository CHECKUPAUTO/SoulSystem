...
use soulsystem_safety::LeakDetector;
...
/// (e.g. `soulsystem=debug`) without restarting the process.
...
impl LogLevelHandle {
    ...
    /// Change the `soulsystem=<level>` directive at runtime.
    ///
    ...
    pub fn set_level(&self, level: &str) -> Result<(), String> {
        ...
        let filter_str = if self.base_filter.is_empty() {
            format!("soulsystem={}", level)
        } else {
            format!("soulsystem={},{}", level, self.base_filter)
        };
    ...
    /// Returns the current soulsystem log level (e.g. "info", "debug").
...
pub fn init_tracing(
    ...
    let raw_filter =
        std::env::var("RUST_LOG").unwrap_or_else(|_| "soulsystem=info,tower_http=warn".to_string());

    // Split into the soulsystem directive and "everything else" (base_filter).
    let mut soulsystem_level = String::from("info");
    let mut base_parts: Vec<&str> = Vec::new();

    for part in raw_filter.split(',') {
        let trimmed = part.trim();
        if trimmed.starts_with("soulsystem=") {
            if let Some(lvl) = trimmed.strip_prefix("soulsystem=") {
                soulsystem_level = lvl.to_string();
            }
...
///   INFO soulsystem::agent: Request completed url="http://..." status=200
///
...
mod tests {
    ...
    fn test_log_broadcaster_subscribe() {
        ...
        broadcaster.send(LogEntry {
            ...
            target: "soulsystem::test".to_string(),
    ...
    fn test_log_entry_serialization() {
        let entry = LogEntry {
            ...
            target: "soulsystem::agent".to_string(),
    ...
    fn test_leak_detector_scrubs_api_key_in_log() {
        let detector = soulsystem_safety::LeakDetector::new();
