...
//! resolve while the soulsystem#2599 migration is in progress.
...
// --- Backward-compat re-exports for the soulsystem#2599 migration ---
//
                ...
                crate::extensions::ExtensionSource::WasmBuildable { .. } => {
                    format!(
                        "'{}' requires building from source. \
                         Run `soulsystem registry install {}` from the CLI.",
                        req.name, req.name
                    )
                }
                _ => format!(
...
pub(crate) async fn extensions_activate_handler(
    ...
    let name = soulsystem_common::ExtensionName::new(&name).map_err(|e| {
        (
...
mod tests {
    ...
    use soulsystem_gateway::{NONCE_PLACEHOLDER, assets};
    ...
    async fn pending_gate_extension_name_falls_back_to_provider_extension() {
        ...
        let extension_name = pending_gate_extension_name(
            ...
            "{}",
            &soulsystem_engine::ResumeKind::Authentication {
                credential_name: soulsystem_common::CredentialName::new("notion_token").unwrap(),
                instructions: "paste token".to_string(),
                auth_url: None,
            },
        )
        ...
        *state.msg_tx.write().await = Some(tx);
        let thread_id = {
            ...
            let thread_id = {
                ...
                thread.enter_auth_mode(soulsystem_common::ExtensionName::new("telegram").unwrap());
                thread_id
            };
    ...
    async fn test_chat_auth_cancel_handler_clears_requested_thread_auth_mode() {
            ...
            .session_manager = Some(Arc::clone(&session_manager));
        {
            ...
            sess.create_thread_with_id(target_thread_id, Some("gateway"))
                .enter_auth_mode(soulsystem_common::ExtensionName::new("telegram").unwrap());
            sess.create_thread_with_id(other_thread_id, Some("gateway"))
                .enter_auth_mode(soulsystem_common::ExtensionName::new("notion").unwrap());
    ...
    async fn test_chat_auth_token_handler_expired_auth_broadcasts_failed_onboarding_state() {
        ...
        let expected_thread_id = {
            ...
            thread.pending_auth = Some(crate::agent::session::PendingAuth {
                extension_name: soulsystem_common::ExtensionName::new("telegram").unwrap(),
    ...
    fn fresh_pending_oauth_flow(
        ...
        crate::auth::oauth::PendingOAuthFlow {
            extension_name: soulsystem_common::ExtensionName::new("test_tool").unwrap(),
    ...
    async fn test_extensions_readiness_handler_reports_phase_summary() {
        ...
        // developer's real `~/.soulsystem/mcp-servers.json` (which would
    ...
    fn test_stamp_nonce_into_html_does_not_mutate_widget_body() {
        ...
        // mentions `__SOULSYSTEM_CSP_NONCE__` in a comment, log line, or
    ...
    async fn test_oauth_callback_happy_path_with_gateway_token_fallback() {
        ...
        let _exchange_url_guard =
            set_env_var("SOULSYSTEM_OAUTH_EXCHANGE_URL", Some(&proxy.base_url()));
        let _proxy_auth_guard = set_env_var("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN", None);
    ...
    async fn test_oauth_callback_exchange_failure_broadcasts_auth_completed_failure() {
        ...
        let _env_guard = crate::config::helpers::lock_env();
        let _exchange_url_guard =
            set_env_var("SOULSYSTEM_OAUTH_EXCHANGE_URL", Some("http://127.0.0.1:1"));
        let _proxy_auth_guard = set_env_var("SOULSYSTEM_OAUTH_PROXY_AUTH_TOKEN", None);
        let _gateway_token_guard = set_env_var("GATEWAY_AUTH_TOKEN", Some("gateway-test-token"));
    ...
    /// `~/.soulsystem/mcp-servers.json`. Any test that calls `install` for an
    ...
    async fn test_ext_mgr_with_db() -> (
        ...
        // fall back to `~/.soulsystem/mcp-servers.json` on dev machines.
