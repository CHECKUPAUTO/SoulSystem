...
pub const SESSION_COOKIE_NAME: &str = "soulsystem_session";
...
/// back-edge that would otherwise violate the soulsystem#2599 layering
...
mod tests {
    ...
    fn test_extract_cookie_value_uses_cookie_parser() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("other=\"quoted;value\"; soulsystem_session=abc123"),
        );
    ...
    async fn test_cookie_session_authenticates() {
        ...
        let req = Request::builder()
            ...
            .header(
                "Cookie",
                format!("soulsystem_session={TEST_AUTH_SECRET_TOKEN}"),
            )
            .body(Body::empty())
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
    ...
    async fn test_bearer_header_takes_priority_over_cookie() {
        ...
        let req = Request::builder()
            .uri("/api/chat/history")
            .header("Authorization", format!("Bearer {TEST_AUTH_SECRET_TOKEN}"))
            .header("Cookie", "soulsystem_session=wrong-token")
            .body(Body::empty())
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn test_empty_cookie_ignored() {
        ...
        let req = Request::builder()
            .uri("/api/chat/history")
            .header("Cookie", "soulsystem_session=")
            .body(Body::empty())
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    ...
    async fn test_cookie_among_multiple() {
        ...
        let req = Request::builder()
            ...
            .header(
                "Cookie",
                format!("other=foo; soulsystem_session={TEST_AUTH_SECRET_TOKEN}; bar=baz"),
            )
            .body(Body::empty())
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
