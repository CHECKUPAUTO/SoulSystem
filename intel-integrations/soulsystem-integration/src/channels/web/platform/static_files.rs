...
use soulsystem_gateway::assets;
use soulsystem_gateway::{
...
/// See `crates/soulsystem_gateway/static/js/core/widgets.js` — the
/// layout-config IIFE already reads `window.__SOULSYSTEM_LAYOUT__`, which
...
pub(crate) async fn build_frontend_html(state: &GatewayState) -> Option<String> {
    ...
    let html = if widgets.is_empty() && !layout_has_customizations(&layout) {
        ...
        Some(soulsystem_gateway::assemble_index(
            assets::INDEX_HTML,
            &bundle,
        ))
    };
...
//
// All frontend assets are embedded in the `soulsystem_gateway` crate.
...
/// `__SOULSYSTEM_CSP_NONCE__` inside their own source — in a comment, a log
...
/// `nonce="__SOULSYSTEM_CSP_NONCE__"`, which is the exact shape
...
/// Shared logic: resolve the file inside `~/.soulsystem/projects/{project_id}/`,
...
async fn serve_project_file(project_id: &str, path: &str) -> axum::response::Response {
    ...
    let base = soulsystem_base_dir().join("projects").join(project_id);
...
// further trimmed in the next soulsystem#2599 increment.
