...
///
/// Used on hosted instances where `SOULSYSTEM_OAUTH_CALLBACK_URL` points to
...
pub(crate) async fn oauth_callback_handler(
    ...
    let state_param = match params.get("state") {
        ...
        _ => {
            return oauth_error_page("SoulSystem");
        }
    };
...
pub(crate) async fn slack_relay_oauth_callback_handler(
    ...
    state.sse.broadcast(AppEvent::OnboardingState {
        extension_name: soulsystem_common::ExtensionName::from_trusted(relay_extension_name.clone()),
        state: if success {
            crate::channels::web::types::OnboardingStateDto::Ready
        } else {
            crate::channels::web::types::OnboardingStateDto::Failed
    ...
    if success {
        axum::response::Html(
            "<html><body style='font-family: system-ui; text-align: center; padding: 60px;'>\
             ...
             <p>You can close this tab and return to SoulSystem.</p>\
