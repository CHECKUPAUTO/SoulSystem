...
/// Get the history file path (~/.soulsystem/history).
fn history_path() -> std::path::PathBuf {
    soulsystem_base_dir().join("history")
}
    ...
    async fn start(&self) -> Result<MessageStream, ChannelError> {
        ...
        std::thread::spawn(move || {
            ...
            if !suppress_banner.load(Ordering::Relaxed) {
                println!(
                    "{}SoulSystem{}  /help for commands, /quit to exit",
                    fmt::bold(),
                    fmt::reset()
                );
                println!();
...
mod tests {
    ...
    async fn auth_required_status_unblocks_input_polling_loop() {
        ...
        repl.send_status(
            StatusUpdate::AuthRequired {
                extension_name: soulsystem_common::ExtensionName::new("google_oauth_token").unwrap(),
