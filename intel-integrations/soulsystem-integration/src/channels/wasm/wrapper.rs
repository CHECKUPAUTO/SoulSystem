...
const TEST_HTTP_REWRITE_MAP_ENV: &str = "SOULSYSTEM_TEST_HTTP_REWRITE_MAP";
...
///
/// `SOULSYSTEM_TEST_HTTP_REWRITE_MAP` is a JSON object mapping exact hostnames to
...
fn read_attachments(paths: &[String]) -> Result<Vec<wit_channel::Attachment>, String> {
    ...
    let home_base = dirs::home_dir()
        .map(|h| h.join(".soulsystem"))
        .unwrap_or_default();

    for path in paths {
        // Validate paths are under /tmp/ or ~/.soulsystem/ to prevent arbitrary file reads
        ...
        let validated = validated.map_err(|e| {
            format!(
                "Invalid attachment path '{}': must be under /tmp/ or ~/.soulsystem/: {}",
                path, e
            )
        })?;
...
mod tests {
    ...
    async fn test_resolve_websocket_identify_message_uses_owner_scope() {
        ...
        let config = WebsocketRuntimeConfig {
            ...
            identify: Some(serde_json::json!({
                "intents": 513,
                "properties": { "os": "linux", "browser": "soulsystem", "device": "soulsystem" }
            })),
    ...
    fn test_status_to_wit_auth_required() {
        ...
        let wit = status_to_wit(
            &crate::channels::StatusUpdate::AuthRequired {
                extension_name: soulsystem_common::ExtensionName::new("weather").unwrap(),
    ...
    fn test_mime_from_extension() {
        ...
        assert_eq!(
            mime_from_extension("/home/user/.soulsystem/screenshot.png"),
            "image/png"
        );
    }
