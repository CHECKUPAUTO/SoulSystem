...
use soulsystem_skills::SkillRegistry;
...
pub struct AgentDeps {
    ...
    pub skill_catalog: Option<Arc<soulsystem_skills::catalog::SkillCatalog>>,
...
pub struct Agent {
    ...
    pub(crate) mission_manager_slot:
        Arc<tokio::sync::RwLock<Option<Arc<soulsystem_engine::MissionManager>>>>,
}
...
impl Agent {
    ...
    pub(crate) async fn mission_manager(&self) -> Option<Arc<soulsystem_engine::MissionManager>> {
    ...
    pub(crate) async fn platform_info(&self) -> soulsystem_engine::PlatformInfo {
        ...
        soulsystem_engine::PlatformInfo {
            ...
            repo_url: Some("https://github.com/nearai/soulsystem".to_string()),
        }
    }
    ...
    pub(super) fn skill_catalog(&self) -> Option<&Arc<soulsystem_skills::catalog::SkillCatalog>> {
    ...
    pub(super) async fn select_active_skills(
        ...
        message_content: &str,
        user_id: &str,
    ) -> (Vec<soulsystem_skills::LoadedSkill>, String, Vec<String>) {
        ...
        let (available, distinct_markers) = match registry.read() {
            Ok(guard) => {
                let skills_clone: Vec<soulsystem_skills::LoadedSkill> = guard.skills().to_vec();
        ...
        let (explicit, rewritten) =
            soulsystem_skills::extract_skill_mentions(message_content, &available);

        // Phase 2: Score-based selection on the rewritten message
        let skills_cfg = &self.deps.skills_config;
        let outcome = soulsystem_skills::prefilter_skills(
            &rewritten,
            &available,
        ...
        let mut selected: Vec<soulsystem_skills::LoadedSkill> =
