...
impl Agent {
    ...
    pub(super) async fn handle_system_command(
        ...
        match command {
            ...
            "restart" => {
                ...
                tracing::debug!("[commands::restart] SOULSYSTEM_IN_DOCKER={}", in_docker);
                ...
                if !in_docker {
                    ...
                    return Ok(SubmissionResult::error(
                        "Restart is not available in this environment. \
                         The SOULSYSTEM_IN_DOCKER environment variable must be set to 'true' for Docker deployments."
                            .to_string(),
                    ));
                }
    ...
    async fn persist_selected_model(&self, tenant: &crate::tenant::TenantCtx, model: &str) {
        ...
        if let Err(e) = tokio::task::spawn_blocking(move || {
            // 3a. Update the backend-specific model env var in ~/.soulsystem/.env
            ...
            let env_path = crate::bootstrap::soulsystem_env_path();
