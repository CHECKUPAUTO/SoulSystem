...
SoulSystem uses two sandbox layers for tool execution: WASM sandbox for tools, and Docker sandbox for jobs.
...
# Docker image
export SANDBOX_IMAGE=soulsystem-worker:latest
```
...
SoulSystem can run inside Docker and still sandbox jobs:
...
<Warning>
**SoulSystem runs alongside Docker** (for job sandboxing), not inside Docker by default.

- **Default**: SoulSystem binary → spawns containers for jobs
- **Optional**: SoulSystem inside container → still spawns sibling containers
</Warning>

See [Docker Install](/install/docker) for running SoulSystem itself in a container.
