...
SoulSystem uses a zero-exposure credential model: secrets are encrypted at rest and never exposed to untrusted code.
...
```bash
# List secrets
soulsystem secret list

# Get a secret (decrypted)
soulsystem secret get telegram_bot_token

# Set a secret
soulsystem secret set telegram_bot_token "your-token"

# Delete a secret
soulsystem secret delete telegram_bot_token
```
...
Common secret names used by SoulSystem:
...
<AccordionGroup>
  ...
  <Accordion title="Secret not found" icon="search">
    ...
    - Re-run `soulsystem onboard` to reconfigure
  </Accordion>
