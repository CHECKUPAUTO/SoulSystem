...
description: Default LLM provider for SoulSystem
---

NEAR AI is the default provider for SoulSystem, offering access to multiple models including Claude via a simple OAuth flow.
...
**How it works:**
1. `soulsystem onboard` opens your browser
2. Log in with GitHub or Google
3. Session token saved to `~/.soulsystem/session.json`
4. Automatic renewal
...
<Warning>
**VPS / Remote servers:** Browser OAuth won't work without a browser. Use API key mode or set `SOULSYSTEM_OAUTH_CALLBACK_URL` to a publicly reachable URL.
</Warning>
...
```bash
soulsystem config set nearai.session_token sess_xxxxx
```

Or edit `~/.soulsystem/session.json`:
