...
ClawHub is the official registry for SoulSystem skills. It lets you discover community-built skills, install them with a single command, and keep them up to date — without leaving the agent interface.
...
<Warning>
Skills from ClawHub are installed with **Installed** trust, not **Trusted** trust. They cannot execute shell commands, write files, or make HTTP requests. If you need elevated access, download the SKILL.md manually and place it in `~/.soulsystem/skills/` after reviewing the contents.
</Warning>
...
# Example output:
# kubernetes-deploy  (trusted)    ~/.soulsystem/skills/
# git-helper         (trusted)    <workspace>/skills/
# python-lint        (installed)  ~/.soulsystem/installed_skills/
```
...
Download and install a skill from ClawHub into `~/.soulsystem/installed_skills/`.
...
Remove an installed skill from `~/.soulsystem/installed_skills/`.

```
skill_remove python-lint
```

Trusted skills in `~/.soulsystem/skills/` must be removed manually by deleting the directory.
...
<Steps>
  ...
  <Step title="Install">
    Run `skill_install <slug>`. The agent fetches the SKILL.md from the registry and writes it to `~/.soulsystem/installed_skills/<slug>/SKILL.md`.
  </Step>
...
2. Test it locally by placing it in `~/.soulsystem/skills/`
