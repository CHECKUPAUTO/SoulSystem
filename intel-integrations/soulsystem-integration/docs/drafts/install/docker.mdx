...
sidebarTitle: Docker
description: Run SoulSystem in a Docker container
---

Run SoulSystem inside a Docker container. This provides isolation and consistency across environments.

<Warning>
**Important distinction:** SoulSystem runs **alongside** Docker (for job sandboxing), not inside Docker by default. This page covers running SoulSystem itself in a container — a different use case from the default installation.
</Warning>
...
- **Multi-tenant**: Run multiple SoulSystem instances on one host
...
docker run -d \
  --name soulsystem \
  -v ~/.soulsystem:/home/soulsystem/.soulsystem \
  # enabling this mount will allow the container to control the docker daemon, use at own risk
  # -v /var/run/docker.sock:/var/run/docker.sock \
  -p 3000:3000 \
  -p 8080:8080 \
  # Pin to a specific SoulSystem version for reproducible, rollback-friendly deployments.
  nearai/soulsystem:latest 
```
...
services:
  soulsystem:
    ...
    volumes:
      # Persistent data
      - ~/.soulsystem:/home/soulsystem/.soulsystem
...
|-----------|---------------|---------|
| `~/.soulsystem` | `/home/soulsystem/.soulsystem` | Config, database, logs |
...
SoulSystem can launch sandbox containers. In Docker, this requires:

1. **Docker socket mount** (shown above): Allows SoulSystem to launch sibling containers
...
```bash
mkdir -p /opt/soulsystem/data

docker run -d \
  --name soulsystem \
  -e SOULSYSTEM_BASE_DIR=/data \
  -v /opt/soulsystem/data:/data \
  ...
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission denied on data directory" icon="lock">
    Fix ownership:
    ```bash
    sudo chown -R 1000:1000 ~/.soulsystem
    ```
  </Accordion>
