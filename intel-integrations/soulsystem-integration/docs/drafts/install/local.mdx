...
sidebarTitle: Local
description: Install SoulSystem on Linux, macOS, or Windows
---

Install SoulSystem directly on your local machine for personal use. This is the simplest and most common installation method.
...
<Tabs>
  <Tab title="Linux">
    ...
    This installs to `~/.local/bin/soulsystem`. Add to your PATH if needed:
    ...
    echo "deb [signed-by=/usr/share/keyrings/soulsystem.gpg] https://repo.soulsystem.ai stable main" | sudo tee /etc/apt/sources.list.d/soulsystem.list

    # Install
    sudo apt update
    sudo apt install soulsystem
    ```
  ...
  <Tab title="macOS">
    ...
    Installs to `~/.local/bin/soulsystem`.
    ...
    2. **macOS Keychain** (Step 2): Two system dialogs are expected:
       ...
       - "Allow soulsystem to access this keychain item"
  ...
  <Tab title="Windows (WSL2)">
    ...
    <Note>
    WSL2 is the recommended way to run SoulSystem on Windows. Native Windows support is available but less tested.
    </Note>
  </Tab>
  ...
  <Tab title="Windows (Native)">
    ...
    ```powershell
    irm https://install.soulsystem.ai | iex
    ```

    Or manually:
    1. Download `soulsystem-x86_64-pc-windows-msvc.zip` from [releases](https://github.com/nearai/soulsystem/releases)
...
→ Select "libSQL"
→ Path: ~/.soulsystem/soulsystem.db (default)
```
...
<AccordionGroup>
  <Accordion title="macOS" icon="apple">
    ...
    2. "Allow soulsystem to access this keychain item"
...
Run SoulSystem as a background service:
...
<Tabs>
  <Tab title="Linux (systemd)">
    ...
    # View logs
    sudo journalctl -u soulsystem -f
    ```
  </Tab>
...
# Expected output:
# ✓ Binary: soulsystem v0.13.0
# ✓ Config directory: /home/user/.soulsystem
...
1. **Run the wizard**: `soulsystem onboard`
2. **Start SoulSystem**: `soulsystem run`
