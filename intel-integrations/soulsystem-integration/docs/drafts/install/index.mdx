...
description: Choose how to install SoulSystem
---

SoulSystem can run in multiple environments — from your local machine to a cloud VPS. Choose the installation method that fits your needs.

## Installation Methods

<CardGroup cols={2}>
  <Card title="Local Install" icon="monitor" href="/install/local">
    Run SoulSystem directly on your machine. Best for personal use.
  ...
  <Card title="Docker" icon="container" href="/install/docker">
    Run SoulSystem in a container. Good for consistent environments.
...
<AccordionGroup>
  ...
  <Accordion title="Optional: Docker" icon="box">
    Docker is **optional** for running SoulSystem itself, but required if you want to use:
...
- **Auto-created**: Database created automatically at `~/.soulsystem/soulsystem.db`
...
- [ ] Run the onboarding wizard: `soulsystem onboard`
- [ ] Start SoulSystem: `soulsystem run`
...
<Steps>
  <Step title="Install SoulSystem" icon="download">
  ...
  <Step title="Start Using SoulSystem" icon="message-square">
