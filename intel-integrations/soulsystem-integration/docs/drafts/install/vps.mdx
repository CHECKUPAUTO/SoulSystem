...
description: Deploy SoulSystem to a cloud server
---

Deploy SoulSystem to a remote VPS or cloud server for always-on operation.
...
CREATE USER soulsystem WITH PASSWORD 'your-secure-password';
CREATE DATABASE soulsystem OWNER soulsystem;
EOF
...
# Enable the vector extension in the soulsystem database
sudo -u postgres psql -d soulsystem -c "CREATE EXTENSION IF NOT EXISTS vector;"
```
...
2. **Custom callback URL**: Set `SOULSYSTEM_OAUTH_CALLBACK_URL` to a publicly reachable URL
...
1. **Database**: Select "PostgreSQL"
   - Enter connection string: `postgres://soulsystem:your-secure-password@localhost/soulsystem`
...
<Warning>
**Important:** SoulSystem's orchestrator binds to `0.0.0.0:50051` on Linux for container communication. This port should **not** be exposed externally. The firewall configuration below includes rules to block external access to this port—do not add any UFW allow rules for `50051`.
</Warning>
...
# Configure site
sudo tee /etc/nginx/sites-available/soulsystem <<'EOF'
server {
...
# Enable site
sudo ln -sf /etc/nginx/sites-available/soulsystem /etc/nginx/sites-enabled/
...
# Configure service
sudo systemctl edit soulsystem
```
...
Environment="DATABASE_URL=postgres://soulsystem:your-secure-password@localhost/soulsystem"
...
# Create filter
sudo tee /etc/fail2ban/filter.d/soulsystem.conf <<'EOF'
[Definition]
...
# Create jail
sudo tee /etc/fail2ban/jail.d/soulsystem.conf <<EOF
[soulsystem]
enabled = true
port = http,https
filter = soulsystem
logpath = /var/log/soulsystem/web.log
maxretry = 5
bantime = 3600
EOF
