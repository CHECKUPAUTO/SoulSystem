...
SoulSystem uses the [tracing](https://docs.rs/tracing) crate for structured logging. Output goes to stdout/stderr and can be streamed in real time via the Web Gateway SSE endpoint.
...
|---------------|----------|
| `soulsystem=error` | Production: errors only |
| `soulsystem=warn` | Production: errors and warnings |
| `soulsystem=info` | Production: normal operational events (recommended) |
| `soulsystem=debug` | Troubleshooting: detailed request/response flow |
| `soulsystem=trace` | Deep debugging: all internal events including LLM token streams |
| `soulsystem=info,tower_http=warn` | Reduce HTTP access log noise |
| `soulsystem=debug,tower_http=debug` | Debug with HTTP request details |

Set in your environment file or shell:

```bash
# In .env or /etc/soulsystem/soulsystem.env
RUST_LOG=soulsystem=info,tower_http=warn
...
RUST_LOG=soulsystem::agent=debug,soulsystem::agent::scheduler=trace

# LLM provider calls only
RUST_LOG=soulsystem::llm=debug

# Safety layer only
RUST_LOG=soulsystem::safety=debug

# Skills system only
RUST_LOG=soulsystem::skills=debug

# Sandbox and proxy
RUST_LOG=soulsystem::sandbox=debug

# Database queries
RUST_LOG=soulsystem::db=trace

# Everything + HTTP access log
RUST_LOG=soulsystem=debug,tower_http=debug

# Silence noisy dependencies
RUST_LOG=soulsystem=info,hyper=warn,reqwest=warn,tower_http=warn
```
...
SoulSystem emits structured logs in the following format:

```
2024-01-15T10:30:00.123456Z  INFO soulsystem::agent::worker: job started job_id="job_01j9abc" intent="summarize recent activity" session="default"
2024-01-15T10:30:00.456789Z DEBUG soulsystem::llm::nearai_chat: sending request model="claude-3-5-sonnet-20241022" tokens=1024
2024-01-15T10:30:02.891234Z  INFO soulsystem::agent::worker: tool call tool="memory_search" job_id="job_01j9abc"
2024-01-15T10:30:03.234567Z  INFO soulsystem::agent::worker: job completed job_id="job_01j9abc" duration_ms=3111 tokens_used=1847
```
...
| `module` | Rust module path (e.g., `soulsystem::agent::worker`) |
...
# Last 200 lines
journalctl -u soulsystem -n 200

# Since a specific time
journalctl -u soulsystem --since "2024-01-15 10:00:00"

# Last hour only
journalctl -u soulsystem --since "1 hour ago"

# Last hour, error level and above
journalctl -u soulsystem --since "1 hour ago" -p err

# Filter by a specific string (grep equivalent)
journalctl -u soulsystem -g "job_01j9abc"

# Export to JSON for analysis
journalctl -u soulsystem --since today -o json > soulsystem-today.json
...
SoulSystem records every LLM API call in the `llm_calls` database table. Each record includes:
...
sqlite3 ~/.soulsystem/soulsystem.db
...
```
event: log
data: {"level":"INFO","message":"job started","module":"soulsystem::agent::worker","job_id":"job_01j9abc","ts":"2024-01-15T10:30:00Z"}
...
Some dependencies are verbose at the default log level. Silence them while keeping SoulSystem output at debug:

```bash
# Quiet HTTP infrastructure
RUST_LOG=soulsystem=debug,hyper=warn,reqwest=warn,tower_http=warn,h2=warn

# Quiet database layer
RUST_LOG=soulsystem=info,soulsystem::db=warn

# Maximum quiet (errors only everywhere)
RUST_LOG=error

# Recommended production setting
RUST_LOG=soulsystem=info,tower_http=warn,hyper=warn
```
...
RUST_LOG=soulsystem=debug,soulsystem::sandbox=off

# Suppress all LLM call details
RUST_LOG=soulsystem=debug,soulsystem::llm=info

# Suppress routine engine tick logs
RUST_LOG=soulsystem=info,soulsystem::agent::routine_engine=warn
```
