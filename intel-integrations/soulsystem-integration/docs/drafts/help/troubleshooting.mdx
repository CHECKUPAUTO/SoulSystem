...
Solutions for common SoulSystem issues.
...
<AccordionGroup>
  ...
  <Accordion title="Permission denied" icon="lock">
    **Solution:**
    ...
    sudo mv ~/.local/bin/soulsystem /usr/local/bin/
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Database connection failed" icon="database">
    **PostgreSQL:**
    ...
    # Verify connection
    psql postgres://user:pass@localhost/soulsystem
    ```
...
<AccordionGroup>
  <Accordion title="Browser didn't open for OAuth" icon="globe">
    **Solutions:**
    ...
    export SOULSYSTEM_OAUTH_CALLBACK_URL=https://your-server:9876
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Web Gateway token not found" icon="key">
    **Solution:**
    ```bash
    # View logs
    RUST_LOG=soulsystem=info soulsystem run 2>&1 | grep "Gateway auth token"
  ...
  <Accordion title="Owner binding timeout" icon="clock">
    **Solutions:**
    ...
    2. Re-run `soulsystem onboard --channels-only`
...
```bash
# All modules
RUST_LOG=debug soulsystem run

# Just SoulSystem
RUST_LOG=soulsystem=debug soulsystem run

# Specific module
RUST_LOG=soulsystem::agent=debug soulsystem run

# With HTTP requests
RUST_LOG=soulsystem=debug,tower_http=debug soulsystem run
```
...
1. Run `soulsystem doctor --json`
2. Check logs with `RUST_LOG=debug`
3. Search [GitHub Issues](https://github.com/soulsystem-ai/soulsystem/issues)
