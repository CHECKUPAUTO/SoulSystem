...
Common questions about SoulSystem.

## General

<AccordionGroup>
  <Accordion title="What is SoulSystem?" icon="help-circle">
    SoulSystem is a secure, self-hosted AI assistant that runs on your own hardware. It provides a personal AI with strong privacy guarantees through multi-layer security.
  </Accordion>

  <Accordion title="Do I need Docker?" icon="container">
    **No for basic use, yes for job sandboxing.**

    SoulSystem runs as a standalone binary. Docker is only required if you want to use:
    - Sandboxed job execution (Claude Code mode)
    - WASM tool building with automatic compilation

    Without Docker, SoulSystem still works fully — just without the Docker sandbox layer.
  </Accordion>

  <Accordion title="Can I run without PostgreSQL?" icon="database">
    **Yes.** SoulSystem includes libSQL (embedded SQLite) as an alternative.
  ...
  <Accordion title="How is SoulSystem different from agent?" icon="git-compare">
    | Feature | SoulSystem | Agent |
    |---------|----------|----------|
    | Deployment | Local, self-hosted | Cloud service |
    | Data | Your hardware, your control | agent infrastructure |
    | Language | Rust | Varies |
    | Security | Multi-layer, local-first | Cloud security |

    SoulSystem is the self-hosted version for users who want complete control over their data.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Where is my data stored?" icon="database">
    By default: `~/.soulsystem/`

    ```
    ~/.soulsystem/
    ├── soulsystem.db          # libSQL database
    ...
    Change with `SOULSYSTEM_BASE_DIR`.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Why is the wizard asking for a browser?" icon="globe">
    ...
    - Or set `SOULSYSTEM_OAUTH_CALLBACK_URL` to a public URL
  </Accordion>

  <Accordion title="How do I run SoulSystem as a service?" icon="server">
    ```bash
    # Install service
    soulsystem service install

    # Start
    sudo systemctl enable --now soulsystem  # Linux
    brew services start soulsystem            # macOS
    ```
  </Accordion>

  <Accordion title="Can I use SoulSystem behind a reverse proxy?" icon="network">
...
<AccordionGroup>
  <Accordion title="How secure is SoulSystem?" icon="shield">
    SoulSystem uses defense in depth:
  ...
  <Accordion title="Can SoulSystem be hacked?" icon="alert-triangle">
    No software is unhackable. SoulSystem reduces risk through:
...
<AccordionGroup>
  <Accordion title="Can I run SoulSystem on a Raspberry Pi?" icon="cpu">
    **Yes**, with caveats:
  ...
  <Accordion title="Can I contribute?" icon="github">
    Yes! SoulSystem is open source:

    - GitHub: https://github.com/soulsystem-ai/soulsystem
...
- [GitHub Issues](https://github.com/soulsystem-ai/soulsystem/issues) — Bug reports
