...
description: Complete environment variable reference for SoulSystem
---

SoulSystem is configured primarily through environment variables. This page documents all available configuration options.

## Two-Layer Configuration

SoulSystem uses a two-layer configuration system:

<AccordionGroup>
  <Accordion title="Layer 1: ~/.soulsystem/.env (Bootstrap)" icon="file">
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Managed through the wizard or `soulsystem config` command.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Security" icon="shield" href="#security">
    SECRETS_MASTER_KEY, SOULSYSTEM_BASE_DIR
  </Card>
</CardGroup>
...
|----------|------|---------|-------------|
| `AGENT_NAME` | string | `soulsystem` | Agent name displayed in responses |
...
| `LIBSQL_PATH` | path | `~/.soulsystem/soulsystem.db` | libSQL database file |
...
| `WASM_CHANNELS_DIR` | path | `~/.soulsystem/channels` | Channel modules directory |
...
| `SANDBOX_IMAGE` | string | `soulsystem-worker:latest` | Docker image |
...
| `SOULSYSTEM_BASE_DIR` | path | `~/.soulsystem` | Data directory |
| `SOULSYSTEM_OAUTH_CALLBACK_URL` | URL | `http://127.0.0.1:9876` | OAuth callback URL |

## Environment File Example

Create `~/.soulsystem/.env`:
...
# Set value
soulsystem config set llm.backend nearai

# Delete value (reset to default)
soulsystem config delete llm.backend
```
