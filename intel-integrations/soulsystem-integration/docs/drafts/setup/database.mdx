...
SoulSystem supports two database backends: **PostgreSQL** and **libSQL** (embedded SQLite). Choose based on your deployment needs.
...
```bash
# Create database
sudo -u postgres psql -c "CREATE DATABASE soulsystem;"
sudo -u postgres psql -c "CREATE USER soulsystem WITH PASSWORD 'your-password';"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE soulsystem TO soulsystem;"
...
export DATABASE_URL="postgres://soulsystem:your-password@localhost/soulsystem"
```
...
- Database is a single file (`~/.soulsystem/soulsystem.db`)
...
export LIBSQL_PATH="/home/user/.soulsystem/soulsystem.db"
```
...
1. **Export from libSQL:**
   ```bash
   sqlite3 ~/.soulsystem/soulsystem.db ".dump" > soulsystem.sql
   ```

2. **Import to PostgreSQL:**
   ```bash
   psql -d soulsystem -f soulsystem.sql
   ```

3. **Update SoulSystem config:**
   ```bash
   export DATABASE_BACKEND=postgres
   export DATABASE_URL="postgres://user:pass@localhost/soulsystem"
   ```

4. **Restart SoulSystem**

### From PostgreSQL to libSQL

1. **Export:**
   ```bash
   pg_dump -h localhost -U soulsystem soulsystem > soulsystem.sql
   ```
...
- Running SoulSystem on a personal laptop/desktop
...
```bash
# Backup
pg_dump -h localhost -U soulsystem soulsystem > backup.sql

# Restore
psql -d soulsystem -f backup.sql
```
...
cp ~/.soulsystem/soulsystem.db ~/.soulsystem/soulsystem.db.backup

# Restore
cp ~/.soulsystem/soulsystem.db.backup ~/.soulsystem/soulsystem.db
