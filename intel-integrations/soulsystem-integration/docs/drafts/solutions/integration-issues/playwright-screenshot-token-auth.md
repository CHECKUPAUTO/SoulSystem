...
description: "Building a UI screenshot capture pipeline that auto-authenticates with SoulSystem web gateway"
...
Building an automated screenshot documentation pipeline for SoulSystem's web gateway UI that:
...
1. **Token URL Construction**: The `getSoulSystemUrlWithToken()` function was creating malformed URLs like `/?token=TOKEN/skills` when appending paths
2. **Client-Side Routing**: SoulSystem's web gateway uses client-side routing; only `/` is served by the backend
...
### 4. Auto-Detection of SoulSystem Port
...
| `docs/tests/fixtures/seed.ts` | Fixed `getSoulSystemUrlWithToken()` with optional path param |
...
# Run full pipeline
cd /home/opselite/ai_projects/soulsystem-src && bash docs/scripts/capture-screenshots.sh
```

## References

- SoulSystem web gateway auth: `src/channels/web/static/app.js` lines 96-114
