---
title: "Playwright Screenshot Pipeline for SoulSystem Web UI"
...
root_causes:
  ...
  - SoulSystem web UI uses client-side routing (SPA)
...
Build a screenshot capture pipeline for SoulSystem documentation that:
1. Auto-detects running SoulSystem instances
...
SOULSYSTEM_TOKEN="..." pnpm exec playwright test
```
...
**Solution:** Modify `getSoulSystemUrlWithToken()` to accept optional path parameter
...
await page.goto(await getSoulSystemUrlWithToken('/'));
...
SOULSYSTEM_TOKEN=your-token-here
SOULSYSTEM_URL=http://127.0.0.1:3000
```

### 2. Token Helper Function (docs/tests/fixtures/seed.ts)

```typescript
export async function getSoulSystemUrlWithToken(path?: string): Promise<string> {
  const baseUrl = await getBaseUrl();
  const token = process.env.SOULSYSTEM_TOKEN ?? 'screenshot-test-token';
...
test('routines tab overview', async ({ page }) => {
  // Check if SoulSystem is running
  const ready = await isSoulSystemReady();
  if (!ready) {
    test.skip(true, 'SoulSystem not running');
    return;
  }

  // Navigate to root with token
  const url = await getSoulSystemUrlWithToken('/');
  await page.goto(url);
...
if [ -f "$DOCS_DIR/.env.screenshot" ]; then
  ...
  export SOULSYSTEM_URL
  export SOULSYSTEM_TOKEN
  export SCREENSHOT_VIEWPORT
  export HEALTH_TIMEOUT
fi

# Auto-detect SoulSystem port
find_soulsystem_http_port() {
...
### SoulSystem Web UI Authentication Flow

The SoulSystem web UI (`src/channels/web/static/app.js`) has an `autoAuth()` function that:
...
- SoulSystem web UI source: `src/channels/web/static/app.js` (autoAuth function)
