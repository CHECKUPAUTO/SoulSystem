...
This document describes the SoulSystem Engine v2 architecture for new contributors. It covers the execution model, the Python orchestrator, the bridge layer, and how everything fits together.

## Overview

SoulSystem Engine v2 replaces ~10 fragmented abstractions (Session, Job, Routine, Channel, Tool, Skill, Hook, Observer, Extension, LoopDelegate) with a unified model built on 5 primitives. The engine lives in `crates/soulsystem_engine/` as a standalone crate with no dependency on the main `soulsystem` crate.
...
The bridge connects the engine to existing SoulSystem infrastructure:
...
Set `ENGINE_V2=true` in the environment. Without it, SoulSystem keeps using the legacy v1 agent loop. The router in `src/bridge/router.rs` intercepts messages and routes them through the engine instead of the v1 agent loop.

```bash
# Installed binary
ENGINE_V2=true soulsystem

# Run from source
ENGINE_V2=true cargo run
```

For trace debugging set `SOULSYSTEM_RECORD_TRACE=1`. Engine v2 reuses the host crate's `RecordingLlm` (see `src/llm/recording.rs`) — the engine's `LlmBackend` is wired to the same provider chain, so LLM interactions are captured in the standard `trace_*.json` fixture file (configurable via `SOULSYSTEM_TRACE_OUTPUT`). There is no separate engine trace file.
...
Skills live in the `soulsystem_skills` crate (extracted from `src/skills/`), shared by both v1 and v2 engines. The engine crate depends on `soulsystem_skills` with `default-features = false` (no catalog/registry — just types + selection).

```
soulsystem_skills crate (shared)
...
Pica's model is optimized for programmatic API access (SDK calls from code). For LLM agents, the skill-as-knowledge approach is superior because it avoids tool list bloat while leveraging the LLM's ability to construct HTTP calls from documentation. The two approaches share the insight that **integrations should be data, not code**. SoulSystem extends this further: the skill-extraction mission can learn new skills from successful thread executions, making the integration library self-expanding.

## Key Files

| File | Purpose |
|------|---------|
| `crates/soulsystem_engine/orchestrator/default.py` | The Python execution loop (v0) |
| `crates/soulsystem_engine/src/executor/orchestrator.rs` | Host functions + versioning + loading |
| `crates/soulsystem_engine/src/executor/loop_engine.rs` | Bootstrap (loads + runs orchestrator, skill injection) |
| `crates/soulsystem_engine/src/executor/scripting.rs` | Monty VM integration, user code execution, CodeAct skill snippets |
| `crates/soulsystem_engine/src/executor/prompt.rs` | System prompt construction, skill section formatting |
| `crates/soulsystem_engine/src/runtime/manager.rs` | ThreadManager (spawn, stop, join, skill selector wiring) |
| `crates/soulsystem_engine/src/runtime/mission.rs` | MissionManager (lifecycle, firing, learning missions) |
| `crates/soulsystem_engine/src/capability/skill_selector.rs` | MemoryDoc → LoadedSkill bridge, deterministic selection |
| `crates/soulsystem_engine/src/capability/skill_tracker.rs` | Confidence tracking, versioned updates, rollback |
| `crates/soulsystem_engine/src/types/` | All core data structures |
| `crates/soulsystem_engine/src/traits/` | LlmBackend, Store, EffectExecutor |
| `crates/soulsystem_skills/` | Shared skills crate (types, selector, parser, validation) |
...
```bash
cargo check -p soulsystem_skills                                    # skills crate compiles
cargo test -p soulsystem_skills                                     # 94 tests (types, selector, parser, gating, registry, catalog)
cargo check -p soulsystem_engine                                    # engine crate compiles
cargo test -p soulsystem_engine                                     # 203 tests (execution, missions, skills, tracking)
