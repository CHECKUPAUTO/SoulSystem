...
This document describes how SoulSystem improves itself at runtime — fixing bugs, evolving prompts, and patching its own execution loop without a Rust rebuild.
...
read_file("crates/soulsystem_engine/src/executor/scripting.rs")
apply_patch(...)
result = shell("cargo test -p soulsystem_engine")
...
- Files outside `crates/soulsystem_engine/` and `src/bridge/` without human approval
...
| File | Purpose |
|------|---------|
| `crates/soulsystem_engine/orchestrator/default.py` | The v0 orchestrator (self-modifiable) |
| `crates/soulsystem_engine/src/executor/orchestrator.rs` | Loading, versioning, rollback, host functions |
| `crates/soulsystem_engine/src/executor/prompt.rs` | Prompt overlay loading |
| `crates/soulsystem_engine/src/runtime/mission.rs` | Self-improvement mission, OnSystemEvent wiring, fix patterns |
...
`SOULSYSTEM_RECORD_TRACE=1` is the unified flag — it enables `RecordingLlm`,
...
```bash
ENGINE_V2=true SOULSYSTEM_RECORD_TRACE=1 RUST_LOG=soulsystem_engine=debug cargo run
```
