...
Built the core engine crate (`crates/soulsystem_engine/`) from scratch in 6 phases:
...
**Key design decision**: The engine has zero dependency on the main soulsystem crate. All interaction goes through three traits (LlmBackend, Store, EffectExecutor) implemented by bridge adapters.
...
Studied [Pica](https://github.com/withoneai/pica) (formerly IntegrationOS, 200+ third-party API integrations) to understand how to rapidly scale the number of available integrations in SoulSystem.
...
**Implementation** (6 files created/modified in `soulsystem_skills`, 4 in main crate):

1. **Credential types in skill frontmatter** — `SkillCredentialSpec`, `SkillCredentialLocation`, `SkillOAuthConfig`, `ProviderRefreshStrategy` in `crates/soulsystem_skills/src/types.rs`. Skills declare credentials in YAML; values never in LLM context.
...
**The problem**: The agent had no knowledge of its own identity. It didn't know it was SoulSystem, its GitHub repo, its version, active channels, LLM backend, or database. The system prompt just said "You are SoulSystem Agent, a secure autonomous assistant" with no specifics.
...
Files: `src/agent/submission.rs` (parser), `src/agent/commands.rs` (handler), `src/agent/agent_loop.rs` (mission_manager slot + dispatch), `src/bridge/router.rs` (wiring), `crates/soulsystem_engine/src/runtime/mission.rs` (4th learning mission), `crates/soulsystem_engine/prompts/mission_expected_behavior.md` (goal prompt).
...
- **Glob re-exports removed** — `pub use soulsystem_safety::*` and `pub use soulsystem_skills::*` deleted; ~35 files migrated to direct imports.
...
- **Mission prompt templates extracted** to `crates/soulsystem_engine/prompts/mission_*.md` from inline Rust strings.
...
Built a v2-engine-specific E2E test framework: mock API servers (aiohttp), mock LLM tool call patterns, dedicated soulsystem server fixtures with `ENGINE_V2=true`, `HTTP_ALLOW_LOCALHOST=true`, `SECRETS_MASTER_KEY`.
...
- Engine crate (`crates/soulsystem_engine/`) — `NeedAuthentication` already existed
...
4. **No tool restriction in plan mode** — Unlike Codex which prompts the LLM not to mutate, SoulSystem's plan-mode skill naturally guides the LLM to only use read/search/write tools during planning. During execution (mission thread), all tools are available per capability leases.

### Files Changed

| File | Role |
|------|------|
| `crates/soulsystem_engine/src/types/memory.rs` | `DocType::Plan` variant |
| `crates/soulsystem_common/src/event.rs` | `PlanStepDto`, `AppEvent::PlanUpdate`, tests |
