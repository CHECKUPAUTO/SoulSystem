...
2. 为项目命名（例如 `soulsystem-calendar`），然后点击 **Create**
...
3. 为客户端命名（例如 `soulsystem-calendar`）
...
向 SoulSystem 提供您的 OAuth 凭证：

```bash
soulsystem tool auth google-calendar
```

按照提示粘贴 `credentials.json` 文件内容，或者提供该文件的路径。SoulSystem 会为您打开一个浏览器窗口来授权访问日历。授权完成后，token 会被安全存储。

<Info>
授权流程只需要运行一次。之后 SoulSystem 会在需要时自动刷新访问 token。
</Info>
