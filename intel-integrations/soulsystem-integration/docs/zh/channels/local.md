---
title: "本地"
description: "通过终端或浏览器在本地使用 SoulSystem"
icon: keyboard
---

默认情况下，SoulSystem 提供两种本地界面与智能体对话：
...
只需运行 `soulsystem`，TUI 将在终端中启动。使用以下快捷键进行导航和对话。
| 按键 | 操作 |
|-----|--------|
| `Enter` | 发送消息 |
| `Shift+Enter` | 在编辑器中换行 |
...
默认情况下，SoulSystem 在启动时生成认证令牌并在日志中打印。要在重启间使用固定令牌：
