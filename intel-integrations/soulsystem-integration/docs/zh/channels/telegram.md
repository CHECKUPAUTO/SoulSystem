...
您可以创建 Telegram 机器人并将 SoulSystem 智能体连接到它。配置完成后，您可以在私信中与智能体对话，也可以将其添加到群聊中参与讨论。
...
<Steps>
    ...
    <Step title="获取机器人令牌">
    创建机器人后，BotFather 会给您一个类似这样的令牌：`123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ`。此令牌用于认证您的机器人并允许其访问 Telegram API。请妥善保管此令牌，不要与任何人分享。稍后您将需要它来在 SoulSystem 中配置 Telegram 频道。
    </Step>
</Steps>

</Step>
<Step title="在 SoulSystem 中配置 Telegram 频道">

    使用 `--channels-only` 标志调用 SoulSystem CLI 引导向导，仅配置频道而无需再次执行整个引导过程：
...
<Step title="测试 Telegram 频道">
    配置完 Telegram 频道后，是时候测试一下了。如果智能体尚未运行，请先启动 `soulsystem`：
    ...
    ```
    soulsystem pairing approve telegram <PAIRING_CODE>
    ```
</Step>
...
您可以通过 `.soulsystem/channels/telegram.capabilities.json` 文件配置 Telegram 频道的行为，该文件在首次设置频道后自动创建。
...
SoulSystem 可以配置为参与 Telegram 群聊。默认情况下，机器人只回复命令（使用 `/help` 查看可用命令列表）。如果您希望机器人回复提及或所有群组消息，需要进行配置。
...
3. **在 SoulSystem 中配置 `bot_username`**：
   ```json
   {
     "bot_username": "MySoulSystemBot"
   }
   ```
...
当使用命令（如 `/skills`）或提及机器人（如 `@MySoulSystemBot 天气怎么样？`）时，机器人会响应。
...
当 SoulSystem 在 webhook 模式下运行时，Telegram 通过向您的公共 URL 发送 HTTP 请求来传递消息。由于该 URL 可从互联网访问，任何第三方都可以向其发送伪造请求。

Webhook 密钥是您在 SoulSystem 中配置的共享令牌。Telegram 在每个请求中包含该令牌。SoulSystem 拒绝不携带正确令牌的任何请求，因此只有真正的 Telegram 流量才能到达您的智能体。

要启用此功能，在 `.soulsystem/channels/telegram.capabilities.json` 中添加 `telegram_webhook_secret`：
...
<AccordionGroup>
  ...
  <Accordion title="设置时所有者绑定超时">
    向导等待 120 秒接收第一条消息。如果超时，请在 Telegram 中向您的机器人发送 `/start`，然后重新运行 `soulsystem onboard --channels-only`。
  </Accordion>
</AccordionGroup>
