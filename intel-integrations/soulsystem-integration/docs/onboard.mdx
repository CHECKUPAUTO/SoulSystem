...
<Danger>
If you are new to SoulSystem, we recommend you to configure [channels](/channels/telegram), tools, and other settings one at a time instead of 
...
If you are new to SoulSystem, we recommend you to configure channels, tools, and other settings one at a time instead of all at once through the onboard command.
...
The wizard will first ask you to select a path for the agent's database, by default `/home/agent/.soulsystem/soulsystem.db`. This is where the agent will store your configuration.
...
SoulSystem can execute code, run builds, and use tools inside Docker
...
If you are running SoulSystem in an environment without Docker (like a server or a container), you can disable sandboxing
</Info>
