---
title: "Local"
description: "Use SoulSystem locally via terminal or browser"
---

By default, SoulSystem provides two local interfaces for chatting with your agent:
...
Simply run `soulsystem` and the TUI will launch in your terminal. Use the keyboard shortcuts below to navigate and chat with your agent.
| Key | Action |
|-----|--------|
| `Enter` | Send message |
...
By default, SoulSystem generates an auth token at startup and prints it in logs. To use a stable token across restarts:
...
<AccordionGroup>
  ...
  <Accordion title="Web UI connection refused">
    - Verify `soulsystem run` is active
