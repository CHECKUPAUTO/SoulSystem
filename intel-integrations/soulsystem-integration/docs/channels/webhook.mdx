...
The HTTP Webhook channel provides a REST API for integrating external services with SoulSystem.
...
Configure GitHub to send events to your SoulSystem webhook URL:
...
Use Zapier's Webhook action to send events to SoulSystem.
...
<AccordionGroup>
  ...
  <Accordion title="Messages not processed" icon="message-circle">
    - Check logs: `RUST_LOG=soulsystem=debug soulsystem run`
    - Verify JSON format
