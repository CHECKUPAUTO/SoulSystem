...
description: "Build a WASM messaging channel that plugs into SoulSystem"
---
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for SoulSystem"
...
mkdir -p ~/.soulsystem/channels
cp my-channel.wasm ~/.soulsystem/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.soulsystem/channels/
```
...
mkdir -p ~/.soulsystem/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.soulsystem/channels/
```

<Danger>
If you are contributing a channel to the public repository, **do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. SoulSystem builds channels from source.
</Danger>
