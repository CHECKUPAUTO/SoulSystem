---
title: "OAuth Setup"
description: "One-time setup for any Google extension in SoulSystem"
---
...
2. Give it a name (e.g. `soulsystem`) and click **Create**
...
3. Give it a name (e.g. `soulsystem`)
...
To complete the OAuth flow, we need to allow Google to reach the SoulSystem server. Since port 9876 is only accessible from within the server, you need to open an SSH tunnel that forwards your local port 9876 to the server.

Open a new SSH session using port forwarding:

```bash
# ssh -p <SSH-PORT> -L 9876:127.0.0.1:9876 <user>@<soulsystem-server-ip>
ssh -p 15222 -L 9876:127.0.0.1:9876 REDACTED@near.ai
```
