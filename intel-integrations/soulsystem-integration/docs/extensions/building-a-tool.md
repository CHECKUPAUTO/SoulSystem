...
<Card title="weather-tool source" icon="github" href="https://github.com/matiasbenary/soulsystem/tree/tools/weather/tools-src/weather">
...
version = "0.1.0"
edition = "2021"
description = "Weather information tool for SoulSystem (WASM component)"
...
Every SoulSystem tool is a WASM component that implements a WIT interface. The host provides HTTP, logging, and workspace capabilities; your tool exports `execute`, `schema`, and `description`.
...
<Note>
The `wit/tool.wit` file ships with SoulSystem. If you are building inside the SoulSystem repo (e.g. under `tools-src/my-tool/`), the path `../../wit/tool.wit` is correct. If you are building in a standalone directory, copy `wit/tool.wit` from the repo root and adjust the path accordingly.
</Note>
...
if !near::agent::host::secret_exists("example_api_token") {
        return Err("Missing secret: example_api_token. Run: soulsystem tool auth <tool-name>".into());
}
```
...
`near::agent::host::log` emits a structured log line visible in `soulsystem` output. The host collects all log entries and flushes them after the call completes.
...
fn api_get(url: &str) -> Result<String, String> {
    let headers = serde_json::json!({
        ...
        "User-Agent": "SoulSystem-Weather-Tool/0.1"
    }).to_string();
...
This weather tool uses Open-Meteo, so it does not need a secret. If your tool calls an API that needs a token, declare that in the capabilities file so SoulSystem can inject it at request time.

Example capability sections (pattern used in `tools-src/*` on the SoulSystem repo):
...
- `auth` tells SoulSystem how to collect credentials.

After installing the tool, run auth once:

```bash
soulsystem tool auth <tool-name>
```
...
```bash
soulsystem tool setup <tool-name>
```
...
```bash
soulsystem tool install ./target/wasm32-wasip2/release/weather_tool.wasm \
...
Start SoulSystem and ask your agent:
