...
**Goal:** Build a Python + Playwright E2E testing framework that exercises the SoulSystem web gateway through a real browser against the real binary with a mock LLM backend.

**Architecture:** pytest session fixtures start a mock OpenAI-compat HTTP server and the soulsystem binary (libSQL in-memory, gateway enabled), then per-test Playwright browser instances navigate to the gateway and make DOM assertions.
...
- Support `stream: true` with proper SSE chunk format (critical for SoulSystem's streaming)
...
- SoulSystem logs `Web UI: http://{host}:{port}/` to stdout (main.rs:508) using the config port, not the bound port. So we must use a fixed port, not port 0.
...
Session-scoped: build binary, start mock LLM, start soulsystem.
...
def soulsystem_binary():
    """Ensure soulsystem binary is built. Returns the binary path."""
    binary = ROOT / "target" / "debug" / "soulsystem"
    if not binary.exists():
        print("Building soulsystem (this may take a while)...")
        subprocess.run(
...
@pytest.fixture(scope="session")
async def soulsystem_server(soulsystem_binary, mock_llm_server):
    """Start the soulsystem gateway. Yields the base URL."""
    env = {
        **os.environ,
...
@pytest.fixture
async def page(soulsystem_server):
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(f"{soulsystem_server}/?token={AUTH_TOKEN}")
...
git commit -m "feat: E2E conftest with session fixtures for mock LLM and soulsystem"
```
...
```markdown
# SoulSystem E2E Tests

Browser-level end-to-end tests for the SoulSystem web gateway using Python + Playwright.

## Prerequisites

- Python 3.11+
- Rust toolchain (for building soulsystem)
...
The tests need the soulsystem binary built with libsql support:
...
2. **SoulSystem** -- the real binary with gateway enabled, pointing to the mock LLM
