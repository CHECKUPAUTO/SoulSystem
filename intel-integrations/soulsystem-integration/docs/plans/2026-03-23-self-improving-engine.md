...
**What it changes:** `crates/soulsystem_engine/prompts/*.md` files
...
**Validation:** Run existing test suite (`cargo test -p soulsystem_engine`). Only apply if tests pass.

### Level 3: Code Patching (high risk, high value)

The engine proposes Rust code changes to fix bugs it detects in itself.

**What it changes:** Any file in `crates/soulsystem_engine/` or `src/bridge/`
...
2. Run full test suite (`cargo test -p soulsystem_engine`)
3. Run clippy (`cargo clippy -p soulsystem_engine --all-targets -- -D warnings`)
4. Never modify files outside `crates/soulsystem_engine/` and `src/bridge/` without human approval
...
```
You are a debugging agent analyzing execution traces from the SoulSystem engine.
...
- Prompt templates: crates/soulsystem_engine/prompts/*.md
- Bridge adapters: src/bridge/*.rs
- Engine code: crates/soulsystem_engine/src/**/*.rs
...
- Always run `cargo test -p soulsystem_engine` after making changes
...
4. Runs `cargo test -p soulsystem_engine` and `cargo clippy`
