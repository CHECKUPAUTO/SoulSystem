# SoulSystem Engine v2: Unified Thread-Capability-CodeAct Architecture

**Date:** 2026-03-20
**Updated:** 2026-04-01
**Status:** In Progress (Phases 1-6 complete, engine running end-to-end)
**Goal:** Replace SoulSystem's ~10 fragmented abstractions with a unified execution model built on 5 primitives: Thread, Step, Capability, MemoryDoc, Project. Developed as a standalone crate (`soulsystem_engine`) that can be swapped in when it passes all acceptance tests.

---

## Motivation

SoulSystem currently has Session, Job, Routine, Channel, Tool, Skill, Hook, Observer, Extension, and LoopDelegate as separate abstractions. All share common patterns (lifecycle, messaging, state, capabilities) but are implemented independently. This causes:
...
Single crate: `crates/soulsystem_engine/`

```
crates/soulsystem_engine/
  Cargo.toml
  CLAUDE.md
  src/
...
**Goal:** Bridge adapters connect the engine to existing SoulSystem infrastructure. Strategy C: parallel deployment via `ENGINE_V2=true` env var.
...
- Depends on `soulsystem_common` crate with `AppEvent` type (PR #1615, merged into branch)
...
- `soulsystem_types` — shared types for WASM extensions
- `soulsystem_capability` — if used by tooling/CLI independently

---

## Phase 8: Sandboxed Execution + Infrastructure Integration

**Goal:** Leverage existing SoulSystem infrastructure for sandboxed execution. This is NOT about running CodeAct/RLM in different runtimes — Monty is the sole Python executor. This is about isolating threads and running third-party tools safely.
...
```bash
# Engine crate only:
cargo check -p soulsystem_engine
cargo clippy -p soulsystem_engine --all-targets -- -D warnings
cargo test -p soulsystem_engine
