# Automated QA Plan for SoulSystem
...
A review of all closed issues and merged bug-fix PRs reveals that most SoulSystem bugs fall into a few recurring categories:
...
Add the same validation for WASM tools (loaded from `~/.soulsystem/tools/`) and MCP tools (mock a simple MCP manifest and validate the schema it produces).
...
docker-build:
  ...
  steps:
    ...
    - name: Build Docker image
      run: docker build -t soulsystem-test:ci .
```
...
**Cost:** High (requires Anthropic computer use API, headless browser, soulsystem running)
...
```
+------------------+     +-----------------+     +------------------+
|  Test Runner     |     |  Headless       |     |  SoulSystem        |
...
1. **Test runner** -- Python or TypeScript script that orchestrates the flow. Starts soulsystem, waits for readiness, launches Playwright browser, runs scenarios.
...
tests/
  e2e/
    conftest.py             # pytest fixtures: start soulsystem, browser
    ...
    Dockerfile.test         # Container for CI: soulsystem + chromium
```

**Fixture: start soulsystem**

```python
@pytest.fixture(scope="session")
async def soulsystem_server():
    """Start soulsystem with gateway enabled, return base URL."""
    env = {
        "CLI_ENABLED": "false",
...
```python
@pytest.fixture
async def browser_agent(soulsystem_server):
    """Playwright browser + Claude computer use agent."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page(viewport={"width": 1280, "height": 720})
        await page.goto(f"{soulsystem_server}/?token=test-token-e2e")
...
```python
async def test_sse_reconnect_preserves_history(browser_agent, soulsystem_server):
    ...
    await restart_soulsystem(soulsystem_server)
...
```python
async def test_onboarding_wizard_completes(tmp_soulsystem_home):
    """Bugs: #187, #174, #129, #185 (wizard persistence and re-trigger)"""
    # Start soulsystem with a fresh home directory (no prior config)
    ...
    proc = pexpect.spawn(
        "cargo run",
        env={"SOULSYSTEM_HOME": str(tmp_soulsystem_home), **base_env},
        timeout=60,
    )

    # Step through wizard
    proc.expect("Welcome to SoulSystem")
    ...
    proc2.expect("Agent soulsystem ready")  # Should skip wizard
    # Must NOT see "Welcome to SoulSystem" again
    assert not proc2.match_any(["Welcome to SoulSystem"], timeout=5)
    proc2.close()
```
