...
**Date:** 2026-02-24
**Status:** Approved
**Goal:** Deterministic browser-level E2E tests for the SoulSystem web gateway using Python + Playwright, with a mock LLM backend for CI reliability.
...
2. Session-scoped fixture builds soulsystem binary (or reuses cached)
3. Session-scoped fixture starts mock LLM on OS-assigned port
4. Session-scoped fixture starts soulsystem subprocess pointing to mock LLM, gateway on OS-assigned port, libSQL in-memory
...
7. Teardown kills soulsystem and mock LLM
...
```
tests/e2e/
  conftest.py              # pytest fixtures: build binary, start soulsystem, mock LLM, browser
...
- Supports `stream: true` by returning SSE chunks with `delta` objects (critical: SoulSystem streams responses via SSE to the browser)
...
**`soulsystem_binary`**
- Checks if `./target/debug/soulsystem` exists
...
**`soulsystem_server(soulsystem_binary, mock_llm_server)`**
- Starts the soulsystem binary with environment:
...
- Parses actual gateway port from soulsystem stdout (`Gateway listening on 127.0.0.1:XXXX`)
...
SoulSystem logs `Gateway listening on 127.0.0.1:XXXX` at startup. The fixture reads stdout line-by-line until it finds this pattern, extracts the port.
...
async def wait_for_port(process, pattern=r"Gateway listening on .+:(\d+)", timeout=60):
    ...
    raise TimeoutError("soulsystem did not report listening port")
```
...
1. `pytest tests/e2e/ -v` passes locally with a pre-built soulsystem binary
