...
Already defined in `crates/soulsystem_engine/src/types/mission.rs`.
...
`crates/soulsystem_engine/src/runtime/mission.rs` — already has types, needs execution logic:
...
`crates/soulsystem_engine/src/runtime/mission_prompt.rs`:
...
```python
mission_create(
    goal="Increase SoulSystem test coverage from 60% to 80%",
