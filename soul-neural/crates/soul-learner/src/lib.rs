...
impl QAScenario {
    pub fn new() -> Self {
        Self {
            questions: vec![
                (
                    "Quelle est la capitale de la France ?".into(),
                    "Paris".into(),
                ),
                ("2 + 2 ?".into(), "4".into()),
            ],
...
mod tests {
    ...
    async fn test_qascenario_run() {
        ...
        let identity = Arc::new(Mutex::new(soul_identity::Identity::new(
            embedder,
            Arc::new(store),
        )));
        let tools = Arc::new(Mutex::new(soul_tools::ToolOrchestrator::new(
            Embedder::new(&["test"]),
        )));
