...
pub struct DriveRegistry {
    pub drives: Vec<HomeostaticDrive>,
}
impl DriveRegistry {
    pub fn new_instantiated() -> Self {
        Self {
            drives: vec![
                HomeostaticDrive {
                    drive_type: DriveType::SelfPreservation,
                    current_value: 1.0,
                    target_setpoint: 1.0,
                ...
                HomeostaticDrive {
                    drive_type: DriveType::ArchitectAlignment,
                    current_value: 1.0,
                    target_setpoint: 1.0,
    ...
    pub fn compute_homeostatic_loss(&self, _state: &AffectiveState) -> f32 {
        ...
        for drive in &self.drives {
            ...
            total_loss += drive.weight * (deviation * deviation);
        }
        total_loss
    }
    pub fn decay_drives(&mut self, penalty: f32) {
        for drive in &mut self.drives {
            if drive.drive_type == DriveType::SelfPreservation {
                drive.current_value -= penalty * 0.5;
            } else if drive.drive_type == DriveType::Curiosity {
                drive.current_value += penalty * 0.2;
            }
