...
impl AffectiveState {
    ...
    pub fn decay_towards_baseline(&self, dt: f32, baseline: &[f32; 3], decay_rates: &[f32; 3]) {
        ...
        for val in data.iter_mut() { if *val > 1.0 { *val = 1.0; } else if *val < -1.0 { *val = -1.0; } }
    }
    pub fn apply_stimulus(&self, stimulus: &Tensor) {
        assert_eq!(stimulus.data.len(), 3); let mut coords_lock = self.coordinates.write(); let data = &mut coords_lock.data;
        data[0] += stimulus.data[0]; data[1] += stimulus.data[1]; data[2] += stimulus.data[2];
        for val in data.iter_mut() { if *val > 1.0 { *val = 1.0; } else if *val < -1.0 { *val = -1.0; } }
    }
