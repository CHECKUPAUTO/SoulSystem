...
for crate_dir in crates/*/src; do
  ...
  for f in $all_files; do
    ...
    if [[ -n "$declared" ]] && grep -qxF "$stem" <<< "$declared"; then
      continue
    fi
    ...
    if [ "$dir" != "." ]; then
      ...
      if [[ -n "$declared" ]] && grep -qxF "$parent_stem" <<< "$declared"; then
        # Le parent est déclaré, le sous-module devrait être dedans
        # Vérification rapide : chercher "mod $stem;" dans le parent
        parent_declares=0
        for parent_candidate in "$crate_dir/$dir/mod.rs" "$crate_dir/$dir.rs"; do
          if [ -f "$parent_candidate" ]; then
            count=$(grep -cE "^[[:space:]]*(pub[[:space:]]+)?mod[[:space:]]+${stem}[[:space:]]*;" "$parent_candidate" 2>/dev/null || true)
            parent_declares=$((parent_declares + ${count:-0}))
          fi
        done
