...
async fn v1_headless_navigate(
    ...
    // Fill form fields using native CDP events via session.type_into (defensive against JS injection)
    for f in &req.fill {
        let parts: Vec<&str> = f.splitn(2, '=').collect();
        if parts.len() == 2 {
            let _ = session.type_into(parts[0], parts[1]).await;
        }
    }

    // Click elements using native CDP events via session.click (defensive against JS injection)
    for sel in &req.click {
        let _ = session.click(sel).await;
    }
