...
fn script_regex() -> &'static Regex {
    ...
    RE.get_or_init(|| Regex::new(r"<script[^\u003e]*\u003e").unwrap())
}

fn link_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"<link[^\u003e]*\u003e").unwrap())
}
...
pub fn compute_budget(html: &str) -> PerformanceBudget {
    // Optimization: Use pre-compiled regexes to avoid redundant compilation
    // and expensive DFA construction for every call to compute_budget.
    let js_requests = script_regex().find_iter(html).count();
    let css_requests = link_regex()
        .find_iter(html)
        .filter(|m| m.as_str().to_lowercase().contains("stylesheet"))
        .count();
    let image_requests = img_regex().find_iter(html).count();
