...
impl HeadlessSession {
    ...
    /// Click an element identified by a CSS selector using native CDP events.
    ///
    /// # Errors
    /// Returns `HeadlessError::Page` if the element isn't found or click fails.
    pub async fn click(&self, selector: &str) -> Result<(), HeadlessError> {
        let page = self.page()?;
        let element = page
    ...
    pub async fn type_into(&self, selector: &str, text: &str) -> Result<(), HeadlessError> {
        ...
        // Clear via JS first. Use JSON serialization for safe escaping of the selector.
        let selector_json =
            serde_json::to_string(selector).map_err(|e| HeadlessError::Page(e.to_string()))?;
        let clear_js =
            format!("const el = document.querySelector({selector_json}); if(el) el.value = '';");
