...
const REPO_URL    = 'https://github.com/CHECKUPAUTO/openclaw-gateway.git';
const CARGO_BIN   = process.env.CARGO_BIN || 'cargo';
const INSTALL_DIR = process.env.OPENCLAW-GATEWAY_HOME || path.join(os.homedir(), '.openclaw-gateway');
const SRC_DIR     = path.join(INSTALL_DIR, 'src');
const BIN_DIR     = path.join(INSTALL_DIR, 'bin');
const BIN_PATH    = path.join(BIN_DIR, 'openclaw-gateway');
const VERSION_FILE = path.join(INSTALL_DIR, '.version');
const BRANCH      = 'main';
const SUBDIR      = '';

function log(msg)   { console.log(`[openclaw-gateway] ${msg}`); }
function warn(msg)  { console.warn(`[openclaw-gateway] ⚠  ${msg}`); }
function err(msg)   { console.error(`[openclaw-gateway] ❌ ${msg}`); process.exit(1); }
function ok(msg)    { console.log(`[openclaw-gateway] ✅ ${msg}`); }
...
async function install() {
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   openclaw-gateway v2.0.0 Installer          ║');
    console.log('╚══════════════════════════════════════════════╝\n');
    ...
    const builtBin = path.join(buildDir, 'openclaw-gateway');
...
function postinstall() {
    console.log('');
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   openclaw-gateway installed via npm           ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
    console.log('  Install binary:  npx openclaw-gateway-install install');
    console.log('  Update binary:   npx openclaw-gateway-install update');
    console.log('');
}
...
(async () => {
    switch (cmd) {
        ...
        case 'version': case '--version': case '-v':
            console.log('openclaw-gateway npm installer v2.0.0'); break;
        default:
            console.log('openclaw-gateway Installer\n');
            console.log('  npx openclaw-gateway-install install   Install from source');
            console.log('  npx openclaw-gateway-install update    Update to latest');
            console.log('  openclaw-gateway                           Run after install\n');
    }
})();
