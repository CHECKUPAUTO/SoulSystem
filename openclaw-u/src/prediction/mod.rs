...
impl Predictor {
    ...
    pub fn predict(&self,
        ...
        info!("🔮 PREDICT: dans {} cycles | CPU:{:.0}% MEM:{:.0}% DISK:{:.0}% | conf={:.2}",
    ...
    pub fn will_alert(&self,
        ...
        if pred.cpu > cpu_threshold {
            return Some(format!("ALERTE CPU imminente: {:.0}% dans {} cycles", pred.cpu, steps_ahead));
        }
        if pred.mem > mem_threshold {
            return Some(format!("ALERTE MEM imminente: {:.0}% dans {} cycles", pred.mem, steps_ahead));
        }
        if pred.disk > disk_threshold {
            return Some(format!("ALERTE DISK imminente: {:.0}% dans {} cycles", pred.disk, steps_ahead));
        }
