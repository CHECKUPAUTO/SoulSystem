...
use scirust_solvers::linalg::{qr_decompose, solve_qr_least_squares, Matrix};
use scirust_stats::{describe, SplitMix64};
...
pub fn fit_high_breakdown(
    ...
    for _ in 0..config.subset_starts {
        ...
        let Some(start) = solve_ols(features, targets, &subset, config.fit_intercept) else {
            singular_starts += 1;
            continue;
        };
...
fn mm_refine(
    ...
    for _ in 0..maximum_iterations {
        ...
        let weights: Vec<f64> = (0..n)
            ...
            .collect();
        let Some(next) = solve_weighted(features, targets, &weights, fit_intercept) else {
            break;
        };
        let change: f64 = beta
            .iter()
