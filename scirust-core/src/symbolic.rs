...
impl PatternMemory {
    ...
    /// Record a pattern with input/output expressions and success flag.
    pub fn record(&mut self, input: &Expr, output: &Expr, success: bool) {
        let key = format!("{}", input);
        let val = eval(output, &HashMap::new()).unwrap_or(0.0);
        *self.hits.entry(key.clone()).or_insert(0) += 1;
        self.patterns.insert(key.clone(), vec![val]);
        let delta = if success { 1.0 } else { -0.5 };
        let entry = self.scores.entry(key).or_insert(0.0);
        *entry = (*entry + delta).max(0.0);
    }

    /// Return the top N patterns sorted by score (highest first).
    pub fn best_patterns(&self, n: usize) -> Vec<String> {
        let mut scored: Vec<(&String, &f64)> = self.scores.iter().collect();
