...
fn main() {
    ...
    for (family, make) in families {
        ...
        for &r in r_dict.iter().chain(two_hops.iter()) {
            ...
            let Some(row) = slack_of(r, support, &points) else {
                continue;
            };
    ...
    for (family, make) in families {
        ...
        for &c in &two_hops {
            let RepresentationChoice::Composed(a, b) = c else {
                continue;
            };
            let Some(row_c) = slack_of(c, support, &points) else {
                continue;
            };
            let hop1 = RepresentationChoice::Certified(a);
            let Some(row_1) = slack_of(hop1, support, &points) else {
                continue;
            };
            ...
            let encoded: Vec<f64> = points
                ...
                .filter(|&e| e != 0.0)
                .collect();
            let Some(row_2) = slack_of(RepresentationChoice::Certified(b), image, &encoded) else {
                continue;
            };
