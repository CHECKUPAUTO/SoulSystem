...
impl ReplState {
    pub fn new(config: LlmConfig) -> Self {
        ...
        Self {
            ...
            conversations: soul_memory::ConversationStore::open(&data_dir.join("conversations"))
                .ok(),
            session_id: None,
...
async fn handle_input(state: &mut ReplState, input: &str) {
    ...
    match cmd.as_str() {
        ...
        "ask" => {
            ...
            let enriched_prompt =
                if !state.graph.find_nodes_by_label(args).is_empty() || args.len() > 5 {
        ...
        "run" | "task" => {
            ...
            let design_id = state
                .design_tree
                .create_node(args, "Auto-created from 'run' command");
            state
                .design_tree
                .transition_node(
                    &design_id,
                    soul_designtree::DesignState::Implementation,
                    Some("auto-started".into()),
                )
                .ok();
            ...
            match state.agent.run_task(args).await {
                Ok(result) => {
                    ...
                    let design_nodes: Vec<String> = state
                        .design_tree
                        .find_by_state(&soul_designtree::DesignState::Implementation)
                        .into_iter()
                        .map(|n| n.id.clone())
                        .collect();
                    for id in &design_nodes {
                        if state
                            ...
                            .transition_node(
                                id,
                                soul_designtree::DesignState::Testing,
        ...
        "plan" => {
            ...
            let plan = state
                .agent
                .planner
                .create_plan_llm(&goal, &[], &state.agent.llm)
                .await;
        ...
        "memory" => {
            ...
            println!(
                "  History: {} actions",
                state.agent.planner.history.actions.len()
            );
            println!(
                "  Success Rate: {:.1}%",
                state.agent.planner.history.success_rate() * 100.0
            );
        }
        ...
        "status" => {
            ...
            println!(
                "  Model: {}",
                status["llm_model"].as_str().unwrap_or("?").green()
            );
            ...
            println!(
                "  Conversation: {}",
                status["conversation"].as_str().unwrap_or("?")
            );
        }

        // ── Models ──
        "models" => match state.agent.llm.list_models().await {
            Ok(models) => {
                        ...
                        match store.load_active_goals() {
                            Ok(active) => match store.load_all_goals() {
                                Ok(all) => {
                                    let completed = all
                                        .iter()
                                        .filter(|g| g.status == soul_memory::GoalStatus::Completed)
                                        .count();
                                    let failed = all
                                        .iter()
                                        .filter(|g| g.status == soul_memory::GoalStatus::Failed)
                                        .count();
                    ...
                    match sub_args[0] {
                        "add" => {
                            if sub_args.len() < 2 || sub_args[1].is_empty() {
                                println!(
                                    "{}",
                                    "Usage: graph add <type> <label> [content]".yellow()
                                );
                            } else {
                        ...
                        "connect" => {
                            if sub_args.len() < 2 || sub_args[1].is_empty() {
                                println!(
                                    "{}",
                                    "Usage: graph connect <source_id> <target_id> <relation>"
                                        .yellow()
                                );
                            } else {
                                let parts: Vec<&str> = sub_args[1].splitn(3, ' ').collect();
                        ...
                        "state" => {
                            if sub_args.len() < 2 || sub_args[1].is_empty() {
                                ...
                                if parts.len() < 2 {
                                    ...
                                    match state
                                        .design_tree
                                        .transition_node(&full_id, state_val, None)
                                    {
                    ...
                    match sub_args[0] {
                            ...
                            if sub_args.len() < 2 || sub_args[1].is_empty() {
                                ...
                                match new_runtime().block_on(transport.connect_client()) {
                                    Ok((tx, rx)) => {
                                        ...
                                        let rt = new_runtime();
                                        match rt.block_on(client.initialize(
                                            soul_mcp::McpServerInfo {
                        ...
                        "ws" => {
                            if sub_args.len() < 2 || sub_args[1].is_empty() {
                                ...
                                let server = soul_mcp::create_soul_mcp_server(
                                    "soulsystem",
                                    Box::new(handler),
                                );
                                ...
                                match conv.list_sessions() {
                                    Ok(sessions) => {
                                        if sessions.is_empty() {
                                            ...
                                            for s in &sessions {
                                                let active =
                                                    if state.session_id.as_deref() == Some(&s.id) {
                                                        " ◀ active"
                                                    } else {
                                                        ""
                                                    };
...
fn print_help() {
    ...
    println!(
        "{}",
        "║   SoulSystem Autonomous Agent v0.3.0   ║".cyan().bold()
    );
    println!("{}", "║   Type 'help' for commands              ║".cyan());
    println!("{}", "╚══════════════════════════════════════════╝".cyan());
    println!();
    ...
    println!(
        "  {} - Run autonomous task (ReAct loop)",
        "run <task>".green()
    );
    ...
    println!(
        ...
        "fetch <url1> <url2>".green()
    );
    println!(
        "  {} - Search web and show results with relevance",
        "websearch <query>".green()
    );
    println!(
        "  {} - Ask question with web context",
        "askweb <question>".green()
    );
    println!();
    ...
    println!(
        "  {} - Match input to skills",
        "skills match <input>".green()
    );
    ...
    println!(
        "  {} - Connect nodes",
        "graph connect <src> <tgt> <rel>".green()
    );
    ...
    println!(
        "  {} - Create design node",
        "design new <name> <desc>".green()
    );
    println!("  {} - List all designs", "design list".green());
    println!(
        "  {} - Transition state",
        "design state <id> <state>".green()
    );
    ...
    println!(
        ...
        "conversations new [title]".green()
    );
    println!(
        "  {} - List all conversations",
        "conversations list".green()
    );
    println!("  {} - Show active session", "conversations active".green());
    println!(
        "  {} - Switch to a session",
        "conversations switch <id>".green()
    );
