...
impl ReplState {
    ...
    pub async fn status(&self) -> String {
        ...
        format!(
            "{}\n  Model: {}\n  Ollama: {} connecté\n  Taux succès actions: {:.0}%\n  Sandbox historique: {} exécutions\n  Mémoire persistante: {} entrées\n  Dernières observations: {:?}",
    ...
    pub async fn run_command(&mut self, cmd: &str) -> String {
        ...
        match parts[0] {
            "ask" => {
                self.cmd_ask(parts.get(1).map(|s| s.trim()).unwrap_or(""))
                    .await
            }
            ...
            _ => "❌ Commande inconnue. Tapez `help`.".to_string(),
        }
    }
    ...
    fn cmd_plan(&mut self, goal_desc: &str) -> String {
        ...
        let plan = self
            .planner
            .create_plan(&goal, &["analyze", "execute"]);
        format!(
            ...
            plan.steps
                .iter()
                .map(|s| format!("   - [{}] {}", s.order, s.command))
                .collect::<Vec<_>>()
                .join("\n")
        )
    }
    ...
    fn cmd_run(&mut self, shell_cmd: &str) -> String {
        if shell_cmd.is_empty() {
            return "❌ run <commande_shell>".into();
        }
    ...
    fn cmd_sandbox(&self, args: &str) -> String {
        ...
        match parts[0] {
            "history" => {
                ...
                if h.is_empty() {
                    return "📜 Historique sandbox vide".into();
                }
                h.iter()
                    .rev()
                    .take(10)
            ...
            "scan" => {
                ...
                if cmd.is_empty() {
                    return "❌ sandbox scan <cmd>".into();
                }
                let threats = self.sandbox.scan(cmd);
                if !threats.is_empty() {
                    format!("⛔ Menaces détectées: {:?}", threats)
                } else {
                    let paths = self.sandbox.scan_paths(cmd);
                    if !paths.is_empty() {
                        format!("⛔ Chemins sensibles: {:?}", paths)
                    } else {
                        match self.sandbox.check(cmd) {
    ...
    fn memory_browse(&self, kind: &str) -> String {
        ...
        if k == "all" {
            ...
            let ids = mem.list_by_kind(k);
            if ids.is_empty() {
                return format!("Aucune entrée de type {}", k.bright_yellow());
            }
            ids.iter()
                .take(10)
                .filter_map(|id| {
                    mem.get(id).ok().map(|e| format!("  - {} (tags: {:?})", &e.id[..8], e.tags))
                })
                .collect::<Vec<_>>()
                .join("\n")
        }
    ...
    fn tools_list(&self) -> String {
        ...
        if all.is_empty() {
            return "🔧 Aucun outil découvert.".into();
        }
        all.iter()
            .map(|t| format!("  {:20} {}", t.name.bright_yellow(), t.description))
            .collect::<Vec<_>>()
            .join("\n")
    }
    ...
    fn memory_list(&self) -> String {
        ...
        obs.iter()
            .enumerate()
            .map(|(i, o)| format!("  [{}] {}", i + 1, o.bright_white()))
            .collect::<Vec<_>>()
            .join("\n")
    }
    ...
    fn cmd_decide(&self, ctx: &str) -> String {
        if ctx.is_empty() {
            return "❌ decide <contexte>".into();
        }
        let decision = self.planner.decide(ctx);
        format!(
            "⚡ Décision: \"{}\"\n   Raisonnement: {}\n   Confiance: {:.0}%",
            decision.action.bright_yellow(),
            decision.reasoning,
...
pub async fn run_repl(state: &mut ReplState) {
    ...
    println!(
        "{}",
        "🧠 SoulSystem REPL — Entité autonome".bright_blue().bold()
    );
