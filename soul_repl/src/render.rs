...
impl App {
    ...
    fn draw_status_bar(&self, f: &mut Frame, area: Rect) {
        ...
        let status_line = Line::from(vec![
            ...
            Span::styled(
                format!(" {} ", self.model_items.first().unwrap_or(&String::new())),
            ...
            Span::raw(" | "),
            Span::styled(
                format!("{} cmds", self.command_count),
                Style::default().fg(Color::DarkGray),
            ),
            Span::raw(" | "),
            Span::styled(
    ...
    fn draw_input(&self, f: &mut Frame, area: Rect) {
        ...
        let block = Block::default()
            .borders(Borders::ALL)
            .title(" Input (Shift+Enter multi-ligne) ")
    ...
    fn draw_provider_dialog(&self, f: &mut Frame, size: Rect) {
        ...
        let block = Block::default()
            .borders(Borders::ALL)
            .title(" Selectionner un provider ")
            .border_style(Style::default().fg(Color::Cyan))
            .style(Style::default().bg(Color::Black));
        let items: Vec<ListItem> = self
            .provider_items
    ...
    fn draw_file_browser(&self, f: &mut Frame, size: Rect) {
        ...
        state.select(Some(
            self.file_browser_selected
                .saturating_sub(self.file_browser_scroll),
        ));
    ...
    fn draw_history_search(&self, f: &mut Frame, size: Rect) {
        ...
        let block = Block::default()
            .borders(Borders::ALL)
            .title(" Recherche historique (Ctrl+R) ")
            .border_style(Style::default().fg(Color::Cyan))
            .style(Style::default().bg(Color::Black));
        let inner = block.inner(area);
        f.render_widget(block, area);
        ...
        if let Some(ref search) = self.history_search {
            ...
            let list = List::new(results).highlight_style(
                Style::default()
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::BOLD),
            );
    ...
    fn draw_session_manager(&self, f: &mut Frame, size: Rect) {
        ...
        if self.sessions.is_empty() {
            let empty =
                Paragraph::new("Aucune session sauvegardee.\nUtilisez Ctrl+S pour sauvegarder.")
