...
pub fn encode(
    ...
    for i in 0..n {
        ...
        let unit_row = &mut unit_flat[i * dim..(i + 1) * dim];
    ...
    // Matrix multiply: rotated = unit_mat (n×dim) * rot_mat^T (dim×dim)
    let mut rotated = vec![0.0f32; n * dim];
    for i in 0..n {
        for j in 0..dim {
            let mut sum = 0.0f32;
            for k in 0..dim {
                sum += unit_flat[i * dim + k] * rotation[j * dim + k];
            }
            rotated[i * dim + j] = sum;
        }
    }
    ...
    for i in 0..n {
        let calib_row = &mut rotated_calib[i * dim..(i + 1) * dim];
    ...
    for i in 0..n {
        ...
        let packed_row = &mut packed[i * bytes_per_row..(i + 1) * bytes_per_row];

        scales[i] = fused_quantize_scale_pack(
            rot_orig,
            rot_calib,
            &shift,
            &inv_scale_tq,
...
fn compute_tqplus_calibration(
    ...
    // Use the Beta LUT for canonical quantiles instead of statrs::Beta
    let a_val = (dim as f64 - 1.0) / 2.0;
    let beta_cdf = crate::beta_lut::BetaCDF::new(dim);
    let qc_lo = (2.0 * beta_cdf.inverse_cdf(TQPLUS_P_LO as f32) - 1.0) as f32;
    let qc_hi = (2.0 * beta_cdf.inverse_cdf(TQPLUS_P_HI as f32) - 1.0) as f32;
    let qc_span = qc_hi - qc_lo;

    let lo_idx = ((n as f64) * TQPLUS_P_LO) as usize;
    let hi_idx = (((n as f64) * TQPLUS_P_HI) as usize).min(n - 1);

    // Each coord is independent — collect, sort, extract quantiles.
    for d in 0..dim {
        ...
        if qe_span > 1e-6 {
            scale[d] = qc_span / qe_span;
            shift[d] = qc_lo / scale[d] - qe_lo;
        }
    }
