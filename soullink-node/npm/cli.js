...
const REPO_URL    = 'https://github.com/CHECKUPAUTO/soullink-node.git';
const CARGO_BIN   = process.env.CARGO_BIN || 'cargo';
const INSTALL_DIR = process.env.SOULLINK-NODE_HOME || path.join(os.homedir(), '.soullink-node');
const SRC_DIR     = path.join(INSTALL_DIR, 'src');
const BIN_DIR     = path.join(INSTALL_DIR, 'bin');
const BIN_PATH    = path.join(BIN_DIR, 'soullink-node');
const VERSION_FILE = path.join(INSTALL_DIR, '.version');
const BRANCH      = 'main';
const SUBDIR      = '';

function log(msg)   { console.log(`[soullink-node] ${msg}`); }
function warn(msg)  { console.warn(`[soullink-node] ⚠  ${msg}`); }
function err(msg)   { console.error(`[soullink-node] ❌ ${msg}`); process.exit(1); }
function ok(msg)    { console.log(`[soullink-node] ✅ ${msg}`); }
...
async function install() {
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   soullink-node v6.0.0 Installer          ║');
    console.log('╚══════════════════════════════════════════════╝\n');
    ...
    const builtBin = path.join(buildDir, 'soullink-node');
...
function postinstall() {
    console.log('');
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   soullink-node installed via npm           ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
    console.log('  Install binary:  npx soullink-node-install install');
    console.log('  Update binary:   npx soullink-node-install update');
    console.log('');
}
...
(async () => {
    switch (cmd) {
        ...
        case 'version': case '--version': case '-v':
            console.log('soullink-node npm installer v6.0.0'); break;
        default:
            console.log('soullink-node Installer\n');
            console.log('  npx soullink-node-install install   Install from source');
            console.log('  npx soullink-node-install update    Update to latest');
            console.log('  soullink-node                           Run after install\n');
    }
})();
